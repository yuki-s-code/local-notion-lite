...
import {
  ...
  type IncrementalSemanticTarget,
} from "../../shared/semantic/semanticTargetPolicy";
import type { Db } from "../db/sqlite";
import { upsertPageFts } from "../db/sqlite";
...
import { WorkspacePageSearchService } from "./search/workspacePageSearchService";
import { WorkspaceLinkQueryService } from "./links/workspaceLinkQueryService";
import { EditorAiEditService } from "./ai/editorAiEditService";
import {
  PageSaveCoordinator,
  type SavePageInput,
} from "./page/pageSaveCoordinator";
import {
  createVaultDomainServices,
  type VaultDomainServices,
} from "./domains/vaultDomainServices";
...
export class VaultService {
  /** Route-facing domain facade; keeps page/link/search/AI contracts independent from this legacy aggregate. */
  readonly domains: VaultDomainServices;
  private attachmentIndexRebuildJob: {
  ...
  private readonly workspacePageSearchService: WorkspacePageSearchService;
  private readonly workspaceLinkQueryService: WorkspaceLinkQueryService;
  private readonly editorAiEditService: EditorAiEditService;
  private readonly pageSaveCoordinator: PageSaveCoordinator;
  ...
  constructor(
    ...
    this.domains = createVaultDomainServices(this);
    this.analysisNotebookService = new AnalysisNotebookService(
      this.db,
      this.localDbPath,
    );
    this.workspacePageSearchService = new WorkspacePageSearchService({
      db: this.db,
      listPages: () => this.listPages(),
      withLock: (page) => this.withLock(page),
    });
    this.workspaceLinkQueryService = new WorkspaceLinkQueryService({
      db: this.db,
      getPage: (pageId) => this.getPage(pageId),
    });
    this.editorAiEditService = new EditorAiEditService({
      getSettings: () => this.getSmartAssistGenerationSettings(),
      checkEngine: () => this.checkSmartAssistGenerationEngine(),
      runGeneration: (prompt, settings, engine) =>
        this.runLlamaGeneration(prompt, settings, engine),
      cleanGeneratedText: (value, prompt) =>
        this.cleanLlamaGeneratedText(value, prompt),
    });
    this.pageSaveCoordinator = new PageSaveCoordinator({
      getCurrent: (pageId) => this.getPage(pageId),
      readSharedMeta: (pageId) => this.readSharedMeta(pageId),
      handleConflict: (current, input, sharedMeta) =>
        this.handlePageSaveConflict(current, input, sharedMeta),
      resolveHistoryReason: (current, input) =>
        input.historyReason ??
        (pageMetadataChanged(current, input) ? "metadata_changed" : undefined),
      matchesCurrent: (current, input) =>
        pageSaveMatchesCurrent(current, input),
      backupHistory: async (bundle, reason) => {
        await this.pageHistoryService.backup(bundle, reason, {
          deduplicate: true,
        });
      },
      buildBundle: (current, input) =>
        this.buildSavedPageBundle(current, input),
      persistBundle: (current, bundle) =>
        this.persistSavedPageBundle(current, bundle),
    });
    ...
    this.journalService = new JournalService({
      ...
      onDeleted: async (date) => {
        try {
          ...
          void this.updateWorkspaceSummaryCache().catch((error) => {
            console.warn(
              "[workspace summary] background refresh failed",
              error,
            );
          });
        } catch {}
  ...
  private workspaceGlossaryPath(): string {
    return path.join(vaultPaths(this.sharedRoot).workspace, "glossary.json");
  }

  private normalizeWorkspaceGlossary(input: unknown): GlossaryTerm[] {
    const rawTerms = Array.isArray(input)
      ? input
      : input &&
          typeof input === "object" &&
          Array.isArray((input as any).terms)
        ? (input as any).terms
        : [];
    const now = new Date().toISOString();
    const result: GlossaryTerm[] = [];
    const seenTerms = new Set<string>();
    for (const raw of rawTerms.slice(0, 500)) {
      ...
      const aliases = Array.from(
        new Set(
          (Array.isArray(item.aliases) ? item.aliases : [])
            ...
            .filter(
              (value) =>
                value.length >= 2 && value.length <= 120 && value !== term,
            ),
        ),
      ).slice(0, 30);
      ...
      const status: GlossaryStatus =
        item.status === "verified" || item.status === "deprecated"
          ? item.status
          : "draft";
      const sourcePageIds = Array.from(
        new Set(
          (Array.isArray(item.sourcePageIds) ? item.sourcePageIds : [])
      ...
      result.push({
        id:
          String(item.id ?? `term_${nanoid(14)}`)
            .replace(/[^a-zA-Z0-9_-]/g, "")
            .slice(0, 80) || `term_${nanoid(14)}`,
        term,
        aliases,
        summary,
        ...
        ...(typeof item.verifiedAt === "string" && item.verifiedAt.trim()
          ? { verifiedAt: item.verifiedAt.trim().slice(0, 80) }
          : {}),
        ...(typeof item.reviewDue === "string" && item.reviewDue.trim()
          ? { reviewDue: item.reviewDue.trim().slice(0, 80) }
          : {}),
        ...(typeof item.owner === "string" && item.owner.trim()
          ? { owner: item.owner.trim().slice(0, 120) }
          : {}),
        updatedAt:
        ...
        updatedBy:
          typeof item.updatedBy === "string" && item.updatedBy
            ? item.updatedBy.slice(0, 120)
            : this.userLabel(),
      });
    }
    return result.sort((a, b) => a.term.localeCompare(b.term, "ja-JP"));
  }

  private glossaryNameKey(value: unknown): string {
    return String(value ?? "")
      .normalize("NFKC")
  ...
  /** A visible term/alias must resolve to exactly one definition. */
  private assertWorkspaceGlossaryNamesUnique(terms: GlossaryTerm[]): void {
    const owners = new Map<string, { term: string; label: string }>();
    for (const item of terms) {
      for (const rawName of [item.term, ...(item.aliases ?? [])]) {
        const key = this.glossaryNameKey(rawName);
        if (key.length < 2) continue;
        const previous = owners.get(key);
        if (previous && previous.term !== item.term) {
          throw new Error(
            `Glossary validation failed: 「${String(rawName).trim()}」は「${previous.term}」と「${item.term}」で重複しています。`,
          );
        }
        owners.set(key, { term: item.term, label: String(rawName).trim() });
      }
    }
  }

  private async readWorkspaceGlossaryDocument(): Promise<{
    revision: number;
    terms: GlossaryTerm[];
  }> {
    const raw = await fs
      .readJson(this.workspaceGlossaryPath())
      .catch(() => ({}));
    return {
      revision:
        Number.isSafeInteger(raw?.revision) && raw.revision >= 0
          ? raw.revision
          : 0,
      terms: this.normalizeWorkspaceGlossary(raw?.terms ?? raw),
    };
  }
  ...
  async getWorkspaceGlossary(): Promise<{
    ...
    return this.readWorkspaceGlossaryDocument();
  }

  async updateWorkspaceGlossary(input: {
    terms: unknown;
    baseTerms?: unknown;
    baseRevision?: number;
  }): Promise<{ revision: number; terms: GlossaryTerm[]; merged: boolean }> {
    const file = this.workspaceGlossaryPath();
    const desired = this.normalizeWorkspaceGlossary(input.terms);
    const base = this.normalizeWorkspaceGlossary(input.baseTerms ?? desired);
    return this.withSharedJsonMutation(file, async () => {
      const current = await this.readWorkspaceGlossaryDocument();
      let next = desired;
      const stale =
        typeof input.baseRevision === "number" &&
        input.baseRevision !== current.revision;
      if (stale) {
        const byId = (items: GlossaryTerm[]) =>
          new Map(items.map((item) => [item.id, item]));
        const baseMap = byId(base);
        const desiredMap = byId(desired);
        const currentMap = byId(current.terms);
        const changedByClient = new Set<string>();
        for (const id of new Set([...baseMap.keys(), ...desiredMap.keys()])) {
          if (
            JSON.stringify(baseMap.get(id) ?? null) !==
            JSON.stringify(desiredMap.get(id) ?? null)
          )
            changedByClient.add(id);
        }
        for (const id of changedByClient) {
          const wanted = desiredMap.get(id);
          if (wanted) currentMap.set(id, wanted);
          else currentMap.delete(id);
        }
        next = this.normalizeWorkspaceGlossary(Array.from(currentMap.values()));
      }
      this.assertWorkspaceGlossaryNamesUnique(next);
      if (JSON.stringify(current.terms) === JSON.stringify(next))
        return { ...current, merged: stale };
      ...
      return { revision, terms: next, merged: stale };
    });
  }

  /**
   * Reads lightweight SQLite indexes only. This endpoint is called only from the
   * glossary screen for the selected term, never while an editor is typing.
   */
  async getWorkspaceGlossaryInsight(termId: string): Promise<GlossaryTermInsight> {
    const glossary = await this.readWorkspaceGlossaryDocument();
    const term = glossary.terms.find((item) => item.id === termId);
    if (!term) throw new Error("用語が見つかりません。");
    const names = Array.from(new Set([term.term, ...(term.aliases || [])]))
      .map((value) => this.glossaryNameKey(value))
      .filter((value) => value.length >= 2)
      .slice(0, 31);
    const makeWhere = (column: string) => names.map(() => `instr(lower(${column}), lower(?)) > 0`).join(" OR ");
    const pageRows = names.length ? this.db.prepare(`SELECT page_id as id,title,updated_at as updatedAt FROM page_search_index WHERE trashed = 0 AND (${makeWhere("search_text")}) ORDER BY updated_at DESC LIMIT 12`).all(...names) as Array<{id:string;title:string;updatedAt:string}> : [];
    const dbRows = names.length ? this.db.prepare(`SELECT r.database_id as databaseId,r.row_id as rowId,r.title_text as title,r.updated_at as updatedAt FROM database_row_index r JOIN database_summary_index d ON d.database_id=r.database_id WHERE d.trashed=0 AND (${makeWhere("r.search_text")}) ORDER BY r.updated_at DESC LIMIT 12`).all(...names) as Array<{databaseId:string;rowId:string;title:string;updatedAt:string}> : [];
    const journalRows = names.length ? this.db.prepare(`SELECT date as id,title,updated_at as updatedAt FROM journal_summary_index WHERE (${makeWhere("full_text")}) ORDER BY updated_at DESC LIMIT 12`).all(...names) as Array<{id:string;title:string;updatedAt:string}> : [];
    const countMatches = (table:string, column:string, extra = "") => names.length ? Number((this.db.prepare(`SELECT COUNT(*) as count FROM ${table} WHERE ${extra ? `${extra} AND ` : ""}(${makeWhere(column)})`).get(...names) as any)?.count || 0) : 0;
    const pages = countMatches("page_search_index", "search_text", "trashed=0");
    const databaseRows = countMatches("database_row_index r JOIN database_summary_index d ON d.database_id=r.database_id", "r.search_text", "d.trashed=0");
    const journals = countMatches("journal_summary_index", "full_text");
    const aliasUsage = (term.aliases || []).filter((alias) => this.glossaryNameKey(alias).length >= 3).slice(0, 30).map((alias) => {
      const key = this.glossaryNameKey(alias);
      // Query only this alias; canonical usage is intentionally not included in replacement suggestions.
      const pageCount = Number((this.db.prepare("SELECT COUNT(*) as count FROM page_search_index WHERE trashed=0 AND instr(lower(search_text), lower(?)) > 0").get(key) as any)?.count || 0);
      const dbCount = Number((this.db.prepare("SELECT COUNT(*) as count FROM database_row_index r JOIN database_summary_index d ON d.database_id=r.database_id WHERE d.trashed=0 AND instr(lower(r.search_text), lower(?)) > 0").get(key) as any)?.count || 0);
      return { alias, count: pageCount + dbCount };
    }).filter((item) => item.count > 0).sort((a,b) => b.count-a.count || a.alias.localeCompare(b.alias,"ja-JP"));
    const sourcePages = term.sourcePageIds.slice(0, 30).map((id) => this.db.prepare("SELECT id,title,updated_at as updatedAt FROM pages WHERE id=? AND trashed=0").get(id) as {id:string;title:string;updatedAt:string}|undefined).filter(Boolean) as Array<{id:string;title:string;updatedAt:string}>;
    const now = Date.now();
    const due = term.reviewDue ? Date.parse(term.reviewDue) : NaN;
    const sourceUpdated = sourcePages.some((page) => Date.parse(page.updatedAt) > Date.parse(term.verifiedAt || term.updatedAt || ""));
    // The glossary summary is the primary definition. Source pages are optional
    // supporting references, so their absence is informative rather than an error.
    const evidence = Number.isFinite(due) && due < now
      ? { state: "review_due" as const, message: "定義の見直し期限を過ぎています。" }
      : Number.isFinite(due) && due - now < 1000 * 60 * 60 * 24 * 30
        ? { state: "review_soon" as const, message: "定義の見直し期限が30日以内です。" }
        : sourceUpdated
          ? { state: "source_updated" as const, message: "補足資料が定義の確認後に更新されています。" }
          : !sourcePages.length
            ? { state: "definition_only" as const, message: "説明文を定義として運用しています。補足資料は未登録です。" }
            : { state: "healthy" as const, message: "定義と補足資料に、確認時点で見直しの必要は見つかっていません。" };
    const related = glossary.terms.filter((item) => item.id !== term.id && item.status === "verified").map((item) => {
      const shared = item.sourcePageIds.filter((id) => term.sourcePageIds.includes(id));
      const sameCategory = Boolean(term.category && item.category && term.category === item.category);
      return shared.length ? { termId:item.id, term:item.term, reason:`共通の補足資料 ${shared.length}件` } : sameCategory ? { termId:item.id, term:item.term, reason:`同じ分類：${term.category}` } : null;
    }).filter(Boolean).slice(0, 8) as Array<{termId:string;term:string;reason:string}>;
    const recentUsage: GlossaryUsageRef[] = [
      ...pageRows.map((row) => ({kind:"page" as const,id:row.id,title:row.title,updatedAt:row.updatedAt})),
      ...dbRows.map((row) => ({kind:"database-row" as const,id:`${row.databaseId}:${row.rowId}`,title:row.title || "無題の行",updatedAt:row.updatedAt,databaseId:row.databaseId,rowId:row.rowId})),
      ...journalRows.map((row) => ({kind:"journal" as const,id:row.id,title:row.title || row.id,updatedAt:row.updatedAt})),
    ].sort((a,b) => String(b.updatedAt||"").localeCompare(String(a.updatedAt||""))).slice(0, 12);
    return { termId:term.id, usage:{pages,databaseRows,journals,total:pages+databaseRows+journals}, recentUsage, aliasUsage, related, evidence:{...evidence,sourcePages} };
  }

  /** Explicit, capped discovery operation. It never runs while browsing pages. */
  async suggestWorkspaceGlossaryCandidates(): Promise<GlossaryCandidate[]> {
    const glossary = await this.readWorkspaceGlossaryDocument();
    const existing = new Set(glossary.terms.flatMap((item) => [item.term,...item.aliases]).map((value) => this.glossaryNameKey(value)));
    const rows = this.db.prepare("SELECT page_id as id,title,updated_at as updatedAt FROM page_search_index WHERE trashed=0 ORDER BY updated_at DESC LIMIT 3000").all() as Array<{id:string;title:string;updatedAt:string}>;
    const counts = new Map<string,{count:number;examples:GlossaryUsageRef[]}>();
    for (const row of rows) {
      const candidates = Array.from(String(row.title||"").matchAll(/[「『]([^」』]{3,40})[」』]|([A-Z][A-Z0-9_-]{2,})|([\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}]{3,32}(?:制度|事業|計画|基準|要綱|要領|指針|区分|保育|学級|委託|支援員|利用))/gu)).map((match) => String(match[1]||match[2]||match[3]||"").trim());
      for (const phrase of candidates) {
        const key=this.glossaryNameKey(phrase); if (key.length<3 || existing.has(key)) continue;
        const found=counts.get(key) || {count:0,examples:[]}; found.count += 1;
        if (found.examples.length<3) found.examples.push({kind:"page",id:row.id,title:row.title,updatedAt:row.updatedAt}); counts.set(key,found);
      }
    }
    return Array.from(counts.entries()).map(([phrase,value])=>({phrase,count:value.count,examples:value.examples})).filter((item)=>item.count>=2).sort((a,b)=>b.count-a.count||a.phrase.localeCompare(b.phrase,"ja-JP")).slice(0,40);
  }
  ...
  async initVault(): Promise<void> {
    ...
    this.vaultInitPromise = (async () => {
      ...
      // A shared SMB folder can add round-trip latency to every ensureDir call.
      // These roots are independent, so initialize them concurrently instead of
      // serially blocking API readiness behind a long chain of network requests.
      await Promise.all([
        fs.ensureDir(p.pages),
        fs.ensureDir(p.privatePages),
        fs.ensureDir(p.attachments),
        fs.ensureDir(p.locks),
        fs.ensureDir(p.backups),
        fs.ensureDir(p.databases),
        fs.ensureDir(p.privateDatabases),
        this.databaseRowContentService().ensureDirs(),
        fs.ensureDir(p.journals),
        fs.ensureDir(p.inbox),
        fs.ensureDir(p.smartAssist),
        fs.ensureDir(p.privateSmartAssist),
        fs.ensureDir(p.workspace),
        fs.ensureDir(p.conflicts),
      ]);
  ...
  private async ensureUnifiedResourceLinkIndex(): Promise<void> {
    ...
    if (this.unifiedResourceLinkIndexPromise)
      return this.unifiedResourceLinkIndexPromise;
  ...
  async importFromShared(): Promise<void> {
    ...
    const cachedSignatures = new Map(
      (
        this.db
          .prepare("SELECT page_id, signature FROM shared_page_file_state")
          .all() as Array<{ page_id: string; signature: string }>
    ...
    tx(bundles);
    // A shared import must preserve the same derived-index contract as local
    // mutations.  Trashed bundles own no active search/link/task/attachment
    // records; recreating them here would make deleted work reappear in views.
    const trashedBundleIds: string[] = [];
    for (const bundle of bundles) {
      if (bundle.meta.trashed) {
        trashedBundleIds.push(bundle.meta.id);
      } else {
        this.upsertPageDerivedIndexes(bundle);
      }
    }
    if (trashedBundleIds.length)
      this.removePageDerivedIndexes(trashedBundleIds);
  ...
  private async listDatabaseRowChildPageIdSet(): Promise<Set<string>> {
    const rows = this.db
      .prepare("SELECT id FROM pages WHERE parent_id LIKE 'database-row:%'")
  ...
  private extractDatabaseRowLinkRefs(text: string): Array<{
    ...
    const add = (
      ...
      const targetDatabaseId = safeDecodeURIComponent(
        String(rawDatabaseId || ""),
      ).trim();
      const targetRowId = safeDecodeURIComponent(String(rawRowId || ""))
        .split("&")[0]
        .trim();
    ...
    const patterns: Array<{
      ...
      {
        re: /local-dbrow:\/\/([^\/\s)\]\"']+)\/([^\s)\]\"']+)/g,
        kind: "local-dbrow-url",
        database: 1,
        row: 2,
      },
      {
        re: /#local-dbrow=([^&\s)\]\"']+)&row=([^\s)\]\"']+)/g,
        kind: "hash-local-dbrow",
        database: 1,
        row: 2,
      },
    ];
    for (const { re, kind, database, row } of patterns) {
      ...
      while ((match = re.exec(source)))
        add(match[database] || "", match[row] || "", kind, match.index);
    }
    return found;
  }
  ...
  private existingActivePageIds(pageIds: Iterable<string>): Set<string> {
    ...
    for (let offset = 0; offset < ids.length; offset += 800) {
      ...
      const rows = this.db
        .prepare(
          `SELECT id FROM pages WHERE trashed = 0 AND id IN (${placeholders})`,
        )
  ...
  private upsertDatabaseRowLinkIndex(
    ...
    const tx = this.db.transaction(() => {
      ...
      const indexedPageTargets = new Set<string>();
      const structuralChildPageIds = new Set<string>(
        rowContent.childPageIds || [],
      );
      for (const item of this.db
        .prepare("SELECT id FROM pages WHERE trashed = 0 AND parent_id = ?")
        .all(`database-row:${database.id}:${rowContent.rowId}`) as Array<{
        id: string;
      }>) {
      ...
      for (const pageId of structuralChildPageIds) {
        ...
        const stable = createHash("sha1")
          .update(
            `${database.id}|${rowContent.rowId}|${pageId}|database-child-page`,
          )
          .digest("hex");
        insertLink.run(
          `dbrow-child:${stable}`,
      ...
      for (const link of pageLinkRefs) {
        if (
          !existingIds.has(link.targetPageId) ||
          indexedPageTargets.has(link.targetPageId)
        )
          continue;
        indexedPageTargets.add(link.targetPageId);
        const stable = createHash("sha1")
          .update(
            `${database.id}|${rowContent.rowId}|${link.targetPageId}|${link.kind}|${link.snippet}`,
          )
          .digest("hex");
        insertLink.run(
          `dbrow:${stable}`,
      ...
      for (const link of this.extractDatabaseRowLinkRefs(haystack)) {
        ...
        const stable = createHash("sha1")
          .update(
            `${database.id}|${rowContent.rowId}|${targetKey}|${link.kind}|${link.snippet}`,
          )
          .digest("hex");
        insertLink.run(
          `dbrow-dbrow:${stable}`,
  ...
  /**
   * Explicit maintenance action: rebuilds the derived attachment catalog from
   * page, Journal, and DB-row attachment JSON. This is intentionally not run
   * at startup; regular uploads update only their one source row.
   */
  async rebuildAttachmentIndex(
    ...
    const rowSources = this.db
      .prepare(
        "SELECT database_id as databaseId, row_id as rowId, title_text as title, updated_at as updatedAt FROM database_row_index",
      )
      .all() as Array<{
      databaseId: string;
      rowId: string;
    ...
    const sources = [
      ...
      ...journals.map((journal) => ({ kind: "journal" as const, journal })),
      ...rowSources.map((row) => ({ kind: "database_row" as const, row })),
    ];
    const items: any[] = [];
    const report = (processed: number, message: string) =>
      options.onProgress?.({
        processed,
    ...
    for (let index = 0; index < sources.length; index += 1) {
      if (options.isCancelled?.()) {
        const error = new Error("Attachment index rebuild cancelled");
        error.name = "AttachmentIndexRebuildCancelled";
        throw error;
      }
      const source = sources[index];
      if (source.kind === "page") {
        const page = source.page;
        for (const attachment of await this.listAttachments(page.id).catch(
          () => [] as AttachmentInfo[],
        )) {
          items.push({
            sourceType: "page",
            ...
            createdAt: attachment.createdAt || page.updatedAt || "",
            relativePath: attachment.relativePath || "",
            title: page.title,
            icon: page.icon || "📄",
            updatedAt: page.updatedAt || "",
            scope: attachment.scope || "shared",
          });
        }
      } else if (source.kind === "journal") {
        const journal = source.journal;
        for (const attachment of await this.listJournalAttachments(
          journal.date,
        ).catch(() => [] as AttachmentInfo[])) {
          items.push({
            sourceType: "journal",
            sourceId: journal.date,
            pageId: `journal:${journal.date}`,
            journalDate: journal.date,
            attachmentId: attachment.id,
            fileName: attachment.fileName || "",
            mimeType: (attachment as any).mimeType || "",
            size: Number(attachment.size || 0),
            createdAt: attachment.createdAt || journal.updatedAt || "",
            relativePath: attachment.relativePath || "",
        ...
        const database = databaseDetails.get(row.databaseId);
        for (const attachment of await this.listDatabaseRowAttachments(
          row.databaseId,
          row.rowId,
          database?.scope === "private" ? "private" : "shared",
        ).catch(() => [] as AttachmentInfo[])) {
          items.push({
            sourceType: "database_row",
            sourceId: `${row.databaseId}:${row.rowId}`,
            pageId: `dbrow:${row.databaseId}:${row.rowId}`,
            ...
            mimeType: (attachment as any).mimeType || "",
            size: Number(attachment.size || 0),
            createdAt: attachment.createdAt || row.updatedAt || "",
            relativePath: attachment.relativePath || "",
            title: `${database?.title || "データベース"} / ${row.title || "無題の行"}`,
            icon: "🧾",
            updatedAt: row.updatedAt || "",
            scope: attachment.scope || "shared",
          });
      ...
      if (index % 12 === 0 || index === sources.length - 1)
        report(index + 1, `添付元を確認中 (${index + 1}/${sources.length})`);
    }
    // Preserve the previous catalog until every source has been scanned. A cancel
    // or transient SMB error must never leave the attachment screen empty.
    const tx = this.db.transaction((rows: any[]) => {
      this.db.prepare("DELETE FROM attachment_index").run();
      const insert = this.db.prepare(`INSERT OR REPLACE INTO attachment_index(
        id,page_id,attachment_id,file_name,mime_type,size,created_at,relative_path,page_title,page_icon,page_updated_at,
        source_type,source_id,database_id,row_id,journal_date,source_title,source_icon,source_updated_at,scope
      ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`);
      for (const item of rows) {
        const id =
          item.sourceType === "page"
            ? `${item.sourceId}:${item.attachmentId}`
            : `${item.sourceType}:${item.sourceId}:${item.attachmentId}`;
        insert.run(
          id,
          item.pageId,
          item.attachmentId,
    ...
    tx(items);
    report(sources.length, `添付Indexを更新しました (${items.length}件)`);
    return items.length;
  }

  getAttachmentIndexRebuildStatus(): Record<string, unknown> {
    return { ...this.attachmentIndexRebuildJob };
  }

  async startAttachmentIndexRebuild(): Promise<Record<string, unknown>> {
    if (this.attachmentIndexRebuildJob.state === "running")
      return this.getAttachmentIndexRebuildStatus();
    this.attachmentIndexRebuildJob = {
      state: "running",
      processed: 0,
      total: 0,
      indexed: 0,
    ...
    void this.rebuildAttachmentIndex({
      isCancelled: () => this.attachmentIndexRebuildJob.cancelRequested,
      onProgress: (progress) => {
        Object.assign(this.attachmentIndexRebuildJob, progress);
      },
    })
      .then((indexed) => {
        Object.assign(this.attachmentIndexRebuildJob, {
          state: "completed",
          indexed,
          finishedAt: new Date().toISOString(),
          message: `完了 (${indexed}件)`,
        });
      })
      .catch((error) => {
        const cancelled = error?.name === "AttachmentIndexRebuildCancelled";
        Object.assign(this.attachmentIndexRebuildJob, {
          state: cancelled ? "cancelled" : "failed",
          finishedAt: new Date().toISOString(),
          message: cancelled
            ? "中止しました。既存Indexは保持されています。"
  ...
  cancelAttachmentIndexRebuild(): Record<string, unknown> {
    if (this.attachmentIndexRebuildJob.state === "running")
      this.attachmentIndexRebuildJob.cancelRequested = true;
    return this.getAttachmentIndexRebuildStatus();
  }
  ...
  private async removeDatabaseChildReferencesAndRefreshIndex(
    pageId: string,
  ): Promise<void> {
    ...
    for (const changed of result.updatedRows || []) {
      try {
        ...
        const content = await this.getDatabaseRowContent(
          changed.databaseId,
          changed.rowId,
          {
        ...
        console.warn(
          "REFRESH_DB_ROW_LINK_INDEX_AFTER_CHILD_REMOVAL_FAILED",
          changed.databaseId,
          changed.rowId,
          error,
        );
  ...
  /** Removes every local derived record owned by pages in bounded set-based transactions. */
  private removePageDerivedIndexes(pageIds: ReadonlyArray<string>): void {
    const ids = Array.from(new Set(pageIds.map(String).filter(Boolean)));
    if (!ids.length) return;
    // SQLite has a host-parameter limit. Batching keeps a deep page-tree delete
    // to a handful of statements rather than six statements per page.
    const tx = this.db.transaction(() => {
      for (let offset = 0; offset < ids.length; offset += 800) {
        const chunk = ids.slice(offset, offset + 800);
        const marks = chunk.map(() => "?").join(",");
        this.db
          .prepare(`DELETE FROM page_fts WHERE id IN (${marks})`)
          .run(...chunk);
        this.db
          .prepare(`DELETE FROM page_search_index WHERE page_id IN (${marks})`)
          .run(...chunk);
        this.db
          .prepare(`DELETE FROM page_search_fts WHERE page_id IN (${marks})`)
          .run(...chunk);
        this.db
          .prepare(
            `DELETE FROM workspace_link_index WHERE source_page_id IN (${marks})`,
          )
          .run(...chunk);
        this.db
          .prepare(
            `DELETE FROM broken_link_index WHERE source_page_id IN (${marks})`,
          )
          .run(...chunk);
        this.db
          .prepare(
            `DELETE FROM attachment_index WHERE page_id IN (${marks}) OR (source_type = 'page' AND source_id IN (${marks}))`,
          )
          .run(...chunk, ...chunk);
        this.db
          .prepare(
            `DELETE FROM task_index WHERE source_type = 'page' AND source_id IN (${marks})`,
          )
          .run(...chunk);
      }
    });
  ...
  /** Finds active page sources that must be re-indexed when a link target disappears. */
  private inboundPageLinkSourceIds(
    targetPageIds: ReadonlyArray<string>,
  ): string[] {
    const targets = Array.from(
      new Set(targetPageIds.map(String).filter(Boolean)),
    );
    const result = new Set<string>();
    for (let offset = 0; offset < targets.length; offset += 800) {
      const chunk = targets.slice(offset, offset + 800);
      const placeholders = chunk.map(() => "?").join(",");
      const rows = this.db
        .prepare(
          `SELECT DISTINCT source_page_id AS id FROM workspace_link_index WHERE source_type = 'page' AND target_type = 'page' AND target_page_id IN (${placeholders}) AND source_page_id IS NOT NULL`,
        )
        .all(...chunk) as Array<{ id?: string }>;
      rows.forEach((row) => {
  ...
  /** Converts now-missing targets into broken-link records without a workspace scan. */
  private refreshInboundPageLinkIndexes(
    sourcePageIds: ReadonlyArray<string>,
    excludedPageIds: ReadonlySet<string>,
  ): void {
    for (const sourcePageId of sourcePageIds) {
      if (excludedPageIds.has(sourcePageId)) continue;
      const source = this.getPage(sourcePageId);
      if (!source || source.meta.trashed) continue;
      this.upsertPageDerivedIndexes(source);
    }
  }

  /** Removes database-row indexes and graph/task records when a trashed DB is permanently deleted. */
  private removeDatabaseDerivedIndexes(databaseId: string): void {
    const encodedPrefix = `${encodeURIComponent(databaseId)}/%`;
    const tx = this.db.transaction(() => {
      this.db
      ...
      this.db
        ...
        .run(databaseId);
      this.db
        .prepare("DELETE FROM database_index_meta WHERE database_id = ?")
        .run(databaseId);
      this.db
        .prepare("DELETE FROM database_index_state WHERE database_id = ?")
        .run(databaseId);
      this.db
        .prepare(
          "DELETE FROM workspace_link_index WHERE source_database_id = ? OR target_database_id = ?",
        )
        .run(databaseId, databaseId);
      this.db
        .prepare(
          "DELETE FROM task_index WHERE source_type = ? AND source_id LIKE ?",
        )
        .run("database-row", encodedPrefix);
    });
    tx();
  }
  ...
  async trashPage(
    ...
    const inboundSources = this.inboundPageLinkSourceIds(ids);
    ...
    for (const pageId of ids) {
      ...
      if (
        ...
        throw new Error(
          `Page is locked by ${childLock.userName} / ${childLock.lockedBy}`,
        );
      }
    }
    ...
    this.removePageDerivedIndexes(ids);
    this.refreshInboundPageLinkIndexes(inboundSources, new Set(ids));
    return {
      ...current.meta,
      affectedPageIds: ids,
      trashed: true,
  ...
  async restoreTrashedPage(
    ...
    for (const pageId of ids) {
      const page = this.getPage(pageId);
      if (!page) continue;
      const parent = page.meta.parentId
        ? this.getPage(page.meta.parentId)
        : null;
      const meta: PageMeta = {
        ...page.meta,
      ...
      // The page may have been trashed with attachments. Restore only this page's
      // SQLite attachment rows instead of rebuilding the workspace-wide catalog.
      await this.attachmentService
        .reindexPageAttachments(pageId)
        .catch((error) =>
          console.warn("RESTORE_PAGE_ATTACHMENT_REINDEX_FAILED", pageId, error),
        );
      if (pageId === id) restoredRoot = meta;
    }

    if (!restoredRoot) throw new Error("Page could not be restored");
    return { ...restoredRoot, affectedPageIds: ids };
  }
  ...
  async savePage(input: SavePageInput): Promise<PageBundle> {
    return this.pageSaveCoordinator.save(input);
  }

  private buildSavedPageBundle(
    current: PageBundle,
    input: SavePageInput,
  ): PageBundle {
    const meta: PageMeta = {
  ...
  private async handlePageSaveConflict(
    current: PageBundle,
    input: SavePageInput,
    sharedMeta: PageMeta,
  ): Promise<never> {
  ...
  private async persistSavedPageBundle(
    current: PageBundle,
    bundle: PageBundle,
  ): Promise<void> {
    const { meta } = bundle;
    ...
    this.upsertPageDerivedIndexes(bundle);
    await this.syncDatabaseChildPageTitleAfterRename(current, bundle);
  }

  private async syncDatabaseChildPageTitleAfterRename(
    current: PageBundle,
    bundle: PageBundle,
  ): Promise<void> {
    const { meta } = bundle;
    if (
      ...
      !this.isDatabaseRowParentId(meta.parentId)
    )
      return;
    ...
    try {
      ...
      const result =
        await this.databaseRowContentService().updateChildPageReferenceTitle(
          meta.id,
          meta.title,
  ...
  async search(query: string): Promise<PageWithLock[]> {
    return this.workspacePageSearchService.search(query);
  }

  /**
   * Returns a bounded workspace-wide map built exclusively from SQLite metadata
   * and the incremental link index. It deliberately favors well-connected and
   * recently changed pages so a large vault never produces an unreadable or
   * expensive all-node graph.
   */
  async getGlobalKnowledgeGraph(
    options: {
      maxNodes?: number;
      expansion?: "pages" | "database_rows" | "attachments" | "journals";
    } = {},
  ): Promise<KnowledgeGraphResult> {
    // 古いローカルDBやリンク索引の移行途中でも、全体マップ画面そのものは開けるようにする。
    // 索引ベースの構築に失敗した場合は、ページメタデータのみの安全な縮退表示へ切り替える。
    try {
      const maxNodes = Math.max(
        60,
        Math.min(500, Number(options.maxNodes || 320)),
      );
      const pageNodeId = (id: string) => `page:${id}`;
      const tagNodeId = (tag: string) => `tag:${tag}`;
      const rowNodeId = (databaseId: string, rowId: string) =>
        `database-row:${databaseId}:${rowId}`;
      const journalNodeId = (date: string) => `journal:${date}`;
      const attachmentNodeId = (id: string) => `attachment:${id}`;
      const expansion = options.expansion || "pages";
      // Keep room for an explicitly requested resource expansion instead of
      // allowing page/tag nodes to consume the entire node budget first.
      const pageBudget = Math.max(
        20,
        maxNodes - (expansion === "pages" ? 32 : 120),
      );
      ...
      const pageRows = this.db
        .prepare(
          `WITH degree_parts AS (
         SELECT source_page_id AS page_id, COUNT(*) AS degree
         FROM workspace_link_index
         WHERE source_type = 'page' AND source_page_id IS NOT NULL
         GROUP BY source_page_id
         UNION ALL
         SELECT target_page_id AS page_id, COUNT(*) AS degree
         FROM workspace_link_index
         WHERE target_type = 'page' AND target_page_id IS NOT NULL
         GROUP BY target_page_id
       ), degree_totals AS (
         SELECT page_id, SUM(degree) AS degree FROM degree_parts GROUP BY page_id
       )
       SELECT p.id,p.title,p.icon,p.updated_at AS updatedAt,p.parent_id AS parentId,p.properties_json AS propertiesJson,
              COALESCE(d.degree, 0) AS degree
       FROM pages p
       LEFT JOIN degree_totals d ON d.page_id = p.id
       WHERE p.trashed = 0
       ORDER BY CASE WHEN COALESCE(d.degree, 0) > 0 THEN 0 ELSE 1 END,
                COALESCE(d.degree, 0) DESC, p.updated_at DESC
       LIMIT ?`,
        )
        .all(pageBudget) as Array<{
        id: string;
        title: string;
        icon?: string | null;
        updatedAt?: string;
      ...
      const linkRows = this.db
        .prepare(
          `SELECT source_page_id AS sourcePageId,target_page_id AS targetPageId,link_kind AS linkKind,updated_at AS updatedAt
       FROM workspace_link_index
       WHERE source_type = 'page' AND target_type = 'page'
         AND source_page_id IS NOT NULL AND target_page_id IS NOT NULL
      ...
      for (const row of linkRows) {
        if (
          !row.sourcePageId ||
          !row.targetPageId ||
          !pageIds.has(row.sourcePageId) ||
          !pageIds.has(row.targetPageId)
        )
          continue;
        addEdge({
          id: `global-link:${row.sourcePageId}:${row.targetPageId}:${row.linkKind || "link"}`,
          source: pageNodeId(row.sourcePageId),
          target: pageNodeId(row.targetPageId),
          kind: "link",
          label: row.linkKind || "リンク",
        });
      }
      for (const page of pageRows) {
        if (!page.parentId || !pageIds.has(page.parentId)) continue;
        addEdge({
          id: `global-parent:${page.parentId}:${page.id}`,
          source: pageNodeId(page.parentId),
          target: pageNodeId(page.id),
          kind: "child",
          label: "子ページ",
        });
      }

      // Tags are only included when they connect multiple pages in the selected
      // subset. This avoids turning a generic one-off tag into visual noise.
      const tagMembers = new Map<string, string[]>();
      for (const page of pageRows) {
        try {
          const tags = (JSON.parse(page.propertiesJson || "{}")?.tags || [])
          ...
          /* invalid legacy properties do not block the graph */
        }
      }
      const rankedTags = Array.from(tagMembers.entries())
        .filter(([, members]) => members.length >= 2)
        .sort(
          (a, b) => b[1].length - a[1].length || a[0].localeCompare(b[0], "ja"),
        )
        .slice(0, Math.min(24, Math.max(8, Math.floor(maxNodes / 12))));
      for (const [tag, members] of rankedTags) {
        if (
          !addNode({
            id: tagNodeId(tag),
            type: "tag",
          ...
          break;
        for (const pageId of members.slice(0, 28)) {
          addEdge({
            id: `global-tag:${pageId}:${tag}`,
            source: pageNodeId(pageId),
      ...
      // Optional expansions are bounded and index-only. They are intentionally
      // opt-in so the default page map remains fast on large shared vaults.
      if (expansion === "database_rows") {
        const linkedRows = this.db
          .prepare(
            `SELECT DISTINCT target_database_id AS databaseId,target_row_id AS rowId,source_page_id AS sourcePageId
         FROM workspace_link_index
         WHERE source_type='page' AND target_type='database-row'
           AND source_page_id IN (${
             Array.from(pageIds)
               .map(() => "?")
               .join(",") || "''"
           })
           AND target_database_id IS NOT NULL AND target_row_id IS NOT NULL
         LIMIT 100`,
          )
          .all(...Array.from(pageIds)) as Array<{
          databaseId: string;
          rowId: string;
          sourcePageId: string;
        }>;
        const rowKeys = linkedRows.map(
          (row) => [row.databaseId, row.rowId] as const,
        );
        const rowMeta = new Map<string, any>();
        for (let offset = 0; offset < rowKeys.length; offset += 250) {
          const batch = rowKeys.slice(offset, offset + 250);
          if (!batch.length) continue;
          const where = batch
            .map(() => "(database_id=? AND row_id=?)")
            .join(" OR ");
          const rows = this.db
            .prepare(
              `SELECT database_id AS databaseId,row_id AS rowId,title_text AS title,updated_at AS updatedAt FROM database_row_index WHERE ${where}`,
            )
            .all(...batch.flat()) as any[];
          rows.forEach((row) =>
            rowMeta.set(`${row.databaseId}:${row.rowId}`, row),
          );
        }
        for (const link of linkedRows) {
          const row = rowMeta.get(`${link.databaseId}:${link.rowId}`);
          const id = rowNodeId(link.databaseId, link.rowId);
          if (
            addNode({
              id,
              type: "database-row",
              ...
              rowId: link.rowId,
              title: row?.title || "データベース行",
              icon: "🧾",
              updatedAt: row?.updatedAt || "",
            })
          ) {
            addEdge({
              id: `global-dbrow:${link.sourcePageId}:${link.databaseId}:${link.rowId}`,
              source: pageNodeId(link.sourcePageId),
              target: id,
              kind: "link",
              label: "DB行リンク",
            });
      ...
      if (expansion === "attachments") {
        const sourceIds = Array.from(pageIds);
        const attachmentRows = sourceIds.length
          ? (this.db
              .prepare(
                `SELECT id,source_type AS sourceType,source_id AS sourceId,attachment_id AS attachmentId,file_name AS fileName,source_title AS sourceTitle,created_at AS createdAt
         FROM attachment_index WHERE source_type='page' AND source_id IN (${sourceIds.map(() => "?").join(",")}) ORDER BY created_at DESC LIMIT 120`,
              )
              .all(...sourceIds) as any[])
          : [];
        for (const attachment of attachmentRows) {
          const id = attachmentNodeId(attachment.id);
          if (
            addNode({
              id,
              type: "attachment",
              attachmentId: attachment.attachmentId,
              title: attachment.fileName || "添付ファイル",
              icon: "📎",
              updatedAt: attachment.createdAt || "",
            })
          ) {
            addEdge({
              id: `global-attachment:${attachment.id}`,
              source: pageNodeId(attachment.sourceId),
              target: id,
              kind: "attachment",
              label: "添付",
            });
      ...
      if (expansion === "journals") {
        const journals = this.db
          .prepare(
            `SELECT date,title,icon,updated_at AS updatedAt FROM journal_summary_index ORDER BY updated_at DESC LIMIT 80`,
          )
          .all() as any[];
        for (const journal of journals)
          addNode({
            id: journalNodeId(journal.date),
            type: "journal",
            journalDate: journal.date,
            title: journal.title || journal.date,
            icon: journal.icon || "📓",
      ...
      return {
        ...
        truncated: truncated || pageRows.length >= pageBudget,
        generatedAt: new Date().toISOString(),
      };
    } catch (error) {
      console.warn("[knowledge-map] global index graph fallback", error);
      const maxNodes = Math.max(
        60,
        Math.min(500, Number(options.maxNodes || 320)),
      );
  ...
  /**
   * Returns a bounded, index-only local knowledge graph for a page.
   * This path intentionally reads SQLite metadata/link indexes and never scans
   * shared-folder page bodies, DB bodies, OCR content, or semantic embeddings.
   */
  async getLocalKnowledgeGraph(
    ...
    if (!center || center.meta.trashed) throw new Error("Page not found");

    const maxNodes = Math.max(
      20,
      Math.min(120, Number(options.maxNodes || 80)),
    );
    const nodes = new Map<string, KnowledgeGraphNode>();
    const edges = new Map<string, KnowledgeGraphEdge>();
    let truncated = false;
    ...
    addNode({
      id: pageNodeId(center.meta.id),
      type: "page",
      pageId: center.meta.id,
      title: center.meta.title || "無題のページ",
      icon: center.meta.icon || "📄",
      updatedAt: center.meta.updatedAt,
      isCenter: true,
    });

    const pageRows = this.db
      .prepare(
        `SELECT id,title,icon,parent_id as parentId,updated_at as updatedAt,properties_json as propertiesJson
         FROM pages WHERE trashed = 0`,
      )
      .all() as Array<{
      id: string;
      title: string;
    ...
    const addDatabaseRow = (
      ...
      const row = this.db
        .prepare(
          `SELECT r.title_text as rowTitle,r.updated_at as updatedAt,d.title as databaseTitle,d.icon as databaseIcon
           FROM database_row_index r
           LEFT JOIN database_summary_index d ON d.database_id = r.database_id
           WHERE r.database_id = ? AND r.row_id = ? LIMIT 1`,
        )
        .get(databaseId, rowId) as
        | {
            rowTitle?: string;
            updatedAt?: string;
        ...
        | undefined;
      return addNode({
        id: rowNodeId(databaseId, rowId),
        type: "database-row",
        databaseId,
        rowId,
        title: row?.rowTitle || fallbackTitle || "データベース行",
        icon: row?.databaseIcon || "🧾",
        updatedAt: row?.updatedAt || "",
      });
    };
    ...
    const linkRows = this.db
      .prepare(
        `SELECT source_type as sourceType,source_page_id as sourcePageId,source_database_id as sourceDatabaseId,
                source_row_id as sourceRowId,source_title as sourceTitle,target_page_id as targetPageId,
                target_type as targetType,target_database_id as targetDatabaseId,target_row_id as targetRowId,
                link_kind as linkKind,updated_at as updatedAt
         FROM workspace_link_index
         WHERE (source_type = 'page' AND source_page_id = ?)
            OR (target_type = 'page' AND target_page_id = ?)
         ORDER BY updated_at DESC LIMIT 90`,
      )
      .all(pageId, pageId) as Array<any>;

    // Resolve all DB-row endpoints in one query.  The previous per-edge query
    // caused an N+1 pattern whenever a page linked to many rows.
    const databaseRowKeys = Array.from(
      new Set(
        linkRows.flatMap((row: any) => {
          const keys: string[] = [];
          if (
            row.targetType === "database-row" &&
            row.targetDatabaseId &&
            row.targetRowId
          )
            keys.push(`${row.targetDatabaseId}\u0000${row.targetRowId}`);
          if (
            row.sourceType === "database-row" &&
            row.sourceDatabaseId &&
            row.sourceRowId
          )
            keys.push(`${row.sourceDatabaseId}\u0000${row.sourceRowId}`);
          return keys;
        }),
      ),
    );
    ...
    >();
    for (let offset = 0; offset < databaseRowKeys.length; offset += 400) {
      const batch = databaseRowKeys
        .slice(offset, offset + 400)
        .map((key) => key.split("\u0000"));
      if (!batch.length) continue;
      const predicates = batch
        .map(() => "(r.database_id = ? AND r.row_id = ?)")
        .join(" OR ");
      const values = batch.flat();
      const rows = this.db
        .prepare(
          `SELECT r.database_id as databaseId,r.row_id as rowId,r.title_text as rowTitle,r.updated_at as updatedAt,d.title as databaseTitle,d.icon as databaseIcon
         FROM database_row_index r LEFT JOIN database_summary_index d ON d.database_id = r.database_id
         WHERE ${predicates}`,
        )
        .all(...values) as Array<any>;
      for (const row of rows)
        databaseRows.set(`${row.databaseId}\u0000${row.rowId}`, row);
    }
    const addResolvedDatabaseRow = (
      databaseId: string,
      rowId: string,
      fallbackTitle = "",
    ) => {
      const row = databaseRows.get(`${databaseId}\u0000${rowId}`);
      return addNode({
        id: rowNodeId(databaseId, rowId),
        type: "database-row",
        databaseId,
    ...
    for (const row of linkRows) {
      const isOutgoing =
        row.sourceType === "page" && row.sourcePageId === pageId;
      let otherId = "";
      if (isOutgoing) {
        if (
          row.targetType === "database-row" &&
          ...
          otherId = rowNodeId(row.targetDatabaseId, row.targetRowId);
          if (!addResolvedDatabaseRow(row.targetDatabaseId, row.targetRowId))
            continue;
        } else if (row.targetPageId && addPage(row.targetPageId)) {
          otherId = pageNodeId(row.targetPageId);
        } else continue;
      } else {
        if (
          row.sourceType === "database-row" &&
          ...
          otherId = rowNodeId(row.sourceDatabaseId, row.sourceRowId);
          if (
            !addResolvedDatabaseRow(
              row.sourceDatabaseId,
              row.sourceRowId,
              row.sourceTitle,
            )
            ...
            continue;
        } else if (row.sourcePageId && addPage(row.sourcePageId)) {
          otherId = pageNodeId(row.sourcePageId);
        } else continue;
      }
      const centerId = pageNodeId(pageId);
      addEdge({
        id: `link:${row.sourceType || "page"}:${row.sourcePageId || row.sourceRowId || ""}:${row.targetType || "page"}:${row.targetPageId || row.targetRowId || ""}:${row.linkKind || "link"}`,
        source: isOutgoing ? centerId : otherId,
        target: isOutgoing ? otherId : centerId,
        kind: isOutgoing ? "link" : "backlink",
        label: row.linkKind || "リンク",
      });
    }

    const centerPageRow = pages.get(pageId);
    if (
      centerPageRow?.parentId &&
      pages.has(centerPageRow.parentId) &&
      addPage(centerPageRow.parentId)
    ) {
      addEdge({
        id: `parent:${centerPageRow.parentId}:${pageId}`,
        source: pageNodeId(centerPageRow.parentId),
        target: pageNodeId(pageId),
        kind: "parent",
        label: "親ページ",
      });
    }
    for (const child of pageRows
      .filter((page) => page.parentId === pageId)
      .slice(0, 24)) {
      if (!addPage(child.id)) break;
      addEdge({
        id: `child:${pageId}:${child.id}`,
        source: pageNodeId(pageId),
        target: pageNodeId(child.id),
        kind: "child",
        label: "子ページ",
      });
    }
    ...
    for (const node of nodes.values()) {
      if (node.type !== "page" || !node.pageId) continue;
      const raw = pages.get(node.pageId)?.propertiesJson || "{}";
      try {
        const tags = (JSON.parse(raw)?.tags || [])
          .map((tag: unknown) => String(tag || "").trim())
          .filter(Boolean)
          .slice(0, 8);
        pageTags.set(node.id, tags);
      } catch {
        pageTags.set(node.id, []);
      }
    }
    const rankedTags = Array.from(pageTags.entries())
      .flatMap(([nodeId, tags]) => tags.map((tag) => ({ nodeId, tag })))
      .reduce(
        (map, item) => map.set(item.tag, (map.get(item.tag) || 0) + 1),
        new Map<string, number>(),
      );
    const visibleTags = Array.from(rankedTags.entries())
    ...
    for (const tag of visibleTags) {
      if (
        ...
        continue;
      for (const [nodeId, tags] of pageTags.entries()) {
  ...
  async listBacklinks(pageId: string): Promise<BacklinkInfo[]> {
    return this.workspaceLinkQueryService.listBacklinks(pageId);
  }
  ...
  /**
   * Stores all attachment sources in one derived SQLite index. Legacy page_* columns
   * remain populated so existing attachment views continue to work unchanged.
   */
  private upsertCommonAttachmentIndex(input: {
    sourceType: "page" | "journal" | "database_row";
    sourceId: string;
    ...
    const pageId =
      input.sourceType === "page"
        ? input.sourceId
        : input.sourceType === "journal"
          ? `journal:${input.journalDate || input.sourceId}`
          : `dbrow:${input.databaseId || ""}:${input.rowId || ""}`;
    // Preserve the legacy page key so page uploads replace, rather than duplicate,
    // the row written by AttachmentService. Other sources use distinct namespaces.
    const id =
      input.sourceType === "page"
        ? `${input.sourceId}:${attachment.id}`
        : `${input.sourceType}:${input.sourceId}:${attachment.id}`;
    this.db
      .prepare(
      ...
      .run(
        ...
        attachment.createdAt || input.updatedAt || "",
  ...
  async addAttachment(
    ...
    const page = this.getPage(pageId);
    if (page)
      this.upsertCommonAttachmentIndex({
        sourceType: "page",
        sourceId: pageId,
        attachment,
        title: page.meta.title,
        icon: page.meta.icon ?? undefined,
  ...
  async addAttachmentFromBase64(
    ...
    const attachment = await this.attachmentService.addAttachmentFromBase64(
      pageId,
      fileName,
      base64,
    );
  ...
  async restoreBackupCenterItem(
    ...
    if (safeId.startsWith("deleted_journal_")) {
      ...
      const fallbackDate = safeId.replace(/^deleted_journal_/, "").slice(0, 10);
  ...
  async listAllAttachments(): Promise<any[]> {
    const rows = this.db
      ...
      SELECT page_id as pageId, attachment_id as id, file_name as fileName, mime_type as mimeType, size, created_at as createdAt,
             ...
             source_type as sourceType, source_id as sourceId, database_id as databaseId, row_id as rowId, journal_date as journalDate,
             source_title as sourceTitle, source_icon as sourceIcon, source_updated_at as sourceUpdatedAt, scope
  ...
  async listBrokenLinks(): Promise<any[]> {
    return this.workspaceLinkQueryService.listBrokenLinks();
  }

  async closeAnalysisNotebook(): Promise<void> {
    await this.analysisNotebookService.close();
  }

  async getAnalysisStatus() {
    return this.analysisNotebookService.status();
  }

  async syncAnalysisData() {
    return this.analysisNotebookService.sync();
  }
  ...
  async queryAnalysis(
    ...
    return this.analysisNotebookService.query(sql, parameters, namedResults);
  }
  async pivotAnalysis(transform: any, namedSource: any) {
    return this.analysisNotebookService.pivot(transform, namedSource);
  }
  getAnalysisResultPage(resultId: string, page = 0, pageSize = 500) {
    return this.analysisNotebookService.getResultPage(resultId, page, pageSize);
  }
  getAnalysisResultAll(resultId: string) {
    return this.analysisNotebookService.getResultAll(resultId);
  }
  releaseAnalysisResult(resultId: string) {
    return this.analysisNotebookService.releaseResult(resultId);
  }
  getAnalysisResultCacheStatus() {
    return this.analysisNotebookService.getResultCacheStatus();
  }

  streamAnalysisResultCsv(
    resultId: string,
    write: (chunk: string) => Promise<void>,
  ) {
    return this.analysisNotebookService.streamResultCsv(resultId, write);
  }
  ...
  async generateAnalysisAiDraft(input: any): Promise<any> {
    ...
    if (!instruction)
      return {
        ok: false,
        generated: false,
        message: "作りたい分析を文章で入力してください。",
      };
    if (instruction.length > 1_500)
      return {
        ok: false,
        generated: false,
        message: "指示は1,500文字以内にしてください。",
      };
    ...
    const schema = dictionary.datasets
      .map((dataset: any) => {
    ...
    try {
      const generated = await this.runLlamaGeneration(
        prompt,
        {
          ...
          maxTokens: Math.max(
            256,
            Math.min(900, Number(settings.maxTokens || 512)),
          ),
          contextSize: Math.max(
            2048,
            Math.min(6144, Number(settings.contextSize || 4096)),
          ),
          temperature: Math.max(
            0,
            Math.min(0.25, Number(settings.temperature ?? 0.1)),
          ),
        } as any,
        check,
      );
      ...
      if (!match)
        throw new Error(
          "AIが分析案のJSONを返しませんでした。もう一度お試しください。",
        );
      ...
      const chartType = allowedChartTypes.has(String(parsed?.chart?.type || ""))
        ? String(parsed.chart.type)
        : "table";
      const chart = {
        type: chartType,
        x: String(parsed?.chart?.x || ""),
        y: String(parsed?.chart?.y || ""),
      };
      const validation = await this.analysisNotebookService.validateAiDraft(
        candidateSql,
        chart,
      );
        ...
        draft: {
          ...
          warnings: [
            ...(Array.isArray(parsed.warnings)
              ? parsed.warnings
                  .map((value: any) => String(value).slice(0, 300))
                  .slice(0, 6)
              : []),
            ...validation.warnings,
          ].slice(0, 8),
      ...
      return {
        ...
        generated: false,
        message: String(
          error?.message || "AIによる分析コードの作成に失敗しました。",
        ),
      };
    }
  }

  getAnalysisDataDictionary() {
    return this.analysisNotebookService.getDataDictionary();
  }

  getAnalysisWorkspaceSettings() {
    return this.analysisNotebookService.getWorkspaceSettings();
  }

  saveAnalysisWorkspaceSettings(input: any) {
    return this.analysisNotebookService.saveWorkspaceSettings(input);
  }

  listAnalysisNotebooks() {
    return this.analysisNotebookService.listNotebooks();
  }

  getAnalysisNotebook(id: string) {
    return this.analysisNotebookService.getNotebook(id);
  }

  saveAnalysisNotebook(input: any) {
    return this.analysisNotebookService.saveNotebook(input);
  }

  deleteAnalysisNotebook(id: string) {
    return this.analysisNotebookService.deleteNotebook(id);
  }

  listAnalysisDashboardPins() {
    return this.analysisNotebookService.listDashboardPins();
  }
  saveAnalysisDashboardPin(input: any) {
    return this.analysisNotebookService.saveDashboardPin(input);
  }
  deleteAnalysisDashboardPin(id: string) {
    return this.analysisNotebookService.deleteDashboardPin(id);
  }
  ...
  async getWorkspaceDashboard(): Promise<any> {
    ...
    const recentDatabases = databases
      ...
      .map((database: any) => ({
        ...
        propertyCount: Array.isArray(database.properties)
  ...
  async createDatabaseRows(
    ...
    return this.createDatabaseRowsCore(id, input);
  }
  ...
  async deleteDatabaseRows(
    ...
    return this.deleteDatabaseRowsCore(id, input);
  }
  ...
  async listDatabaseRowAttachments(
    ...
    return this.databaseRowContentService().listRowAttachments(
      databaseId,
      rowId,
  ...
  async addDatabaseRowAttachmentFromBase64(
    databaseId: string,
    rowId: string,
    fileName: string,
    base64: string,
    ...
    const attachment =
      await this.databaseRowContentService().addRowAttachmentFromBase64(
        databaseId,
        rowId,
        fileName,
        base64,
    ...
    const database = await this.getDatabase(databaseId).catch(() => null);
    const row = this.db
      .prepare(
        "SELECT title_text as title, updated_at as updatedAt FROM database_row_index WHERE database_id = ? AND row_id = ?",
      )
      .get(databaseId, rowId) as any;
    this.upsertCommonAttachmentIndex({
      sourceType: "database_row",
      sourceId: `${databaseId}:${rowId}`,
      attachment,
      title: `${database?.title || "データベース"} / ${row?.title || "無題の行"}`,
      icon: "🧾",
      updatedAt: row?.updatedAt || database?.updatedAt || "",
      databaseId,
      rowId,
    });
    return attachment;
  ...
  async getDatabaseRowAttachmentFile(
    databaseId: string,
    rowId: string,
    attachmentId: string,
    scope: WorkspaceScope = "shared",
  ): Promise<{ info: AttachmentInfo; filePath: string }> {
    return this.databaseRowContentService().getRowAttachmentFilePath(
      databaseId,
      rowId,
      attachmentId,
      scope === "private" ? "private" : "shared",
  ...
  > {
    ...
    for (const item of indexRows) {
      ...
      const row = database?.rows.find(
        (candidate) => candidate.id === item.rowId,
      );
  ...
  async listDatabaseRowLinks(
    ...
    for (const item of outboundIndexRows) {
      ...
      if (item.targetType === "database-row") {
        const targetDb = item.targetDatabaseId
          ...
          : null;
        const targetRow = targetDb?.rows.find(
          (candidate) => candidate.id === item.targetRowId,
        );
        if (
          !targetDb ||
          !targetRow ||
        ...
        outboundLinks.push({
          ...
          to: {
            type: "database-row",
            databaseId: item.targetDatabaseId,
            rowId: item.targetRowId,
          },
    ...
    for (const item of backlinkIndexRows) {
      if (item.sourceType === "database-row") {
        ...
        const sourceRow = sourceDb?.rows.find(
          (candidate) => candidate.id === item.sourceRowId,
        );
        ...
        backlinks.push({
          ...
          sourceTitle:
            item.sourceTitle ||
            `${sourceDb.title} / ${databaseRowTitle(sourceDb, sourceRow)}`,
          sourceIcon: item.sourceIcon || "🧾",
    ...
    const resourceKey = (resource: ResourceRef): string => {
      ...
      if (resource.type === "database")
        return `database:${resource.databaseId}`;
      return `database-row:${resource.databaseId}:${resource.rowId}`;
    };
    const uniqueOutbound = Array.from(
      new Map(
        outboundLinks.map((link) => [resourceKey(link.to), link] as const),
      ).values(),
    );
    const uniqueBacklinks = Array.from(
      new Map(
        backlinks.map((link) => [resourceKey(link.from), link] as const),
      ).values(),
    );
    return {
      childPages: Array.from(
        new Map(childPages.map((page) => [page.id, page])).values(),
      ),
      outboundLinks: uniqueOutbound,
      backlinks: uniqueBacklinks.sort((a, b) =>
  ...
  async saveDatabaseCore(input: WorkspaceDatabase): Promise<WorkspaceDatabase> {
    ...
    const changedRowIds = database.rows
      .filter((row) => {
        ...
        return (
          !before ||
          String(before.updatedAt || "") !== String(row.updatedAt || "")
        );
      })
      .map((row) => row.id);
    ...
    const schemaChanged = current
      ? this.databaseIndexSchemaHash(current) !==
        this.databaseIndexSchemaHash(database)
      : true;
    await this.saveDatabaseFile(database, {
      changedRowIds,
      deletedRowIds,
  ...
  private nextDatabaseUniqueId(
    database: WorkspaceDatabase,
    property: DatabaseProperty,
  ): string {
    const prefix =
      String(property.uniqueIdPrefix || property.name || "ID")
        .trim()
        .slice(0, 24) || "ID";
    const digits = Math.max(
      1,
      Math.min(10, Number(property.uniqueIdDigits || 4)),
    );
  ...
  private backfillDatabaseUniqueIds(
    database: WorkspaceDatabase,
    property: DatabaseProperty,
  ): void {
    const prefix =
    ...
    for (const row of database.rows || []) {
      if (!String(row.cells?.[property.id] || "").trim()) {
        row.cells[property.id] =
          `${prefix}-${String(next++).padStart(digits, "0")}`;
      }
    }
  }
  ...
  async patchDatabaseRowsCore(
    ...
    if (
      ...
      if (isTrueConflict) {
        const error: any = new Error(
          `Database conflict detected. current=${database.updatedAt}, base=${baseUpdatedAt}`,
        );
    ...
    for (const patch of input.patches || []) {
      if (!patch?.rowId || !patch.cells || typeof patch.cells !== "object")
        continue;
      patchesByRowId.set(String(patch.rowId), {
        ...(patchesByRowId.get(String(patch.rowId)) || {}),
        ...patch.cells,
      });
    }
    if (!patchesByRowId.size)
      return {
        databaseId: id,
        rows: [],
        updatedAt: database.updatedAt,
    ...
    database.rows = database.rows.map((row) => {
      ...
      for (const [propertyId, value] of Object.entries(patch)) {
        ...
        if (
          ...
          continue;
        if (JSON.stringify(cells[propertyId]) === JSON.stringify(value))
          continue;
    ...
    await this.saveDatabaseFile(database, {
      changedRowIds: changedRows.map((row) => row.id),
    });
    return {
      databaseId: id,
      rows: changedRows,
  ...
  private assertDatabaseRevision(database: WorkspaceDatabase, baseUpdatedAt?: string): void {
    if (
      baseUpdatedAt &&
      database.updatedAt &&
      database.updatedAt !== baseUpdatedAt
  ...
  private buildDatabaseRowCells(
    ...
    for (const prop of database.properties) {
      ...
      if (prop.type === "unique_id") {
        cells[prop.id] = this.nextDatabaseUniqueId(database, prop);
        continue;
      }
      const provided = inputCells[prop.id];
      if (provided !== undefined) {
        cells[prop.id] = provided as any;
        continue;
      }
      cells[prop.id] =
        prop.type === "status"
  ...
  async createDatabaseRowsCore(
    ...
    const database = await this.getDatabase(id);
    if (!database) throw new Error("Database not found");
    this.assertDatabaseRevision(database, input.baseUpdatedAt);
    const now = new Date().toISOString();
    const requests: Array<{ sourceRowId?: string; cells?: Record<string, any> }> =
      input.rows?.length ? input.rows : [{}];
    const originalRows = [...(database.rows || [])];
    const sourceById = new Map(originalRows.map((row) => [row.id, row]));
    const createdRows: DatabaseRow[] = [];
    for (const request of requests.slice(0, 500)) {
      const source = request.sourceRowId ? sourceById.get(request.sourceRowId) : null;
      const baseCells = source ? { ...source.cells } : {};
      const cells = this.buildDatabaseRowCells(database, {
        ...baseCells,
        ...(request.cells || {}),
      });
      ...
      // Temporarily append so nextDatabaseUniqueId() sees rows created earlier
      // in the same request and produces stable sequential IDs.
      database.rows.push(row);
      createdRows.push(row);
    }
    database.rows = [...createdRows, ...originalRows];
    database.updatedAt = now;
    database.updatedBy = this.userLabel();
    await this.saveDatabaseFile(database, {
      changedRowIds: createdRows.map((row) => row.id),
    });
    return {
      databaseId: id,
      rows: createdRows,
  ...
  async deleteDatabaseRowsCore(
    ...
    const now = new Date().toISOString();
    const trashedRows = (database.rows || [])
    ...
    database.rows = database.rows.filter((row) => !rowIds.has(row.id));
    database.trash = {
      ...database.trash,
      rows: [...(database.trash?.rows ?? []), ...trashedRows],
    } as any;
    database.updatedAt = now;
    database.updatedBy = this.userLabel();
    await this.saveDatabaseFile(database, {
      deletedRowIds: trashedRows.map((row) => row.id),
    });
    return {
      databaseId: id,
      deletedRowIds: trashedRows.map((row) => row.id),
  ...
  async deleteTrashedDatabasePermanentlyCore(
    ...
    const deletedRowIds = Array.from(
      new Set(
        (database.rows || [])
    ...
    // Files were archived and removed successfully. Now clear only this DB's
    // local row/search/graph/task material; semantic removal is queued by the renderer.
    this.removeDatabaseDerivedIndexes(database.id);
    return { ok: true, id, deletedRowIds };
  }
  ...
  private async saveDatabaseFile(
    ...
    await this.atomicWriteJson(primary.file, { ...database, scope }, {
      // High-frequency row mutations already update the row index precisely.
      // Skipping .bak here removes one full-file copy per cell/row edit on SMB shares.
      backup: !indexMutation?.changedRowIds && !indexMutation?.deletedRowIds,
    });
  ...
  private upsertDatabaseRowIndexForKnownChanges(
    ...
    if (
      ...
      (database.rows.length > 0 &&
        (indexedCount === 0 || hashCount !== indexedCount))
    ) {
    ...
    const changed = new Set(
      (mutation.changedRowIds || []).map(String).filter(Boolean),
    );
    const deleted = new Set(
      (mutation.deletedRowIds || []).map(String).filter(Boolean),
    );
    const tx = this.db.transaction(() => {
      for (const rowId of deleted)
        this.deleteDatabaseRowIndexEntry(database.id, rowId);
    ...
    try {
      ...
      console.warn(
        "TARGETED_DATABASE_ROW_INDEX_UPDATE_FAILED",
        database.id,
        error,
      );
      this.rebuildDatabaseRowIndex(database);
  ...
  private queryDatabaseRowsWithSqliteView(
    ...
    const where = whereParts.length
      ? ` AND (${whereParts.join(filterJoin)})`
      : "";
  ...
  async aggregateDatabaseRowsCore(
    ...
    const view =
      database.views?.find(
        (item) => item.id === (input.viewId ?? database.activeViewId),
      ) ?? database.views?.[0];
    ...
    if (searchWords.length) {
      rows = rows.filter((row) => {
        const text = database.properties
          ...
          .map((property) =>
            dbCellPlainText(row.cells[property.id]).toLowerCase(),
          )
          .join(" ");
    ...
    for (const [propertyId, mode] of requested) {
      ...
      if (mode === "count") {
        values[propertyId] = `${rawValues.length.toLocaleString("ja-JP")}件`;
        continue;
      }
      if (mode === "filled") {
        values[propertyId] = `${filled.length.toLocaleString("ja-JP")}件`;
        continue;
      }
      if (mode === "empty") {
        values[propertyId] =
          `${Math.max(0, rawValues.length - filled.length).toLocaleString("ja-JP")}件`;
        continue;
      }
      if (mode === "unique") {
        values[propertyId] =
          `${new Set(filled.map((value) => dbCellPlainText(value))).size.toLocaleString("ja-JP")}件`;
        continue;
      }
      if (mode === "checked") {
        values[propertyId] =
          `${rawValues.filter(isChecked).length.toLocaleString("ja-JP")}件`;
        continue;
      }
      if (mode === "unchecked") {
        values[propertyId] =
          `${rawValues.filter((value) => !isChecked(value)).length.toLocaleString("ja-JP")}件`;
        continue;
      }
      if (mode === "percent_checked") {
        values[propertyId] = rawValues.length
          ? `${Math.round((rawValues.filter(isChecked).length / rawValues.length) * 100)}%`
          : "0%";
        continue;
      }
      const numbers = filled
        .map((value) => dbCellNumber(value))
      ...
      if (mode === "sum")
        ...
        values[propertyId] = format(sorted[sorted.length - 1]);
      else if (mode === "range")
        values[propertyId] = format(sorted[sorted.length - 1] - sorted[0]);
    }
  ...
  private normalizeDatabase(
    ...
    const properties = Array.isArray(input.properties)
      ? input.properties.map((p: any) => ({
          ...
          isSubItemRelation:
            p.isSubItemRelation === true &&
            p.type === "relation" &&
            (p.relationTargetType ?? "database") === "database" &&
            (p.relationDatabaseId || input.id || fallbackId) ===
              (input.id || fallbackId),
          isDependencyRelation:
            p.isDependencyRelation === true &&
            p.type === "relation" &&
            (p.relationTargetType ?? "database") === "database" &&
            (p.relationDatabaseId || input.id || fallbackId) ===
              (input.id || fallbackId),
          ...
          options: Array.isArray(p.options)
            ...
            : p.type === "status"
              ...
              : undefined,
          uniqueIdPrefix:
            typeof p.uniqueIdPrefix === "string"
              ? p.uniqueIdPrefix.slice(0, 24)
              : undefined,
          uniqueIdDigits: Number.isFinite(Number(p.uniqueIdDigits))
            ? Math.max(1, Math.min(10, Number(p.uniqueIdDigits)))
            : undefined,
          buttonAction:
            p.buttonAction === "set_today"
              ? "set_today"
              : p.buttonAction === "mark_status_done"
                ? "mark_status_done"
                : undefined,
          buttonTargetPropertyId:
            typeof p.buttonTargetPropertyId === "string"
              ? p.buttonTargetPropertyId
              : undefined,
        }))
      : [];
    ...
    const subItemProperty = properties.find(
      (item) => item.type === "relation" && item.isSubItemRelation,
    );
    if (subItemProperty) {
      ...
      for (const row of rows) {
        ...
        const raw: string[] = Array.isArray(rawParentValue)
          ...
          : [];
        const parentId = raw.find(
          (id: string) => id && id !== row.id && rowById.has(id),
        );
      ...
      for (const row of rows) {
        ...
        while (cursor) {
          ...
          cursor = String(
            (nextRow?.cells?.[subItemProperty.id] || [])[0] || "",
          );
        }
  ...
  private parseDatabaseRowTaskSourceId(
    sourceId: string,
  ...
  private async updateWorkspaceSummaryCache(): Promise<void> {
    try {
      ...
      const recentAttachments = this.db
        .prepare(
          `SELECT page_id as pageId, attachment_id as id, file_name as fileName, mime_type as mimeType, size, created_at as createdAt, relative_path as relativePath, page_title as pageTitle, page_icon as pageIcon, page_updated_at as pageUpdatedAt, source_type as sourceType, source_id as sourceId, database_id as databaseId, row_id as rowId, journal_date as journalDate, source_title as sourceTitle, source_icon as sourceIcon, source_updated_at as sourceUpdatedAt, scope FROM attachment_index ORDER BY created_at DESC LIMIT 6`,
        )
        .all() as any[];
  ...
  async updateTask(
    ...
    const sourceType = parts[0] as
      "page" | "journal" | "inbox" | "database-row";
    ...
    if (sourceType === "page") {
      ...
      const scope = database.scope === "private" ? "private" : "shared";
      const current = await this.getDatabaseRowContent(
        parsed.databaseId,
        parsed.rowId,
        {
  ...
  async sendAttachmentToOcrCenter(input: {
    ...
    if (input.sourceType === "page") {
      ...
      filePath = await this.getAttachmentFilePath(
        input.pageId,
        input.attachmentId,
      );
      sourceLabel =
        input.sourceTitle?.trim() ||
        this.getPage(input.pageId)?.meta.title ||
        "ページ添付";
    } else if (input.sourceType === "journal") {
      if (!input.date) throw new Error("Journal日付がありません");
      const source = await this.getJournalAttachmentFile(
        input.date,
        input.attachmentId,
      );
      ...
      if (!input.databaseId || !input.rowId)
        throw new Error("データベース行情報がありません");
      ...
      sourceLabel =
        input.sourceTitle?.trim() ||
        (db && row
          ? `${db.title} › ${databaseRowTitle(db, row)}`
          : "データベース行添付");
    }
    ...
    const existing = (await this.inboxService.list()).find((candidate) => {
      ...
      return (
        source?.sourceType === ocrSource.sourceType &&
  ...
  async searchJournals(query: string, limit = 30): Promise<JournalSummary[]> {
    ...
    const hasText = this.db
      .prepare(
        "SELECT COUNT(*) as count FROM journal_summary_index WHERE full_text <> ''",
      )
      .get() as { count?: number };
  ...
  async addJournalAttachment(
    ...
    await this.initVault();
    const attachment = await this.journalService.addAttachmentFromSource(
      date,
      sourcePath,
    );
    const journal = await this.journalService.get(date);
    this.upsertCommonAttachmentIndex({
      sourceType: "journal",
      sourceId: date,
      attachment,
  ...
  async addJournalAttachmentFromBase64(
    date: string,
    fileName: string,
    base64: string,
  ): Promise<AttachmentInfo> {
    await this.initVault();
    const attachment = await this.journalService.addAttachmentFromBase64(
      date,
      fileName,
      base64,
    );
  ...
  async getJournalAttachmentFile(
    date: string,
    attachmentId: string,
  ): Promise<{ attachment: AttachmentInfo; filePath: string }> {
    await this.initVault();
    const attachment = await this.journalService.getAttachmentInfo(
      date,
      attachmentId,
    );
    const filePath = await this.journalService.getAttachmentFilePath(
      date,
      attachmentId,
    );
  ...
  private semanticDatabaseRollupValue(
    ...
    if (fn === "percent_checked")
      return values.length
        ...
        : "0%";
    if (fn === "count_status_done")
      return String(values.filter(isDoneStatus).length);
    if (fn === "count_status_open")
      return String(values.filter((value) => !isDoneStatus(value)).length);
  ...
  private workspaceAiDbQuestionPlan(question: string): {
    ...
    let dateMode:
      "today" | "this_week" | "this_month" | "overdue" | "upcoming" | undefined;
  ...
  /**
   * Fast path for edits: build semantic chunks for only the changed source(s).
   * It deliberately avoids opening every page and every DB-row body file.
   * Full rebuilds still use collectWorkspaceSemanticChunks() to detect global
   * additions/deletions and to refresh all cross-source metadata.
   */
  private async collectWorkspaceSemanticChunksForTargets(
    targets: Array<{
    ...
    const uniqueTargets = normalizeIncrementalSemanticTargets(
      targets as IncrementalSemanticTarget[],
    );
    const chunks: SemanticChunk[] = [];

    for (const target of uniqueTargets.filter((item) => item.type === "page")) {
      let bundle: PageBundle | null = null;
      try {
        bundle = this.getPage(target.sourceId);
      } catch (error) {
        console.warn(
          "[semantic-index] skipped unreadable target page",
          target.sourceId,
          error,
        );
        continue;
      }
      if (!bundle || (bundle.meta as any).trashed) continue;
    ...
    const journalTargets = uniqueTargets.filter(
      (item) => item.type === "journal",
    );
    for (const target of journalTargets) {
      const journal = await this.getJournal(target.sourceId).catch(() => null);
      // Missing Journal intentionally returns no chunks: buildIndex removes stale
      // chunks for the targeted source key without touching other sources.
      if (!journal) continue;
    ...
    const databaseTargets = uniqueTargets.filter(
      (item) => item.type === "database_row" && item.databaseId,
    );
    if (databaseTargets.length) {
      // Relation/Rollup labels need metadata from related resources, but this
      // does not open any unrelated DB-row body files.
      const databases = await this.listDatabases().catch(
        () => [] as WorkspaceDatabase[],
      );
      const pages = await this.listPages().catch(() => [] as PageWithLock[]);
      ...
      for (const target of databaseTargets) {
        const database =
          databases.find((item) => item.id === target.databaseId) ||
          (await this.getDatabase(String(target.databaseId)).catch(() => null));
        if (!database || (database as any).trashed) continue;
        const row = (database.rows || []).find(
          (item) => item.id === target.sourceId,
        );
        // Missing rows intentionally return no chunks: buildIndex removes stale
        // chunks for the targeted source key without touching other sources.
        if (!row) continue;
        const rowTitle = databaseRowTitle(database, row);
  ...
  async rebuildWorkspaceSemanticIndex(
    ...
    const targets = normalizeIncrementalSemanticTargets(
      options.targets as IncrementalSemanticTarget[],
      100,
    );
    const partialSourceUpdate = !isFull && targets.length > 0;
    const chunks = partialSourceUpdate
      ? await this.collectWorkspaceSemanticChunksForTargets(targets)
      : await this.collectWorkspaceSemanticChunks();
    ...
    return service.buildIndex(chunks, previous, {
      ...
      preferredChunkIds,
      replaceSourceKeys: partialSourceUpdate
        ? targets.map(
            (target) =>
              `${target.type}:${target.databaseId || ""}:${target.sourceId}`,
          )
        : undefined,
  ...
  async reindexWorkspaceSemanticSource(
    ...
    const requestedDatabaseId = String(databaseId || "").trim();

    // A targeted retry must stay targeted too. The previous implementation
    // collected every page, DB row and journal before filtering one source.
    // Resolve the DB id from the current index so a database-row retry can
    // rebuild only that row, including the stale-source deletion case.
    const matchingItems = (previous?.items || []).filter(
      (item) =>
        String(item.sourceId) === safeSourceId &&
        (!safeType || String(item.type) === safeType),
    );
    const first = matchingItems[0];
    const target =
      (safeType === "database_row" || first?.type === "database_row") &&
      (requestedDatabaseId || first?.databaseId)
        ? {
            type: "database_row" as const,
            sourceId: safeSourceId,
            databaseId: requestedDatabaseId || String(first?.databaseId),
          }
        : safeType === "page" || first?.type === "page"
          ? { type: "page" as const, sourceId: safeSourceId }
          : null;
    if (!target) {
      throw new Error(
        "対象のSemantic Indexデータが見つかりません。DB行を再Indexする場合はdatabaseIdが必要です。",
      );
    }
    const chunks = await this.collectWorkspaceSemanticChunksForTargets([
      target,
    ]);
    return service.buildIndex(chunks, previous, {
      mode: "diff",
      ...
      preferredChunkIds: chunks.map((chunk) => chunk.id),
    });
  }
  ...
  async getWorkspaceSemanticIndexRevision(): Promise<{
    ...
    const service = await this.semanticIndexService();
    // This endpoint is called before every passive related-panel lookup. Do not
    // make a first page open await hydration of a potentially large local cache.
    const index = service.getLoadedIndex();
    if (!index) {
      const missing = service.hasConfirmedMissingIndex();
      if (!missing) service.warmIndexInBackground();
      return {
        ok: false,
        revision: null,
        indexedCount: 0,
  ...
  async getWorkspaceSemanticIndexInfo(): Promise<any> {
    ...
    return {
      ...
      embeddingProfile: index?.embeddingProfile || "legacy-title-weighted",
  ...
  async generateEditorAiEdit(input: any): Promise<any> {
    return this.editorAiEditService.generate(input);
  }
...
${meta}`;
  ...
  /**
   * Compose a body-first query for passive related recommendations. The title
   * remains a small disambiguator, but is intentionally appended after sampled
   * body passages so it cannot consume the embedding budget or dominate FTS.
   */
  private buildWorkspaceRelatedQuery(
    target: Pick<SemanticChunk, "title" | "text" | "tags" | "keywords">,
    sourceChunks: Array<Pick<SemanticChunk, "text">> = [],
  ): string {
    const sampleSource = sourceChunks.length
      ? sourceChunks.map((item) => String(item.text || ""))
      : [String(target.text || "")];
    const positions = Array.from(
      new Set([
        0,
        Math.floor(sampleSource.length / 2),
        Math.max(0, sampleSource.length - 1),
      ]),
    );
    const body = positions
      .map((position) => sampleSource[position] || "")
      .map((value) => {
        const normalized = value.replace(/\s+/g, " ").trim();
        if (normalized.length <= 950) return normalized;
        // One unsplit draft/chunk still needs coverage beyond its first lines.
        const middleStart = Math.max(
          0,
          Math.floor(normalized.length / 2) - 180,
        );
        return [
          normalized.slice(0, 360),
          normalized.slice(middleStart, middleStart + 360),
          normalized.slice(-230),
        ]
          .filter(Boolean)
          .join("\n");
      })
    ...
    const tags = Array.from(
      new Set([...(target.tags || []), ...(target.keywords || [])]),
    )
      .map((value) => String(value || "").trim())
      .filter(Boolean)
      .slice(0, 20);
    ...
    return [
      ...body,
      tags.length ? `関連語: ${tags.join(" ")}` : "",
      title ? `見出し: ${title.slice(0, 180)}` : "",
    ]
      .filter(Boolean)
      .join("\n")
      .slice(0, 3_500);
  ...
  async getWorkspaceSemanticRelatedDraft(input: {
    ...
    if (!index) {
      const missing = service.hasConfirmedMissingIndex();
      if (!missing) {
        service.warmIndexInBackground();
        return service.groupRelated(
          null,
          [],
          null,
          "Semantic Indexを準備中です。準備が完了すると下書きの関連候補を表示します。",
          { warming: true },
        );
      }
      return service.groupRelated(
    ...
    const excluded = (index.items || [])
      ...
      .map((item) => item.id);
    const query = this.buildWorkspaceRelatedQuery(target);
    const searched = await service
      .search(query, index, {
  ...
  async getWorkspaceSemanticRelated(input: {
    ...
    const service = await this.semanticIndexService();
    const index = service.getLoadedIndex();
    // Related information is passive. On a cold app start, return immediately
    // and hydrate the local-only cache after this response rather than making
    // page navigation wait for SQLite/shared-index reads.
    if (!index) {
      const missing = service.hasConfirmedMissingIndex();
      if (!missing) {
        service.warmIndexInBackground();
        return service.groupRelated(
          ...
          [],
          null,
          "Semantic Indexを準備中です。準備が完了すると関連候補を自動表示します。",
          { warming: true },
        );
      }
      return service.groupRelated(
    ...
    // Represent a long source by its beginning, middle and end. The query is
    // body-first; titles remain only a compact disambiguator at the end.
    const query = this.buildWorkspaceRelatedQuery(target, sourceChunks);
    const searched = await service
      .search(query, index, {
...
${normalized}`,
  ...
  private async atomicWriteJson(
    ...
    await this.atomicWriteText(file, JSON.stringify(data, null, 2), options);
  }

  private async atomicWriteText(
    file: string,
    data: string,
    options: { backup?: boolean } = { backup: false },
  ): Promise<void> {
    await fs.ensureDir(path.dirname(file));
    const tmp = `${file}.${this.appInstanceId}.${Date.now()}.${nanoid(6)}.tmp`;
    await fs.writeFile(tmp, data, "utf8");
    if (options.backup && (await fs.pathExists(file))) {
