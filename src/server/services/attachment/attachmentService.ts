...
export class AttachmentService {
  ...
  /** Rebuilds only one page's attachment rows after restore/import; never scans the workspace. */
  async reindexPageAttachments(pageId: string): Promise<number> {
    const page = this.deps.getPage(pageId);
    if (!page || page.meta.trashed) return 0;
    const items = await this.listAttachments(pageId);
    const tx = this.deps.db.transaction(() => {
      this.deps.db.prepare("DELETE FROM attachment_index WHERE page_id = ?").run(pageId);
