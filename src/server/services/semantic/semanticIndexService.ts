...
import { embedTextWithTransformer, getActiveTransformerModelId, normalizeJapaneseText } from '../transformerSemanticRetrieval';
import type { SemanticChunk, SemanticIndexItem, SemanticRelatedResult, SemanticSearchResult, SemanticWorkspaceIndex } from './semanticTypes';

export const WORKSPACE_SEMANTIC_INDEX_VERSION = 1;
export const WORKSPACE_SEMANTIC_ENGINE = 'workspace-semantic-ruri-v3-v3-chunked' as const;
...
function buildFtsMatchQuery(input: string): string | null {
  ...
  const words = normalized.split(/\s+/).map((value) => value.trim()).filter((value) => value.length >= 3);
  const grams: string[] = [];
  for (let i = 0; i <= compact.length - 3; i += 1) grams.push(compact.slice(i, i + 3));
...
function uniqueWords(input: string): string[] {
  ...
  const tokens = normalized.split(/\s+/).map((item) => item.trim()).filter((item) => item.length >= 2);
  const compact = normalized.replace(/\s+/g, '');
  const grams: string[] = [];
  for (let i = 0; i < Math.max(0, compact.length - 1); i += 1) grams.push(compact.slice(i, i + 2));
...
function lexicalScore(query: string, text: string): number {
  ...
  return Math.max(0, Math.min(100, Math.round((weighted / Math.max(4, queryTerms.length)) * 34 + (hits / queryTerms.length) * 38)));
}
...
function keywordOverlapScore(a: string[] = [], b: string[] = []): number {
  const left = new Set(a.map((item) => compactText(item)).filter((item) => item.length >= 2));
  const right = b.map((item) => compactText(item)).filter((item) => item.length >= 2);
  if (!left.size || !right.length) return 0;
  const hits = right.filter((item) => left.has(item)).length;
  return Math.min(100, Math.round((hits / Math.max(1, Math.min(left.size, right.length))) * 100));
}

function relationBoost(target: SemanticChunk | null | undefined, item: SemanticChunk): number {
  if (!target) return 0;
  let boost = 0;
  if (target.intentId && item.intentId && compactText(target.intentId) === compactText(item.intentId)) boost += 10;
  if (target.databaseId && item.databaseId && target.databaseId === item.databaseId) boost += 7;
  if (target.parentPageId && item.parentPageId && target.parentPageId === item.parentPageId) boost += 5;
  boost += Math.round(keywordOverlapScore(target.tags || [], item.tags || []) * 0.07);
  boost += Math.round(keywordOverlapScore(target.keywords || [], item.keywords || []) * 0.06);
...
function relatedEvidenceEnough(item: SemanticSearchResult): boolean {
  ...
  return (
    ...
    (item.titleScore || 0) >= 20 ||
    item.lexicalScore >= 18 ||
    (item.metaScore || 0) >= 20 ||
...
function filterRelatedQuality(results: SemanticSearchResult[]): { visible: SemanticSearchResult[]; hiddenLowScoreCount: number; minScore: number; minSemanticScore: number } {
  const minSemanticScore = 50;
  const visible = results.filter((item) => {
    const minScore = relatedMinScoreForType(item.chunk.type);
    return item.score >= minScore && item.semanticScore >= minSemanticScore && relatedEvidenceEnough(item);
  });
  return {
    visible,
...
function indexPath(sharedRoot: string): string {
  return path.join(vaultPaths(sharedRoot).smartAssist, 'workspace-semantic-index.json');
}
...
const MAX_EMBEDDING_META_CHARS = 420;
...
function stripNonSemanticPayload(input: unknown): string {
  return String(input ?? '')
    ...
    .replace(/<(?:img|image|video|audio|source|object|embed|iframe)\b[^>]*>/gi, ' ')
    // data: URIs, including base64 images embedded by editors.
    .replace(/\bdata:[a-z][a-z0-9+.-]*\/[a-z0-9+.-]+(?:;[^,\s]*)?,[^\s)>'"]+/gi, ' ')
...
function buildEmbeddingText(chunk: SemanticChunk): string {
  const title = safeSemanticText(chunk.title, 180);
  const tags = (chunk.tags || []).map((tag) => safeSemanticText(tag, 80)).filter(Boolean).slice(0, 24);
  const keywords = (chunk.keywords || []).map((word) => safeSemanticText(word, 80)).filter(Boolean).slice(0, 32);
  const meta = safeSemanticText([
    `種別: ${chunk.type}`,
    `タイトル: ${title}`,
    chunk.intentId ? `意図: ${safeSemanticText(chunk.intentId, 120)}` : '',
    tags.length ? `タグ: ${tags.join(' ')}` : '',
    keywords.length ? `重要語: ${keywords.join(' ')}` : '',
    chunk.semanticMetaText ? `メタ情報: ${safeSemanticText(chunk.semanticMetaText, 260)}` : '',
  ].filter(Boolean).join('\n'), MAX_EMBEDDING_META_CHARS);
  const bodyBudget = Math.max(850, MAX_EMBEDDING_TEXT_CHARS - meta.length - title.length - 40);
...
function normalizeChunk(chunk: SemanticChunk): SemanticChunk | null {
  ...
  return {
    ...
    text,
    keywords: Array.isArray(chunk.keywords) ? chunk.keywords.map((item) => safeSemanticText(item, 120)).filter(Boolean).slice(0, 40) : undefined,
    tags: Array.isArray(chunk.tags) ? chunk.tags.map((item) => safeSemanticText(item, 120)).filter(Boolean).slice(0, 40) : undefined,
    semanticMetaText: safeSemanticText(chunk.semanticMetaText, 2_000) || undefined,
  };
}
...
function safeJsonArray(value: unknown): number[] {
  try {
    ...
    return Array.isArray(raw) ? raw.map(Number).filter((item) => Number.isFinite(item)) : [];
  } catch {
    return [];
  }
}
...
export class SemanticIndexService {
  ...
  private vectorCapability: { available: boolean; error?: string } | null = null;
  ...
  private searchTelemetry = {
    ...
    lastEngine: 'not-run' as 'not-run' | 'sqlite-vec' | 'js-fallback' | 'embedding-unavailable',
  ...
  constructor(private sharedRoot: string, private localCacheDir?: string | null) {}
  ...
  private openCacheDb(): any | null {
    ...
    const cols = new Set((db.prepare("PRAGMA table_info(workspace_semantic_items)").all() as Array<{ name: string }>).map((row) => row.name));
    if (!cols.has('chunk_index')) db.exec("ALTER TABLE workspace_semantic_items ADD COLUMN chunk_index INTEGER NOT NULL DEFAULT 0");
    if (!cols.has('chunk_count')) db.exec("ALTER TABLE workspace_semantic_items ADD COLUMN chunk_count INTEGER NOT NULL DEFAULT 1");
    try {
      sqliteVec.load(db);
      ...
      this.vectorCapability = { available: false, error: String(error?.message || error) };
    }
    try {
      ...
      this.ftsCapability = { available: false, error: String(error?.message || error) };
    }
    return db;
  }
  ...
  private recordBuildRun(index: SemanticWorkspaceIndex | null, status: 'success' | 'partial' | 'error', startedAt: string, error?: string): void {
    let db: any;
    try {
      db = this.openCacheDb();
      if (!db) return;
      ...
      db.prepare(`DELETE FROM workspace_semantic_runs WHERE id NOT IN (SELECT id FROM workspace_semantic_runs ORDER BY started_at DESC LIMIT 50)`).run();
    } catch {
  ...
  private async readIndexFromCache(): Promise<SemanticWorkspaceIndex | null> {
    ...
    try {
      ...
      const metaRows = db.prepare('SELECT key, value FROM workspace_semantic_meta').all() as Array<{ key: string; value: string }>;
      const meta = new Map(metaRows.map((row) => [row.key, row.value]));
      if (meta.get('version') !== String(WORKSPACE_SEMANTIC_INDEX_VERSION)) return null;
      if (meta.get('engine') !== WORKSPACE_SEMANTIC_ENGINE) return null;
      if ((meta.get('model') || model) !== model) return null;
      const rows = db.prepare('SELECT * FROM workspace_semantic_items WHERE model = ? ORDER BY type, id').all(model) as any[];
      ...
      const items = rows.map((row) => ({
        ...
        keywords: (() => { try { const v = JSON.parse(row.keywords_json || '[]'); return Array.isArray(v) ? v.map(String).filter(Boolean) : undefined; } catch { return undefined; } })(),
        tags: (() => { try { const v = JSON.parse(row.tags_json || '[]'); return Array.isArray(v) ? v.map(String).filter(Boolean) : undefined; } catch { return undefined; } })(),
      ...
      })).filter((item) => item.id && item.sourceId && item.embedding.length) as SemanticIndexItem[];
  ...
  private toVectorBuffer(values: number[]): Buffer {
    const vector = new Float32Array(values.map((value) => Number.isFinite(Number(value)) ? Number(value) : 0));
  ...
  private syncVectorIndex(db: any, index: SemanticWorkspaceIndex): void {
    ...
    if (!this.vectorCapability?.available || !dimension || !index.items.length) {
      return;
    }
    const table = this.vectorTableName(dimension);
    const meta = db.prepare('INSERT OR REPLACE INTO workspace_semantic_meta (key, value) VALUES (?, ?)');
    const previousDimension = Number((db.prepare("SELECT value FROM workspace_semantic_meta WHERE key = 'sqliteVecDimension'").get() as any)?.value || 0);
    const previousModel = String((db.prepare("SELECT value FROM workspace_semantic_meta WHERE key = 'sqliteVecModel'").get() as any)?.value || '');
    if (previousDimension && (previousDimension !== dimension || previousModel !== model)) {
      const oldTable = this.vectorTableName(previousDimension);
      try { db.exec(`DROP TABLE IF EXISTS ${oldTable}`); } catch {}
      db.prepare('DELETE FROM workspace_semantic_vec_map').run();
    }
    db.exec(`CREATE VIRTUAL TABLE IF NOT EXISTS ${table} USING vec0(embedding float[${dimension}])`);
    const mapRows = db.prepare('SELECT item_id AS itemId, text_hash AS textHash, vec_rowid AS vecRowid FROM workspace_semantic_vec_map WHERE model = ? AND dimension = ?').all(model, dimension) as Array<{ itemId: string; textHash: string; vecRowid: number }>;
    const mapped = new Map(mapRows.map((row) => [String(row.itemId), row]));
    const activeIds = new Set(index.items.filter((item) => item.embedding?.length === dimension).map((item) => String(item.id)));
    const deleteVector = db.prepare(`DELETE FROM ${table} WHERE rowid = ?`);
    const deleteMap = db.prepare('DELETE FROM workspace_semantic_vec_map WHERE item_id = ?');
    for (const row of mapRows) {
      if (!activeIds.has(String(row.itemId))) {
        try { deleteVector.run(Number(row.vecRowid)); } catch {}
        deleteMap.run(String(row.itemId));
      }
    }
    const insertVector = db.prepare(`INSERT INTO ${table} (embedding) VALUES (?)`);
    const insertMap = db.prepare('INSERT OR REPLACE INTO workspace_semantic_vec_map (item_id, text_hash, model, dimension, vec_rowid) VALUES (?, ?, ?, ?, ?)');
    for (const item of index.items) {
      if (!item.embedding?.length || item.embedding.length !== dimension) continue;
      const previous = mapped.get(String(item.id));
      if (previous && previous.textHash === String(item.textHash || '')) continue;
      if (previous) {
        try { deleteVector.run(Number(previous.vecRowid)); } catch {}
      ...
      if (vecRowid > 0) insertMap.run(String(item.id), String(item.textHash || ''), model, dimension, vecRowid);
    }
    ...
    meta.run('sqliteVecRevision', String(index.revision || index.generatedAt || ''));
    meta.run('sqliteVecLastSyncAt', new Date().toISOString());
  }

  private searchVectorCandidates(queryEmbedding: number[], index: SemanticWorkspaceIndex, limit: number): Array<{ id: string; semanticScore: number }> | null {
    const dimension = Math.max(0, Number(index.dimension || 0));
    if (!this.vectorCapability?.available || !dimension || queryEmbedding.length !== dimension) return null;
    let db: any;
    try {
      db = this.openCacheDb();
      if (!db || !this.vectorCapability?.available) return null;
      const meta = db.prepare("SELECT key, value FROM workspace_semantic_meta WHERE key IN ('sqliteVecDimension', 'sqliteVecModel', 'sqliteVecRevision')").all() as Array<{ key: string; value: string }>;
      const values = Object.fromEntries(meta.map((row) => [row.key, row.value]));
      if (Number(values.sqliteVecDimension || 0) !== dimension || values.sqliteVecModel !== String(index.model || '')) return null;
      const table = this.vectorTableName(dimension);
      ...
      `).all(this.toVectorBuffer(queryEmbedding), Math.max(1, Math.min(1000, limit))) as Array<{ itemId: string; distance: number }>;
      return rows.map((row) => ({
        ...
        semanticScore: Math.max(0, Math.min(100, Math.round((1 - ((Number(row.distance || 0) ** 2) / 2)) * 100))),
      }));
    } catch {
      return null;
    } finally {
  ...
  private syncFtsIndex(db: any, index: SemanticWorkspaceIndex): void {
    ...
    const meta = db.prepare('INSERT OR REPLACE INTO workspace_semantic_meta (key, value) VALUES (?, ?)');
    const previousModel = String((db.prepare("SELECT value FROM workspace_semantic_meta WHERE key = 'sqliteFtsModel'").get() as any)?.value || '');
    if (previousModel && previousModel !== model) {
      try { db.prepare('DELETE FROM workspace_semantic_fts').run(); } catch {}
      db.prepare('DELETE FROM workspace_semantic_fts_map').run();
    }
    const mapRows = db.prepare('SELECT item_id AS itemId, text_hash AS textHash, fts_rowid AS ftsRowid FROM workspace_semantic_fts_map WHERE model = ?').all(model) as Array<{ itemId: string; textHash: string; ftsRowid: number }>;
    const mapped = new Map(mapRows.map((row) => [String(row.itemId), row]));
    const activeIds = new Set(index.items.map((item) => String(item.id)));
    const deleteFts = db.prepare('DELETE FROM workspace_semantic_fts WHERE rowid = ?');
    const deleteMap = db.prepare('DELETE FROM workspace_semantic_fts_map WHERE item_id = ?');
    for (const row of mapRows) {
      if (!activeIds.has(String(row.itemId))) {
        try { deleteFts.run(Number(row.ftsRowid)); } catch {}
        deleteMap.run(String(row.itemId));
      }
    }
    const insertFts = db.prepare('INSERT INTO workspace_semantic_fts (item_id, title, text, meta) VALUES (?, ?, ?, ?)');
    const insertMap = db.prepare('INSERT OR REPLACE INTO workspace_semantic_fts_map (item_id, text_hash, model, fts_rowid) VALUES (?, ?, ?, ?)');
    for (const item of index.items) {
      ...
      if (previous) {
        try { deleteFts.run(Number(previous.ftsRowid)); } catch {}
        deleteMap.run(String(item.id));
      }
      const result = insertFts.run(String(item.id), title, text, metaText);
      const ftsRowid = Number(result?.lastInsertRowid || 0);
      if (ftsRowid > 0) insertMap.run(String(item.id), lexicalHash, model, ftsRowid);
    }
    meta.run('sqliteFtsEnabled', 'true');
    meta.run('sqliteFtsEngine', SQLITE_FTS_ENGINE_VERSION);
    meta.run('sqliteFtsModel', model);
    meta.run('sqliteFtsRevision', String(index.revision || index.generatedAt || ''));
    meta.run('sqliteFtsLastSyncAt', new Date().toISOString());
  }

  private searchFtsCandidates(query: string, index: SemanticWorkspaceIndex, limit: number): Array<{ id: string }> | null {
    ...
    try {
      ...
      const metaRows = db.prepare("SELECT key, value FROM workspace_semantic_meta WHERE key IN ('sqliteFtsModel', 'sqliteFtsRevision')").all() as Array<{ key: string; value: string }>;
      const meta = Object.fromEntries(metaRows.map((row) => [row.key, row.value]));
      const expectedRevision = String(index.revision || index.generatedAt || '');
      if (meta.sqliteFtsModel !== String(index.model || '') || meta.sqliteFtsRevision !== expectedRevision) return null;
      const rows = db.prepare(`
        ...
        LIMIT ?
      `).all(match, String(index.model || getActiveTransformerModelId()), Math.max(1, Math.min(500, limit))) as Array<{ itemId: string }>;
  ...
  private runCacheMaintenance(db: any, options: { vacuum?: boolean; reason?: string } = {}): {
    removedItems: number;
    ...
    const before = {
      items: Number((db.prepare('SELECT COUNT(*) AS count FROM workspace_semantic_items').get() as any)?.count || 0),
      vectorMaps: Number((db.prepare('SELECT COUNT(*) AS count FROM workspace_semantic_vec_map').get() as any)?.count || 0),
      ftsMaps: Number((db.prepare('SELECT COUNT(*) AS count FROM workspace_semantic_fts_map').get() as any)?.count || 0),
      failures: Number((db.prepare('SELECT COUNT(*) AS count FROM workspace_semantic_failures').get() as any)?.count || 0),
    };
    const activeItemSql = 'SELECT id FROM workspace_semantic_items';
    const vectorRows = db.prepare(`SELECT item_id AS itemId, vec_rowid AS vecRowid FROM workspace_semantic_vec_map WHERE item_id NOT IN (${activeItemSql})`).all() as Array<{ itemId: string; vecRowid: number }>;
    const ftsRows = db.prepare(`SELECT item_id AS itemId, fts_rowid AS ftsRowid FROM workspace_semantic_fts_map WHERE item_id NOT IN (${activeItemSql})`).all() as Array<{ itemId: string; ftsRowid: number }>;
    const dimension = Number((db.prepare("SELECT value FROM workspace_semantic_meta WHERE key = 'sqliteVecDimension'").get() as any)?.value || 0);
    const vectorTable = dimension > 0 ? this.vectorTableName(dimension) : null;
    db.transaction(() => {
      if (vectorTable) {
        const removeVector = db.prepare(`DELETE FROM ${vectorTable} WHERE rowid = ?`);
        for (const row of vectorRows) { try { removeVector.run(row.vecRowid); } catch {} }
      }
      const removeVectorMap = db.prepare('DELETE FROM workspace_semantic_vec_map WHERE item_id = ?');
      for (const row of vectorRows) removeVectorMap.run(row.itemId);
      try {
        const removeFts = db.prepare('DELETE FROM workspace_semantic_fts WHERE rowid = ?');
        for (const row of ftsRows) removeFts.run(row.ftsRowid);
      } catch {}
      const removeFtsMap = db.prepare('DELETE FROM workspace_semantic_fts_map WHERE item_id = ?');
      ...
      if (options.vacuum && vectorTable) {
        try { db.prepare(`DELETE FROM ${vectorTable} WHERE rowid NOT IN (SELECT vec_rowid FROM workspace_semantic_vec_map)`).run(); } catch {}
      }
      if (options.vacuum) {
        try { db.prepare('DELETE FROM workspace_semantic_fts WHERE rowid NOT IN (SELECT fts_rowid FROM workspace_semantic_fts_map)').run(); } catch {}
      }
      db.prepare(`DELETE FROM workspace_semantic_failures WHERE id NOT IN (${activeItemSql})`).run();
      db.prepare('DELETE FROM workspace_semantic_runs WHERE id NOT IN (SELECT id FROM workspace_semantic_runs ORDER BY started_at DESC LIMIT 50)').run();
    })();
    let vectorOrphans = 0;
    if (vectorTable) {
      try { vectorOrphans = Number((db.prepare(`SELECT COUNT(*) AS count FROM ${vectorTable} WHERE rowid NOT IN (SELECT vec_rowid FROM workspace_semantic_vec_map)`).get() as any)?.count || 0); } catch {}
    }
    let ftsOrphans = 0;
    try { ftsOrphans = Number((db.prepare('SELECT COUNT(*) AS count FROM workspace_semantic_fts WHERE rowid NOT IN (SELECT fts_rowid FROM workspace_semantic_fts_map)').get() as any)?.count || 0); } catch {}
    ...
    meta.run('maintenanceReason', String(options.reason || (options.vacuum ? 'manual-compact' : 'automatic-sync')));
    meta.run('maintenanceRemovedItems', String(Math.max(0, before.items - after.items)));
    meta.run('maintenanceRemovedVectorMaps', String(Math.max(0, before.vectorMaps - after.vectorMaps)));
    meta.run('maintenanceRemovedFtsMaps', String(Math.max(0, before.ftsMaps - after.ftsMaps)));
    meta.run('maintenanceRemovedFailures', String(Math.max(0, before.failures - after.failures)));
    ...
    return {
      ...
      vectorOrphans, ftsOrphans, vacuumed, completedAt,
    };
  }

  private scheduleCacheMaintenance(): void {
    if (this.maintenanceQueued || this.maintenanceRunning || !this.cacheDbPath()) return;
    ...
    const timer = setTimeout(() => {
      ...
      try { this.runCacheMaintenance(db, { reason: 'automatic-sync' }); } catch {}
  ...
  async maintainCache(options: { vacuum?: boolean } = {}): Promise<any> {
    const dbPath = this.cacheDbPath();
    if (!dbPath) return { ok: false, enabled: false, message: 'ローカルSQLiteキャッシュ保存先が未設定です。' };
    const db = this.openCacheDb();
    if (!db) return { ok: false, enabled: false, message: 'SQLiteキャッシュを開けません。' };
    if (this.maintenanceRunning) return { ok: false, busy: true, message: '別のキャッシュ保守処理を実行中です。' };
    this.maintenanceRunning = true;
    const beforeBytes = Number((await fs.stat(dbPath).catch(() => ({ size: 0 } as any))).size || 0);
    try {
      const result = this.runCacheMaintenance(db, { vacuum: options.vacuum === true, reason: options.vacuum ? 'manual-compact' : 'manual-cleanup' });
      const afterBytes = Number((await fs.stat(dbPath).catch(() => ({ size: 0 } as any))).size || 0);
      return { ok: true, enabled: true, ...result, beforeBytes, afterBytes, reclaimedBytes: Math.max(0, beforeBytes - afterBytes) };
    } finally {
      this.maintenanceRunning = false;
    }
  }

  private async writeIndexToCache(index: SemanticWorkspaceIndex): Promise<void> {
    let db: any;
    try {
      db = this.openCacheDb();
      if (!db) return;
      ...
      const tx = db.transaction(() => {
        ...
        meta.run('revision', index.revision || index.generatedAt || new Date().toISOString());
        meta.run('indexedCount', String(index.indexedCount || index.items.length));
        ...
        meta.run('lastEmbeddedThisRun', String(buildStats.embeddedThisRun ?? ''));
        ...
        meta.run('lastMaxNewEmbeddings', String(buildStats.maxNewEmbeddings ?? ''));
        meta.run('lastNormalizedCount', String(buildStats.normalizedCount ?? ''));
        meta.run('lastFailedEmbeddingCount', String(buildStats.failedEmbeddingCount ?? 0));
        ...
        const failureEntries = Array.isArray(buildStats.failedEmbeddingEntries) ? buildStats.failedEmbeddingEntries : [];
        const resolvedFailureIds = Array.isArray(buildStats.resolvedFailureIds) ? buildStats.resolvedFailureIds.map(String) : [];
        ...
        for (const failure of failureEntries) {
          failureUpsert.run(
            String(failure.id || ''), String(failure.sourceId || ''), String(failure.type || ''),
            String(failure.title || failure.sourceId || ''), Number(failure.chunkIndex || 0), Math.max(1, Number(failure.chunkCount || 1)),
            String(failure.error || 'embedding unavailable').slice(0, 4000), String(failure.failedAt || index.generatedAt || new Date().toISOString()),
          );
        }
        if (resolvedFailureIds.length) {
          const clearFailure = db.prepare('DELETE FROM workspace_semantic_failures WHERE id = ?');
          for (const id of resolvedFailureIds) clearFailure.run(id);
        }
        const existing = db.prepare('SELECT id FROM workspace_semantic_items WHERE model = ?').all(index.model || getActiveTransformerModelId()) as Array<{ id: string }>;
        const remove = db.prepare('DELETE FROM workspace_semantic_items WHERE id = ? AND model = ?');
        for (const row of existing) {
          if (!activeIds.has(String(row.id))) remove.run(row.id, index.model || getActiveTransformerModelId());
        }
  ...
  async readIndex(): Promise<SemanticWorkspaceIndex | null> {
    ...
    if (!raw || raw.version !== WORKSPACE_SEMANTIC_INDEX_VERSION || raw.engine !== WORKSPACE_SEMANTIC_ENGINE || !Array.isArray(raw.items)) return null;
  ...
  async getCacheInfo(): Promise<any> {
    const dbPath = this.cacheDbPath();
    if (!dbPath) return { enabled: false, dbPath: null, itemCount: 0, typeCounts: {}, databaseRowPolicy: 'text-fields-only-v322' };
    let db: any;
    try {
      db = this.openCacheDb();
      const itemCount = Number((db.prepare('SELECT COUNT(*) AS count FROM workspace_semantic_items').get() as any)?.count || 0);
      const rows = db.prepare('SELECT type, COUNT(*) AS count FROM workspace_semantic_items GROUP BY type').all() as Array<{ type: string; count: number }>;
      const typeCounts = rows.reduce((acc: Record<string, number>, row) => { acc[String(row.type)] = Number(row.count || 0); return acc; }, {});
      const metaRows = db.prepare('SELECT key, value FROM workspace_semantic_meta').all() as Array<{ key: string; value: string }>;
      const meta = Object.fromEntries(metaRows.map((row) => [row.key, row.value]));
      const rawRecentRuns = db.prepare(`
        SELECT id, started_at AS startedAt, ended_at AS endedAt, status, mode,
      ...
      const recentRuns = rawRecentRuns.slice(0, 8).map((run) => {
        ...
        return {
          ...run,
          durationMs,
          embeddingMsPerItem: durationMs !== null && embedded > 0 ? Math.round(durationMs / embedded) : null,
        };
      });
      const measuredRuns = rawRecentRuns.map((run) => {
        const startedMs = Date.parse(String(run.startedAt || ''));
        const endedMs = Date.parse(String(run.endedAt || ''));
        const durationMs = Number.isFinite(startedMs) && Number.isFinite(endedMs)
          ? Math.max(0, endedMs - startedMs)
      ...
      const timing = {
        ...
        lastEmbeddingMsPerItem: lastMeasured && lastMeasured.embedded > 0
          ? Math.round(Number(lastMeasured.durationMs || 0) / lastMeasured.embedded)
          : null,
        averageLastFiveRunMs: lastFive.length
          ? Math.round(lastFive.reduce((sum, run) => sum + Number(run.durationMs || 0), 0) / lastFive.length)
          : null,
      ...
      const failureRows = db.prepare(`
        ...
        LIMIT 80
      `).all();
      const failureCount = Number((db.prepare('SELECT COUNT(*) AS count FROM workspace_semantic_failures').get() as any)?.count || 0);
      const vectorMapCount = Number((db.prepare('SELECT COUNT(*) AS count FROM workspace_semantic_vec_map').get() as any)?.count || 0);
      const vector = {
        requested: true,
        mode: Boolean(this.vectorCapability?.available) ? 'primary-with-js-fallback' : 'js-fallback-only',
      ...
      const ftsMapCount = Number((db.prepare('SELECT COUNT(*) AS count FROM workspace_semantic_fts_map').get() as any)?.count || 0);
      const fts = {
        requested: true,
        mode: Boolean(this.ftsCapability?.available) ? 'candidate-injection' : 'disabled',
        ...
        telemetry: { lexicalSearchCount: this.searchTelemetry.lexicalSearchCount, lastLexicalCandidateCount: this.searchTelemetry.lastLexicalCandidateCount },
      };
      ...
      const maintenance = {
        ...
        manualCompactRecommended: sizeBytes >= 256 * 1024 * 1024 || Number(meta.maintenanceVectorOrphans || 0) > 0 || Number(meta.maintenanceFtsOrphans || 0) > 0,
      };
      return { enabled: true, dbPath, itemCount, typeCounts, meta, vector, fts, connection, maintenance, recentRuns, timing, failures: failureRows, failureCount, sizeBytes, databaseRowPolicy: 'text-fields-only-v322', diagnostics: 'v450-cache-hygiene' };
    } catch (error: any) {
      return { enabled: true, dbPath, ok: false, error: String(error?.message || error), databaseRowPolicy: 'text-fields-only-v322' };
    } finally {
      this.releaseCacheDb(db);
    }
  }

  estimateDiff(chunks: SemanticChunk[], previous?: SemanticWorkspaceIndex | null): SemanticDiffInfo {
    const model = getActiveTransformerModelId();
    const normalized = chunks.map(normalizeChunk).filter((item): item is SemanticChunk => Boolean(item));
    ...
    for (const chunk of normalized) {
      ...
      const hash = textHash(embeddingText);
    ...
    const deleted = previousItems.filter((item) => !activeIds.has(item.id)).length;
    return {
  ...
  async buildIndex(chunks: SemanticChunk[], previous?: SemanticWorkspaceIndex | null, options: SemanticBuildOptions = {}): Promise<SemanticWorkspaceIndex> {
    const startedAt = new Date().toISOString();
    const model = getActiveTransformerModelId();
    const normalized = chunks.map(normalizeChunk).filter((item): item is SemanticChunk => Boolean(item));
    const preferredIds = new Set((options.preferredChunkIds || []).map(String).filter(Boolean));
    const onlySourceIds = new Set((options.onlySourceIds || []).map(String).filter(Boolean));
    const forceSourceIds = new Set((options.forceSourceIds || []).map(String).filter(Boolean));
    ...
    if (preferredIds.size) {
      const isPreferred = (id: string) => Array.from(preferredIds).some((preferred) => id === preferred || id.startsWith(`${preferred}:chunk:`));
      normalized.sort((left, right) => Number(isPreferred(right.id)) - Number(isPreferred(left.id)));
    }
    const previousById = new Map(((previous?.model === model ? previous.items : []) || []).map((item) => [item.id, item]));
    const items: SemanticIndexItem[] = [];
    let dimension = previous?.dimension || 0;
    let available = true;
    let error = '';
    const maxNewEmbeddings = Number.isFinite(Number(options.maxNewEmbeddings)) && Number(options.maxNewEmbeddings) > 0
    ...
    const failedEmbeddingEntries: Array<{ id: string; sourceId: string; type: string; title: string; chunkIndex: number; chunkCount: number; error: string; failedAt: string }> = [];
    ...
    for (const chunk of normalized) {
      ...
      const hash = textHash(embeddingText);
      const previousItem = previousById.get(chunk.id);
      const isTargetedSource = !onlySourceIds.size || onlySourceIds.has(String(chunk.sourceId));
      const forceEmbedding = forceSourceIds.has(String(chunk.sourceId));
      if (!isTargetedSource) {
        if (previousItem?.embedding?.length) {
          items.push({ ...chunk, textHash: previousItem.textHash || '', embedding: previousItem.embedding, dimension: previousItem.dimension });
          dimension = previousItem.dimension || dimension;
        }
        continue;
      }
      if (!forceEmbedding && previousItem && previousItem.textHash === hash && previousItem.embedding?.length) {
        items.push({ ...chunk, textHash: hash, embedding: previousItem.embedding, dimension: previousItem.dimension });
        dimension = previousItem.dimension || dimension;
        reusedCount += 1;
        continue;
      }
      ...
      if (!embedded.available || !embedded.embedding.length) {
        ...
        failedEmbeddingEntries.push({
          id: chunk.id, sourceId: chunk.sourceId, type: chunk.type, title: chunk.title,
          chunkIndex: Number(chunk.chunkIndex || 0), chunkCount: Math.max(1, Number(chunk.chunkCount || 1)),
          error: String(embedded.error || 'workspace semantic embedding unavailable'), failedAt: new Date().toISOString(),
        });
        error = error || embedded.error || 'workspace semantic embedding unavailable';
        if (previousItem?.embedding?.length) {
          items.push({ ...chunk, textHash: previousItem.textHash || '', embedding: previousItem.embedding, dimension: previousItem.dimension });
          dimension = previousItem.dimension || dimension;
          staleKeptCount += 1;
      ...
      embeddedThisRun += 1;
      items.push({ ...chunk, textHash: hash, embedding: embedded.embedding, dimension: embedded.dimension });
      resolvedFailureIds.push(chunk.id);
      // Let Electron/Node process pending IPC and paint work during a long full build.
      if (embeddedThisRun % 4 === 0) await new Promise<void>((resolve) => setImmediate(resolve));
    }
    ...
    const index: SemanticWorkspaceIndex = {
      ...
      revision: builtAt,
      indexedCount: items.length,
      available: available && items.length === normalized.length,
      error: error || undefined,
      items,
    };
    (index as any).buildStats = {
      mode: options.mode || (Number.isFinite(maxNewEmbeddings) ? 'diff' : 'full'),
      maxNewEmbeddings: Number.isFinite(maxNewEmbeddings) ? maxNewEmbeddings : null,
      embeddedThisRun,
      reusedCount,
      pendingCount,
      staleKeptCount,
    ...
    await this.writeIndexToCache(index).catch(() => undefined);
    this.memoryIndex = index;
    this.recordBuildRun(index, pendingCount > 0 || !available ? 'partial' : 'success', startedAt, error);
    return index;
  }

  async search(query: string, index: SemanticWorkspaceIndex | null, options: SemanticSearchOptions = {}): Promise<{ available: boolean; results: SemanticSearchResult[]; error?: string }> {
    const startedAt = Date.now();
    const finishTelemetry = (engine: 'sqlite-vec' | 'js-fallback' | 'embedding-unavailable', candidateCount: number, resultCount: number) => {
      if (engine === 'sqlite-vec') this.searchTelemetry.vectorSearchCount += 1;
      else if (engine === 'js-fallback') this.searchTelemetry.fallbackSearchCount += 1;
    ...
    if (!index || !index.items.length) {
      ...
      return { available: false, results: [], error: 'workspace semantic index is missing' };
    }
    ...
    const vectorCandidates = this.searchVectorCandidates(embedded.embedding, index, vectorLimit);
    const usedVector = Boolean(vectorCandidates);
    const ftsCandidates = vectorCandidates ? this.searchFtsCandidates(query, index, Math.max(60, limit * 8)) : null;
    ...
    if (vectorCandidates) {
      for (const candidate of vectorCandidates) candidateScores.set(candidate.id, candidate.semanticScore);
    }
    ...
    if (vectorCandidates && ftsCandidates) {
      for (const candidate of ftsCandidates) {
        ...
        if (!chunk) continue;
        candidateScores.set(candidate.id, Math.round(cosine(embedded.embedding, chunk.embedding) * 100));
      }
    }
    const candidateRows = vectorCandidates
      ? Array.from(candidateScores.entries()).map(([id, semanticScore]) => ({ chunk: byId.get(id), semanticScore }))
      : index.items.map((item) => ({ chunk: item, semanticScore: Math.round(cosine(embedded.embedding, item.embedding) * 100) }));
    const candidates = candidateRows
      .filter((candidate): candidate is { chunk: SemanticIndexItem; semanticScore: number } => Boolean(candidate.chunk))
      .filter(({ chunk }) => !exclude.has(chunk.id) && (!types.size || types.has(chunk.type)))
      .map(({ chunk, semanticScore }) => ({ chunk, semanticScore, relationBoost: relationBoost(options.target, chunk) }));
    ...
    const lexicalPool = options.prefilterBySemantic
      ? candidates.sort((a, b) => (b.semanticScore + b.relationBoost) - (a.semanticScore + a.relationBoost)).slice(0, Math.max(120, limit * 10))
      : candidates;
    const results = lexicalPool
      .map(({ chunk, semanticScore, relationBoost: boost }) => {
        ...
        const lex = Math.round(title * 0.48 + body * 0.34 + meta * 0.18);
        ...
        return { chunk, score, semanticScore, lexicalScore: lex, titleScore: title, metaScore: meta, relationBoost: boost, reasons };
      })
      .filter((item) => item.score >= (options.minScore ?? 30))
      .sort((a, b) => b.score - a.score)
      .slice(0, limit);
    finishTelemetry(usedVector ? 'sqlite-vec' : 'js-fallback', candidates.length, results.length);
    return { available: true, results };
  }

  groupRelated(target: SemanticChunk | null, results: SemanticSearchResult[], index: SemanticWorkspaceIndex | null, warning?: string): SemanticRelatedResult {
    ...
    const compactResults = Array.from(bestBySource.values()).sort((a, b) => b.score - a.score);
    return {
      ok: true,
      target,
      ...
      groups: {
        pages: compactResults.filter((item) => item.chunk.type === 'page').slice(0, 8),
        faqs: compactResults.filter((item) => item.chunk.type === 'faq').slice(0, 8),
        databaseRows: compactResults.filter((item) => item.chunk.type === 'database_row').slice(0, 8),
        journals: compactResults.filter((item) => item.chunk.type === 'journal').slice(0, 8),
        attachments: compactResults.filter((item) => item.chunk.type === 'attachment_summary').slice(0, 8),
      },
    };
  }
}
