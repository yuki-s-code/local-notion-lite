...
export function migrate(db: Db): void {
  ...
  if (!columns.some((column) => column.name === 'properties_json')) {
    db.exec("ALTER TABLE pages ADD COLUMN properties_json TEXT NOT NULL DEFAULT '{}'");
  }
  ...
  const journalColumns = db.prepare("PRAGMA table_info(journal_summary_index)").all() as Array<{ name: string }> ;
  if (!journalColumns.some((column) => column.name === 'full_text')) {
    db.exec("ALTER TABLE journal_summary_index ADD COLUMN full_text TEXT NOT NULL DEFAULT ''");
  }
  db.exec("CREATE INDEX IF NOT EXISTS idx_journal_summary_fulltext_updated ON journal_summary_index(updated_at)");
  ...
  const linkColumns = db.prepare("PRAGMA table_info(workspace_link_index)").all() as Array<{ name: string }>;
  if (!linkColumns.some((column) => column.name === 'target_type')) {
    db.exec("ALTER TABLE workspace_link_index ADD COLUMN target_type TEXT NOT NULL DEFAULT 'page'");
  }
  if (!linkColumns.some((column) => column.name === 'target_database_id')) {
    db.exec("ALTER TABLE workspace_link_index ADD COLUMN target_database_id TEXT");
  }
  ...
  db.exec("CREATE INDEX IF NOT EXISTS idx_workspace_link_target_row ON workspace_link_index(target_type, target_database_id, target_row_id, updated_at)");

}

export function rebuildFts(db: Db): void {
  db.exec('DELETE FROM page_fts;');
  db.exec(`INSERT INTO page_fts(id, title, markdown) SELECT id, title, markdown FROM pages WHERE trashed = 0;`);
}

export function upsertPageFts(db: Db, page: { id: string; title?: string | null; markdown?: string | null; trashed?: boolean | number | null }): void {
