...
/** Tracks an API startup that continues after the first window is visible. */
let apiStartupPromise: Promise<void> | null = null;
...
type StartupProgress = {
  stage: "renderer" | "settings" | "localDb" | "workspace" | "api" | "ready" | "error";
  title?: string;
  message: string;
  detail?: string;
};
...
function publishStartupProgress(progress: StartupProgress): void {
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send("app:startup-progress", progress);
  }
}
...
async function shutdownApplication(): Promise<void> {
  ...
  // The local API now starts after the first window is visible.  If the user
  // closes immediately, wait for that startup to settle so its SQLite handle
  // cannot outlive Electron's controlled shutdown path.
  await apiStartupPromise?.catch(() => undefined);
...
async function chooseSharedRoot(): Promise<string> {
  ...
  const result = await showAppOpenDialog({ title: '社内共有フォルダを選択してください', properties: ['openDirectory', 'createDirectory'] });
...
async function chooseLocalDbPathAtStartup(): Promise<string | undefined> {
  ...
  const answer = mainWindow && !mainWindow.isDestroyed()
    ? await dialog.showMessageBox(mainWindow, sqliteChoiceOptions)
    : await dialog.showMessageBox(sqliteChoiceOptions);
...
async function createWindow(): Promise<void> {
  // Create the visible window before touching a network share, opening SQLite,
  // or showing first-run storage choices.  A slow SMB path must never look like
  // a blank, frozen application.
  publishStartupProgress({
    stage: "renderer",
    title: "画面を準備しています",
    message: "アプリ画面を表示しています。",
  });
  // Do not wait for SQLite migration or shared-folder directory checks before
  // creating the first window.  On a slow SMB share that wait made the whole
  // application appear frozen for tens of seconds.  The renderer starts with a
  // lightweight boot state and receives the API bootstrap payload when ready.
  ...
  let rendererFinishedInitialLoad = false;

  // Kept for compatibility with an already-open renderer.  Unlike the prior
  // implementation, this listener does not wait for API startup: it records
  // that the first paint is available and the background startup publishes the
  // payload afterwards.
  mainWindow.webContents.once('did-finish-load', () => {
    rendererFinishedInitialLoad = true;
    if (bootstrapPayload) mainWindow?.webContents.send('app:ready', bootstrapPayload);
  });
  ...
  console.info(`[startup] renderer-first-load ${Math.round(performance.now() - rendererLoadStartedAt)}ms`);

  // Yield once after renderer load so Chromium can paint the boot screen before
  // the synchronous SQLite open/migration work runs in this Electron process.
  // The API remains authoritative; no workspace data is exposed until this
  // promise publishes its tokenized bootstrap payload.
  apiStartupPromise = new Promise<void>((resolve) => {
    ...
    publishStartupProgress({
      stage: "settings",
      title: "起動設定を確認しています",
      message: "共有フォルダとローカル保存先を確認しています。",
      detail: "初回や社内ネットワーク接続時は少し時間がかかることがあります。",
    });
    ...
    const activeOcrBinaryPath = bootSettings.ocrBinaryPath && await fs.pathExists(bootSettings.ocrBinaryPath)
      ? bootSettings.ocrBinaryPath
      : undefined;
    const activePopplerBinaryPath = bootSettings.popplerBinaryPath && await fs.pathExists(bootSettings.popplerBinaryPath)
      ? bootSettings.popplerBinaryPath
      : undefined;
    const startedApi = await startLocalApi(sharedRoot, localDbPath, {
      privatePagesRoot: bootSettings.privatePagesRoot,
      privateDatabasesRoot: bootSettings.privateDatabasesRoot,
      onStartupProgress: (stage, message, detail) => publishStartupProgress({ stage, message, detail }),
    });
    api = startedApi;
    bootstrapPayload = {
      apiUrl: startedApi.url,
      apiToken: startedApi.apiToken,
      sharedRoot,
      localDbPath,
      privatePagesRoot: bootSettings.privatePagesRoot,
      privateDatabasesRoot: bootSettings.privateDatabasesRoot,
    ...
    publishStartupProgress({
      ...
      detail: "共有フォルダの同期は、画面を開いた後にバックグラウンドで続けます。",
    });
    console.info(`[startup] local-api-ready ${Math.round(performance.now() - apiStartedAt)}ms`, startedApi.startupTimings);
    if (rendererFinishedInitialLoad && mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('app:ready', bootstrapPayload);
    }
  }).catch((error) => {
    console.error('[startup] local API failed after first paint', error);
    publishStartupProgress({
      stage: "error",
      title: "起動を完了できませんでした",
      message: "共有フォルダまたはローカル保存先を確認してください。",
      detail: error instanceof Error ? error.message : String(error),
    });
    throw error;
  });
  // The application must remain usable while API initialization runs.  Keep the
  // rejection observed here; shutdown also awaits the same promise when needed.
  void apiStartupPromise.catch(() => undefined);
}
...
if (hasSingleInstanceLock) app.whenReady().then(async () => {
  ...
  ipcMain.handle('app:getStartupProgress', async () => startupProgress);

  ipcMain.handle('app:openExternalHttpUrl', async (_event, rawUrl: string) => {
    return openExternalHttpUrl(rawUrl).catch(() => false);
  });

  ipcMain.handle('googleWorkspace:getStatus', async () => googleWorkspace.getStatus());
  ipcMain.handle('googleWorkspace:configure', async (_event, clientId: string) => googleWorkspace.configure(clientId));
  ipcMain.handle('googleWorkspace:connect', async (_event, capabilities) => googleWorkspace.connect(capabilities));
  ipcMain.handle('googleWorkspace:disconnect', async () => googleWorkspace.disconnect());
  ipcMain.handle('googleWorkspace:listSharedDrives', async () => googleWorkspace.listSharedDrives());
  ipcMain.handle('googleWorkspace:searchFiles', async (_event, query: string, driveId?: string) => googleWorkspace.searchFiles(query, driveId));
  ipcMain.handle('googleWorkspace:getDriveFileContent', async (_event, fileId: string) => googleWorkspace.getDriveFileContent(fileId));
  ipcMain.handle('googleWorkspace:listCalendars', async () => googleWorkspace.listCalendars());
  ipcMain.handle('googleWorkspace:listCalendarEvents', async (_event, calendarId: string, timeMin: string, timeMax: string) => googleWorkspace.listCalendarEvents(calendarId, timeMin, timeMax));
  ipcMain.handle('googleWorkspace:listDriveChanges', async (_event, pageToken?: string) => googleWorkspace.listDriveChanges(pageToken));
  ipcMain.handle('googleWorkspace:syncDriveChanges', async () => googleWorkspace.syncDriveChanges());
  ipcMain.handle('googleWorkspace:searchGmailMessages', async (_event, query: string) => googleWorkspace.searchGmailMessages(query));
  ipcMain.handle('googleWorkspace:createGmailDraft', async (_event, input) => googleWorkspace.createGmailDraft(input));
  ipcMain.handle('googleWorkspace:createGoogleDoc', async (_event, input) => googleWorkspace.createGoogleDoc(input));
  ipcMain.handle('googleWorkspace:createGoogleSheet', async (_event, input) => googleWorkspace.createGoogleSheet(input));
