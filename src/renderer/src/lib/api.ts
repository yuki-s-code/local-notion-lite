...
import type { AnalysisDataDictionary, AnalysisNotebook, AnalysisParameter, AnalysisQueryResult, AnalysisStatus, AnalysisWorkspaceSettings } from "../../../shared/analysisTypes";
...
export class ApiClient {
  ...
  async runAnalysisSql(sql: string, parameters: AnalysisParameter[] = [], namedResults: import('../../../shared/analysisTypes').AnalysisNamedResult[] = []): Promise<AnalysisQueryResult> {
    return this.post('/analysis/query', { sql, parameters, namedResults });
  }

  async getAnalysisResultPage(resultId: string, page = 0, pageSize = 500): Promise<AnalysisQueryResult> {
    return this.get(`/analysis/results/${encodeURIComponent(resultId)}?page=${page}&pageSize=${pageSize}`);
  }

  async getAnalysisResultAll(resultId: string): Promise<AnalysisQueryResult> {
    return this.get(`/analysis/results/${encodeURIComponent(resultId)}/all`);
  }

  async generateAnalysisAiDraft(instruction: string): Promise<{ ok: boolean; generated: boolean; message?: string; elapsedMs?: number; draft?: import('../../../shared/analysisTypes').AnalysisAiDraft }> {
  ...
  async getAnalysisWorkspaceSettings(): Promise<AnalysisWorkspaceSettings> { return this.get('/analysis/settings'); }

  async saveAnalysisWorkspaceSettings(settings: AnalysisWorkspaceSettings): Promise<AnalysisWorkspaceSettings> { return this.put('/analysis/settings', settings); }

  async listAnalysisNotebooks(): Promise<AnalysisNotebook[]> {
    return this.get('/analysis/notebooks');
  }

  async saveAnalysisNotebook(notebook: AnalysisNotebook): Promise<AnalysisNotebook> {
    return this.put(`/analysis/notebooks/${encodeURIComponent(notebook.id)}`, notebook);
  }
  ...
  async listAnalysisDashboardPins(): Promise<import('../../../shared/analysisTypes').AnalysisDashboardPin[]> {
    return this.get('/analysis/dashboard');
  }

  async saveAnalysisDashboardPin(pin: import('../../../shared/analysisTypes').AnalysisDashboardPin): Promise<import('../../../shared/analysisTypes').AnalysisDashboardPin> {
  ...
  async getPage(id: string, signal?: AbortSignal): Promise<PageBundle> {
    return this.get(`/pages/${id}`, signal ? { signal } : undefined);
  }
  ...
  async createPage(
    ...
    return this.post("/pages", { title, parentId, scope });
  }
  ...
  async listDatabaseRowAttachments(databaseId: string, rowId: string, scope: 'shared' | 'private' = 'shared'): Promise<AttachmentInfo[]> {
    const params = scope === 'private' ? '?scope=private' : '';
    return this.get(`/databases/${this.pathId(databaseId)}/rows/${this.pathId(rowId)}/attachments${params}`);
  }

  databaseRowAttachmentPrettyFileUrl(databaseId: string, rowId: string, attachmentId: string, fileName: string, scope: 'shared' | 'private' = 'shared'): string {
    const params = scope === 'private' ? '?scope=private' : '';
  ...
  async uploadDatabaseRowAttachmentFile(databaseId: string, rowId: string, file: File, scope: 'shared' | 'private' = 'shared'): Promise<string> {
    if (!isBase64AttachmentWithinLimit(file.size)) throw new Error(base64AttachmentLimitMessage(file.name || 'このファイル'));
    const base64 = await fileToBase64(file);
    const info: AttachmentInfo = await this.post(`/databases/${this.pathId(databaseId)}/rows/${this.pathId(rowId)}/attachments/upload`, { fileName: file.name, base64, scope });
    return this.databaseRowAttachmentPrettyFileUrl(databaseId, rowId, info.id, info.fileName || file.name, scope);
  }
  ...
  async cancelInboxAttachmentOcrQueue(inboxId: string, attachmentId: string): Promise<InboxItem> {
    return this.post(`/inbox/${encodeURIComponent(inboxId)}/attachments/${encodeURIComponent(attachmentId)}/ocr/cancel`, {});
  }

  async retryInboxAttachmentOcrQueue(inboxId: string, attachmentId: string): Promise<InboxItem> {
    return this.post(`/inbox/${encodeURIComponent(inboxId)}/attachments/${encodeURIComponent(attachmentId)}/ocr/retry`, {});
  }
  ...
  async searchJournals(query: string, limit = 30): Promise<JournalSummary[]> {
    return this.get(`/journals/search?q=${encodeURIComponent(query)}&limit=${encodeURIComponent(String(limit))}`);
  }
  ...
  async addJournalAttachment(date: string, sourcePath: string): Promise<AttachmentInfo> {
    return this.post(`/journals/${encodeURIComponent(date)}/attachments`, { sourcePath });
  }

  journalAttachmentPrettyFileUrl(date: string, attachmentId: string, fileName: string): string {
  ...
  async uploadJournalAttachmentFile(date: string, file: File): Promise<string> {
    if (!isBase64AttachmentWithinLimit(file.size)) {
      throw new Error(base64AttachmentLimitMessage(file.name || 'このファイル'));
    }
    const base64 = await fileToBase64(file);
    const info: AttachmentInfo = await this.post(`/journals/${encodeURIComponent(date)}/attachments/upload`, { fileName: file.name, base64 });
    return this.journalAttachmentPrettyFileUrl(date, info.id, info.fileName || file.name);
  }
  ...
  async deleteTrashedDatabase(id: string): Promise<{ ok: true; id: string }> {
  ...
  private pathId(value: string): string {
