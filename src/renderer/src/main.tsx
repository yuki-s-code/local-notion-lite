...
import { TreeItem } from "./components/screens/PageTreeItem";
import { CommandPalette } from "./components/screens/CommandPalette";
import {
  WorkspaceAiChatPanel,
  WorkspaceAiSearch,
} from "./components/search/WorkspaceAiSearch";
import {
...
type PageContextMenuState = { x: number; y: number; page: PageTreeNode } | null;

function getViewportSafeContextMenuPosition(
  x: number,
  y: number,
  width = 284,
  height = 560,
...
function PageContextMenu({
  ...
  return (
    ...
    >
      ...
      {PAGE_TEMPLATES.filter((t) => t.key !== "blank").map((template) => (
        <button
          key={template.key}
          onClick={() => {
            onCreateFromTemplate(template, page.id);
            onClose();
          }}
        >
...
function getDateProperty(database: WorkspaceDatabase, view?: DatabaseView) {
  const configured = view?.datePropertyId
    ? database.properties.find(
        (prop) => prop.id === view.datePropertyId && (prop.type === "date" || prop.type === "created_time" || prop.type === "last_edited_time"),
      )
    : undefined;
  return configured ?? database.properties.find((prop) => prop.type === "date" || prop.type === "created_time" || prop.type === "last_edited_time");
}
...
function getTimelineStartProperty(
  ...
  const configured = view?.startDatePropertyId
    ? database.properties.find(
        (prop) => prop.id === view.startDatePropertyId && (prop.type === "date" || prop.type === "created_time" || prop.type === "last_edited_time"),
      )
    : undefined;
...
function getTimelineEndProperty(
  ...
  const configured = view?.endDatePropertyId
    ? database.properties.find(
        (prop) => prop.id === view.endDatePropertyId && (prop.type === "date" || prop.type === "created_time" || prop.type === "last_edited_time"),
      )
    : undefined;
...
function defaultDatabaseCellValue(
  ...
  if (type === "rollup" || type === "formula" || type === "button" || type === "created_time" || type === "last_edited_time") return "";
  return "";
}
...
function getFormulaValue(
  ...
  const safe: string = expression.replace(/\{([^}]+)\}/g, (_match: string, name: string): string =>
  ...
  const openSource = (task: TaskItem) => {
    if (task.sourceType === "page") onOpenPage(task.sourceId);
      ...
      if (rawDatabaseId && rawRowId) {
        try { onOpenDatabaseRow(decodeURIComponent(rawDatabaseId), decodeURIComponent(rawRowId)); }
        catch { onOpenInbox(); }
      } else onOpenInbox();
    } else onOpenInbox();
  };
...
function App() {
  ...
  const [workspaceActiveItem, setWorkspaceActiveItem] = useState<{ kind: "page" | "database"; id: string; rowId?: string | null; parentId?: string | null } | null>(null);
  ...
  >({});
  const saveRetryTimersRef = useRef<Record<string, number | null>>({});
  const saveRetryAttemptsRef = useRef<Record<string, number>>({});
  const lightRefreshInFlightRef = useRef(false);
  ...
  const workspaceDatabaseSaveQueuesRef = useRef<Record<string, {
    inFlight: boolean;
  ...
  const databaseRowPatchQueuesRef = useRef<Record<string, {
    inFlight: boolean;
  ...
  const [journalAttachments, setJournalAttachments] = useState<AttachmentInfo[]>([]);
  const [journalAttachmentUploading, setJournalAttachmentUploading] = useState(false);
  const [journalAttachmentPreview, setJournalAttachmentPreview] = useState<AttachmentInfo | null>(null);
  ...
  const [journalSearchResults, setJournalSearchResults] = useState<JournalSummary[] | null>(null);
  ...
  const markdownPreview = useMemo(
    () => blockNoteToMarkdown(blockNoteBlocks),
    [blockNoteBlocks],
  );

  // Primary navigation and link previews use separate sequences.
  const navigationCoordinatorRef = useRef(new NavigationCoordinator());
  // Abort an obsolete page request when users move to another page before it
  // completes. This prevents slow shared-folder/API work from competing with
  // the page the user actually chose.
  ...
  useEffect(() => {
    function handleOpenOcrCenter(event: Event) {
      const detail = (event as CustomEvent<{ inboxId?: string; attachmentId?: string }>).detail || {};
      setOcrCenterFocusKey(detail.inboxId && detail.attachmentId ? `${detail.inboxId}:${detail.attachmentId}` : '');
      setMainMode('ocr');
      setStatus('OCRセンターを開きました');
    }
    window.addEventListener('local-notion:open-ocr-center', handleOpenOcrCenter as EventListener);
    return () => window.removeEventListener('local-notion:open-ocr-center', handleOpenOcrCenter as EventListener);
  }, []);
  ...
  useEffect(() => {
    if (!window.localNotion?.getBootstrap) {
      setStatus(
        "Electron preload が見つかりません。本文リンクはアプリ内遷移で開いてください。",
      );
      return;
    }
    let disposed = false;
    ...
    // invoke is durable: it cannot be missed when React registers after did-finish-load.
    void window.localNotion
      .getBootstrap()
      .then(applyBootstrap)
      .catch((error: any) =>
        setStatus(error?.message ?? "起動情報を取得できませんでした"),
      );
    const unsubscribeReady = window.localNotion.onReady
      ? window.localNotion.onReady(applyBootstrap)
      : undefined;
    return () => {
      disposed = true;
      unsubscribeReady?.();
  ...
  async function refreshWorkspace(
    ...
    if (opts.databases) jobs.push(api.listDatabases().then((nextDatabases) => {
      nextDatabases.forEach((database) => {
        const known = lastPersistedDatabaseUpdatedAtRef.current[database.id];
        if (!known || Date.parse(database.updatedAt || '') >= Date.parse(known || '')) {
          lastPersistedDatabaseUpdatedAtRef.current[database.id] = database.updatedAt;
        }
      });
  ...
  async function lightRefresh(message = "表示を更新しました") {
    // Avoid overlapping periodic refreshes. A slow SMB/local DB response must
    // not queue another full tree/database/journal fetch behind it.
    if (lightRefreshInFlightRef.current) return;
    lightRefreshInFlightRef.current = true;
    try {
      await refreshWorkspace(message, {
        tree: true,
        databases: true,
      ...
      lightRefreshInFlightRef.current = false;
    }
  }
  ...
  useEffect(() => {
    ...
    // Render cached SQLite-derived lists first. Shared-folder import follows in the
    // background so a slow network share does not block the first usable screen.
    void refreshWorkspace("起動しました", {
      health: true,
      tree: true,
      databases: true,
      journals: true,
    ...
    // Do not start SMB/shared-folder import while Electron is still painting
    // its first usable screen or the user is opening their first page.  Cached
    // local lists are already shown above; the shared import is deliberately
    // deferred until the UI gets an idle slice (with a bounded fallback).
    let idleId: number | null = null;
    const runSharedImport = () => {
      if (cancelled) return;
      void refreshWorkspace("共有フォルダを同期しました", {
        importShared: true,
        health: false,
        tree: true,
        databases: true,
        ...
        if (!cancelled) setStatus(`共有フォルダ同期: ${err.message}`);
      });
    };
    const timer = window.setTimeout(() => {
      ...
      if (requestIdle) {
        idleId = requestIdle(runSharedImport, { timeout: 4000 });
      } else {
        runSharedImport();
      }
    }, 2200);
  ...
  useEffect(() => {
    ...
    const runWhenIdle = () => {
      ...
      if (
        ...
        editing ||
        dbEditing ||
        lightRefreshInFlightRef.current
      )
        return;
      const run = () => {
        void lightRefresh("軽量同期しました").catch(() => undefined);
      };
      const requestIdle = (window as any).requestIdleCallback as
        | undefined
        | ((callback: () => void, options?: { timeout?: number }) => number);
    ...
    const timer = window.setInterval(runWhenIdle, 45_000);
    return () => window.clearInterval(timer);
  }, [api, editing, dbEditing]);

  function clearSaveRecovery(kind: string) {
    const timer = saveRetryTimersRef.current[kind];
    if (timer) window.clearTimeout(timer);
    saveRetryTimersRef.current[kind] = null;
    saveRetryAttemptsRef.current[kind] = 0;
  ...
  function scheduleSaveRetry(
    ...
    const currentTimer = saveRetryTimersRef.current[kind];
    if (currentTimer) return;
    const plan = getSaveRetryPlan(saveRetryAttemptsRef.current[kind] || 0);
    const { attempt, delayMs: delay, exhausted } = plan;
    saveRetryAttemptsRef.current[kind] = attempt;
    setSaveRecovery((prev) => ({
      ...prev,
      [kind]: { label, attempt, exhausted },
    }));
    if (exhausted || delay === null) {
      setStatus(
        `${label}の保存に繰り返し失敗しています。未保存の内容を保持しています。［今すぐ再試行］を押してください。`,
      );
      return;
    }
    setStatus(
      `${label}の保存に失敗しました。${Math.ceil(delay / 1000)}秒後に自動再試行します（${attempt}/3）。`,
    );
    saveRetryTimersRef.current[kind] = window.setTimeout(() => {
      saveRetryTimersRef.current[kind] = null;
  ...
  async function retryPendingSavesNow() {
    Object.values(saveRetryTimersRef.current).forEach((timer) => {
      if (timer) window.clearTimeout(timer);
    });
    saveRetryTimersRef.current = {};
    saveRetryAttemptsRef.current = {};
    setSaveRecovery({});
  ...
  async function cancelInboxAttachmentOcr(inboxId: string, attachmentId: string): Promise<void> {
    if (!api) return;
    const next = await api.cancelInboxAttachmentOcrQueue(inboxId, attachmentId);
    setInboxItems((prev) => prev.map((item) => (item.id === inboxId ? next : item)));
    setStatus("OCRキューの停止を受け付けました。実行中の場合は現在のページ処理完了後に停止します。");
  }

  async function retryInboxAttachmentOcr(inboxId: string, attachmentId: string): Promise<void> {
    if (!api) return;
  ...
  async function sendJournalAttachmentToOcrCenter(attachment: AttachmentInfo): Promise<void> {
    ...
    try {
      ...
      setOcrCenterFocusKey(workingAttachment ? `${item.id}:${workingAttachment.id}` : "");
      await openOcrCenter();
      setStatus(item.attachments?.[0]?.ocr?.status === "ready" || item.attachments?.[0]?.pdfText?.status === "ready"
        ? "OCR結果をOCRセンターで開きました。"
  ...
  async function createChildPageForEditor(): Promise<PageWithLock | null> {
    ...
    try {
      ...
      await lightRefresh("子ページを作成しました");
      return { ...(page.meta as any), isLocked: false } as PageWithLock;
    } catch (e: any) {
      setStatus(e.message);
      return null;
    }
  ...
  async function createPageFromTemplate(
    ...
    try {
      ...
      await lightRefresh("テンプレートからページを作成しました");
      await openPage(page.meta.id);
    } catch (e: any) {
      setStatus(e.message);
    }
  ...
  async function openPage(id: string) {
    ...
    try {
      ...
      setCurrent(page);
      window.dispatchEvent(new CustomEvent("local-notion:workspace-open-item", {
  ...
  function scheduleSemanticIndexUpdate(target: SemanticAutoUpdateTarget): void {
    ...
    semanticAutoUpdatePendingTargetsRef.current.set(target.targetKey, target.preferredChunkId);
    ...
    semanticAutoUpdateTimerRef.current = window.setTimeout(async () => {
      ...
      if (semanticEditorActivityAtRef.current && editorIdleMs < editorQuietDelayMs) {
        semanticAutoUpdateTimerRef.current = window.setTimeout(() => {
          const next = semanticAutoUpdatePendingTargetsRef.current.entries().next().value as [string, string] | undefined;
          if (next) scheduleSemanticIndexUpdate({ targetKey: next[0], preferredChunkId: next[1] });
      ...
      const pendingTargets = Array.from(semanticAutoUpdatePendingTargetsRef.current.entries())
        .slice(0, 20)
        .map(([targetKey, preferredChunkId]) => ({ targetKey, preferredChunkId }));
      if (!pendingTargets.length) return;
      pendingTargets.forEach((item) => semanticAutoUpdatePendingTargetsRef.current.delete(item.targetKey));
      ...
      try {
        ...
        if (!indexInfo.available || !indexInfo.revision) {
          pendingTargets.forEach((item) => semanticAutoUpdatePendingTargetsRef.current.set(item.targetKey, item.preferredChunkId));
          return;
        }
        const result = await api.diffUpdateWorkspaceSemanticIndex(
          pendingTargets.length,
          { preferredChunkIds: pendingTargets.map((item) => item.preferredChunkId), background: true },
        );
        window.dispatchEvent(
          new CustomEvent("local-notion:semantic-index-updated", {
            detail: { revision: result?.revision || result?.generatedAt || null, mode: "autosave-diff", targets: pendingTargets },
          }),
        );
      } catch {
        ...
        if (semanticAutoUpdatePendingTargetsRef.current.size && semanticAutoUpdateTimerRef.current === null) {
          semanticAutoUpdateTimerRef.current = window.setTimeout(() => {
            const next = semanticAutoUpdatePendingTargetsRef.current.entries().next().value as [string, string] | undefined;
            if (next) scheduleSemanticIndexUpdate({ targetKey: next[0], preferredChunkId: next[1] });
          }, 800);
  ...
  function scheduleSemanticIndexUpdateForPage(pageId: string): void {
    if (!pageId) return;
    scheduleSemanticIndexUpdate({ targetKey: `page::${pageId}`, preferredChunkId: `page:${pageId}` });
  }

  useEffect(() => {
    const handleSemanticRefreshRequest = (event: Event) => {
      const detail = (event as CustomEvent<Partial<SemanticAutoUpdateTarget>>).detail;
      const targetKey = String(detail?.targetKey || '').trim();
      const preferredChunkId = String(detail?.preferredChunkId || '').trim();
      if (targetKey && preferredChunkId) scheduleSemanticIndexUpdate({ targetKey, preferredChunkId });
    };
    window.addEventListener('local-notion:semantic-refresh-request', handleSemanticRefreshRequest as EventListener);
    return () => window.removeEventListener('local-notion:semantic-refresh-request', handleSemanticRefreshRequest as EventListener);
  }, [api]);
    ...
    const drain = (async () => {
      ...
      try {
        while (queuedPageSaveRef.current) {
          ...
          setBlocks(blocksFromPage(saved));
          setHistoryPreview(null);
          setHistoryDiff(null);
          if (!queuedPageSaveRef.current) setDirty(false);
          // Refresh only side-panel datasets that the user has already opened.
          // Saving should never trigger an unrelated SMB read burst.
          // Checkpoint saves can change the history count.  Refresh only the
          // badges; full history remains lazy.
          void refreshPageSidebarCounts(next.pageId);
          const loadedTabs =
            pageInfoLoadedTabsRef.current[next.pageId] ??
            new Set<PageInfoTab>();
          for (const tab of Array.from(loadedTabs)) {
            loadedTabs.delete(tab);
            void ensurePageInfoTabData(next.pageId, tab);
          }
          try {
            window.dispatchEvent(
          ...
          void lightRefresh("保存しました").catch(() => undefined);
          setStatus(
            queuedPageSaveRef.current
              ? "保存を続けています…"
  ...
  async function trashPageById(id: string) {
    ...
    try {
      ...
      const mutationDetail: { pageId: string; action: "trashed"; workspaceFallbackKey?: string; workspaceHasFallback?: boolean } = { pageId: id, action: "trashed" };
      window.dispatchEvent(
        new CustomEvent("local-notion:page-tree-mutated", { detail: mutationDetail }),
      );
      ...
      await lightRefresh("ページをゴミ箱に移動しました");
    } catch (e: any) {
      setStatus(e.message);
    }
  }
  ...
  async function restoreTrashedPage(id: string) {
    if (!api) return;
    try {
      await api.restoreTrashedPage(id);
  ...
  async function deleteTrashedPage(id: string) {
    ...
    if (
      ...
      return;
    try {
      await api.deletePagePermanently(id);
      window.dispatchEvent(
        new CustomEvent("local-notion:page-tree-mutated", {
  ...
  async function restoreTrashedDatabase(id: string) {
    if (!api) return;
    try {
      await api.restoreTrashedDatabase(id);
  ...
  async function deleteTrashedDatabase(id: string) {
    ...
    if (
      ...
      return;
    try {
      await api.deleteTrashedDatabase(id);
  ...
  async function addJournalAttachments(): Promise<void> {
    ...
    try {
      ...
      setJournalAttachments(await api.listJournalAttachments(currentJournal.date));
  ...
  async function uploadJournalFileForBlockNote(file: File): Promise<string> {
    if (!api || !currentJournal) throw new Error('Journalが読み込まれていません。');
    const url = await api.uploadJournalAttachmentFile(currentJournal.date, file);
    setJournalAttachments(await api.listJournalAttachments(currentJournal.date));
  ...
  function showJournalConflict(
    ...
    setJournalDirty(true);
    setStatus("Journalの競合を検出しました。内容を確認して保存方法を選んでください。");
  }
  ...
  async function resolveJournalConflict(mode: "remote" | "local" | "merge") {
    ...
    try {
      const localTags = String(conflict.localMeta.tagsText || "")
        ...
        .map((value) => value.trim())
        .filter(Boolean);
      const mergedMarkdown = mode === "merge"
        ? `${conflict.remote.markdown || ""}${conflict.remote.markdown && conflict.local.markdown ? "\n\n---\n\n" : ""}${conflict.local.markdown || ""}`
        : conflict.local.markdown;
      const mergedBlocks = mode === "merge"
        ? journalEntryToBlocks({ ...conflict.local, markdown: mergedMarkdown, blocksuite: null })
      ...
      const payload: JournalEntry = {
        ...
        mood: mode === "merge" ? (conflict.localMeta.mood || conflict.remote.mood || "") : conflict.localMeta.mood,
        weather: mode === "merge" ? (conflict.localMeta.weather || conflict.remote.weather || "") : conflict.localMeta.weather,
        tags: mode === "merge" ? Array.from(new Set([...(conflict.remote.tags || []), ...localTags])) : localTags,
      ...
      setStatus(mode === "merge" ? "共有側と自分側の本文を統合して保存しました" : "自分の内容で共有Journalを更新しました");
    } catch (error: any) {
      setStatus(error?.message || "競合の解決に失敗しました。内容は保持されています。");
    } finally {
      ...
      try {
        ...
        if (isConflict && failedSnapshot) {
          ...
          try {
            ...
            showJournalConflict(
              {
                ...
                blocksuite: { version: 1, kind: "blocknote", blocks: failedSnapshot.blocks },
              },
              failedSnapshot.meta,
            ...
            setStatus(loadError?.message || "競合内容の取得に失敗しました。未保存状態を保持しています。");
          }
        } else {
  ...
  useEffect(() => {
    if (mainMode !== "journal" || !currentJournal || !journalDirty || journalConflict) return;
    const timer = window.setTimeout(() => {
  ...
  async function deleteCurrentJournal() {
    ...
    try {
      await api.deleteJournal(currentJournal.date);
  ...
  function openDatabaseInWorkspace(id: string, mode: "tabs" | "split" | "compare" = "tabs") {
    const dispatch = () => window.dispatchEvent(new CustomEvent("local-notion:workspace-open-item", {
      detail: { kind: "database", id, mode },
    }));
    if (current) {
      dispatch();
      setStatus(mode === "compare" ? "データベースを比較表示に追加しました" : mode === "split" ? "データベースを分割表示に追加しました" : "データベースを作業スペースのタブに開きました");
      return;
    }
  ...
  function openDatabaseRowInWorkspace(databaseId: string, rowId: string, mode: "tabs" | "split" | "compare" = "tabs") {
    const dispatch = () => window.dispatchEvent(new CustomEvent("local-notion:workspace-open-item", {
  ...
  async function openDatabase(id: string, viewId?: string, rowId?: string) {
    ...
    try {
      ...
      const db = viewId && fetched.views?.some((view) => view.id === viewId)
...
編集中の基準日時: ${error.payload.baseUpdatedAt}`
  ...
  function requestDatabaseSemanticRefresh(previous: WorkspaceDatabase | null | undefined, saved: WorkspaceDatabase): void {
    const previousById = new Map((previous?.rows || []).map((row) => [row.id, row]));
    // Row timestamps are stamped by every table mutation. Avoid JSON.stringify
    // over every row/cell and never discard changes after an arbitrary limit.
    const changed = (saved.rows || []).filter((row) => {
      const before = previousById.get(row.id);
      return !before || before.updatedAt !== row.updatedAt;
    });
    for (const row of changed) {
      window.dispatchEvent(new CustomEvent('local-notion:semantic-refresh-request', {
        detail: { targetKey: `database_row:${saved.id}:${row.id}`, preferredChunkId: `database_row:${saved.id}:${row.id}` },
      }));
    }
  }

  async function patchDatabaseRows(databaseId: string, patches: Array<{ rowId: string; cells: Record<string, any> }>) {
    if (!api || !patches.length) return;
    const queue = databaseRowPatchQueuesRef.current[databaseId] ?? { inFlight: false, pending: new Map<string, Record<string, any>>() };
    databaseRowPatchQueuesRef.current[databaseId] = queue;
    for (const patch of patches) queue.pending.set(patch.rowId, { ...(queue.pending.get(patch.rowId) || {}), ...patch.cells });
    if (queue.inFlight) return;
    queue.inFlight = true;
    try {
      while (queue.pending.size) {
        const batch = [...queue.pending.entries()].slice(0, 500).map(([rowId, cells]) => ({ rowId, cells }));
        for (const item of batch) queue.pending.delete(item.rowId);
        const latest = currentDbRef.current?.id === databaseId ? currentDbRef.current : databases.find((item) => item.id === databaseId) ?? null;
        const baseUpdatedAt = resolveDatabaseSaveBase(latest, latest);
        const result = await api.patchDatabaseRows(databaseId, { baseUpdatedAt, patches: batch });
        ...
        if (source?.id === databaseId) {
          ...
          const next = { ...source, updatedAt: result.updatedAt, updatedBy: result.updatedBy, rows: source.rows.map((row) => byId.get(row.id) ?? row) };
          lastPersistedDatabaseUpdatedAtRef.current[databaseId] = result.updatedAt;
          applyDatabaseSnapshot(next);
        }
        for (const row of result.rows) {
          window.dispatchEvent(new CustomEvent('local-notion:semantic-refresh-request', {
            detail: { targetKey: `database_row:${databaseId}:${row.id}`, preferredChunkId: `database_row:${databaseId}:${row.id}` },
          }));
        }
      }
  ...
  async function saveWorkspaceDatabase(updated: WorkspaceDatabase) {
    ...
    try {
      while (queue.pending) {
        ...
        queue.pending = null;
        const persistedBase = lastPersistedDatabaseUpdatedAtRef.current[next.id];
        const baseUpdatedAt = persistedBase || (databases.find((database) => database.id === next.id)?.updatedAt ?? next.baseUpdatedAt);
  ...
  async function changeWorkspaceDatabaseScope(databaseId: string, nextScope: WorkspaceScope) {
    if (!api) return;
    const database = databases.find((item) => item.id === databaseId);
    if (!database || workspaceScope(database) === nextScope) return;
    const warning = nextScope === "shared"
      ? "このデータベースをSharedに移動します。共有フォルダ上で他端末・他ユーザーから見える可能性があります。よろしいですか？"
      : "このデータベースをPrivateに移動します。共有フォルダからは見えなくなります。よろしいですか？";
    if (!confirm(warning)) return;
    try {
      const baseUpdatedAt = lastPersistedDatabaseUpdatedAtRef.current[databaseId] || database.updatedAt;
      const saved = await api.saveDatabase({ ...database, scope: nextScope, baseUpdatedAt });
      lastPersistedDatabaseUpdatedAtRef.current[saved.id] = saved.updatedAt;
      requestDatabaseSemanticRefresh(database, saved);
      applyDatabaseSnapshot(saved);
      await refreshWorkspace(
        nextScope === "private" ? "データベースをPrivateにしました" : "データベースをSharedにしました",
  ...
  async function addDatabaseRow() {
    if (!api || !currentDb) return;
    try {
      const saved = await api.addDatabaseRow(currentDb.id);
      requestDatabaseSemanticRefresh(currentDb, saved);
      setCurrentDb(saved);
      await reload("行を追加しました");
    } catch (e: any) {
      setStatus(e.message);
    }
  }
  ...
  function openPageContextMenu(event: React.MouseEvent, node: PageTreeNode) {
    event.preventDefault();
    event.stopPropagation();
    const position = getViewportSafeContextMenuPosition(
      event.clientX,
      event.clientY,
    );
    setContextMenu({ x: position.x, y: position.y, page: node });
  }
  ...
  useEffect(() => {
    ...
    if (!query) { setJournalSearchResults(null); return; }
    const timer = window.setTimeout(() => {
      api?.searchJournals(query, 30).then(setJournalSearchResults).catch(() => setJournalSearchResults(null));
    }, 180);
    return () => window.clearTimeout(timer);
  }, [api, journalSearch]);
  ...
  const filteredJournals = useMemo(() => {
    ...
    return journals.filter((j) => [j.date, j.title, j.previewSnippet, j.mood, j.weather, ...(j.tags || [])].join(" ").toLowerCase().includes(q)).slice(0, 20);
  ...
  return (
    ...
    >
          ...
          <div className="sidebar-scroll-body">
            ...
            <div className="sidebar-command-card">
              ...
              <div className="sidebar-action-grid compact-icon-grid">
                ...
                >
                  <span>⌁</span>
                  {inboxItems.some((item) => (item.attachments || []).some((file) => ["queued", "running", "cancelling"].includes(String(file.ocrQueue?.status || "")))) && <em>•</em>}
                </button>
                <button
                  onClick={openTasks}
                  disabled={!api}
            ...
            <section className="sidebar-tag-search" aria-label="タグで絞り込み">
              <div className="sidebar-tag-search-head">
                <span>タグで絞り込み</span>
                {searchTagFilters.length > 0 && (
                  <button type="button" onClick={() => setSearchTagFilters([])}>
                    クリア
                  </button>
                )}
              </div>
            ...
            {viewMode === "search" ? (
              ...
              <div className="page-list trash-list trash-list-v165">
                ...
                {trashedDatabases
                  .slice(0, Math.max(0, 12 - Math.min(12, trashedPages.length)))
                  .map((db) => (
                ...
                <button
                  className="danger"
                  onClick={emptyTrash}
                  disabled={trashedPages.length + trashedDatabases.length === 0}
                >
                  空にする
                </button>
              </div>
            ...
            <div className="database-sidebar-shell-v264">
              ...
              <DatabaseSidebarTree
                api={api}
                databases={databases}
                currentDatabaseId={workspaceActiveItem?.kind === "database" ? workspaceActiveItem.id : (currentDb?.id ?? null)}
                currentDatabaseRowId={workspaceActiveItem?.kind === "database" ? (workspaceActiveItem.rowId ?? null) : null}
                activePageId={workspaceActiveItem ? (workspaceActiveItem.kind === "page" ? workspaceActiveItem.id : null) : (current?.meta.id ?? null)}
                activePageParentId={workspaceActiveItem?.kind === "page" ? (workspaceActiveItem.parentId ?? (current?.meta.id === workspaceActiveItem.id ? current.meta.parentId ?? null : null)) : null}
      ...
      {journalAttachmentPreview && currentJournal && createPortal(
        <div
        ...
        >
          <section className="journal-attachment-preview-modal-v567" onMouseDown={(event) => event.stopPropagation()}>
            <header>
              <div>
                <span>{isJournalImageAttachment(journalAttachmentPreview.fileName) ? "IMAGE PREVIEW" : "PDF PREVIEW"}</span>
                <h2>{journalAttachmentPreview.fileName}</h2>
                <p>{formatJournalAttachmentSize(journalAttachmentPreview.size)} ・ {new Date(journalAttachmentPreview.createdAt).toLocaleString("ja-JP")}</p>
              </div>
              <div className="journal-attachment-preview-actions-v567">
                <a
                  href={api?.journalAttachmentDownloadUrl(currentJournal.date, journalAttachmentPreview.id) || "#"}
                  title="ダウンロード"
                >ダウンロード</a>
                <a
                  href={api?.journalAttachmentFileUrl(currentJournal.date, journalAttachmentPreview.id) || "#"}
                  target="_blank"
                  rel="noreferrer"
                  title="別ウィンドウで開く"
                >別ウィンドウ</a>
                <button type="button" onClick={() => setJournalAttachmentPreview(null)} aria-label="閉じる" title="閉じる">×</button>
              </div>
            </header>
            <div className="journal-attachment-preview-body-v567">
              {isJournalImageAttachment(journalAttachmentPreview.fileName) ? (
                <img src={api?.journalAttachmentFileUrl(currentJournal.date, journalAttachmentPreview.id) || ""} alt={journalAttachmentPreview.fileName} />
              ) : (
                <iframe src={api?.journalAttachmentFileUrl(currentJournal.date, journalAttachmentPreview.id) || ""} title={journalAttachmentPreview.fileName} />
              )}
            </div>
          </section>
        </div>, document.body
      ...
      {journalConflict && createPortal(
        <div className="journal-conflict-backdrop-v565" role="dialog" aria-modal="true" aria-label="Journalの競合を解決">
          <section className="journal-conflict-modal-v565">
            <header>
              <div>
                <span>共有フォルダの更新を検出</span>
                <h2>{journalConflict.local.date} のJournalをどう保存しますか？</h2>
                <p>別の端末またはウィンドウで同じJournalが更新されています。未保存内容は安全バックアップへ退避したうえで、保存方法を選べます。</p>
              </div>
              <button onClick={() => setJournalConflict(null)} aria-label="閉じる" title="後で確認する">×</button>
            </header>
            <div className="journal-conflict-columns-v565">
              <article>
                <div className="journal-conflict-column-head-v565 local"><b>この端末の未保存内容</b><small>{journalConflict.local.updatedAt}</small></div>
                <div className="journal-conflict-meta-v565">{journalConflict.localMeta.mood && <span>🙂 {journalConflict.localMeta.mood}</span>}{journalConflict.localMeta.weather && <span>☀ {journalConflict.localMeta.weather}</span>}</div>
                <pre>{journalConflict.local.markdown || "（本文は空です）"}</pre>
              </article>
              <article>
                <div className="journal-conflict-column-head-v565 remote"><b>共有フォルダの最新内容</b><small>{journalConflict.remote.updatedAt}</small></div>
                <div className="journal-conflict-meta-v565">{journalConflict.remote.mood && <span>🙂 {journalConflict.remote.mood}</span>}{journalConflict.remote.weather && <span>☀ {journalConflict.remote.weather}</span>}</div>
                <pre>{journalConflict.remote.markdown || "（本文は空です）"}</pre>
              </article>
            </div>
            <footer>
              <button className="journal-conflict-secondary-v565" disabled={journalConflictSaving} onClick={() => void resolveJournalConflict("remote")}>共有側を読み込む</button>
              <button className="journal-conflict-secondary-v565" disabled={journalConflictSaving} onClick={() => void resolveJournalConflict("merge")}>本文を統合して保存</button>
              <button className="journal-conflict-primary-v565" disabled={journalConflictSaving} onClick={() => void resolveJournalConflict("local")}>{journalConflictSaving ? "保存中…" : "自分の内容で上書き"}</button>
            </footer>
          </section>
        </div>, document.body
      )}
        ...
        {mainMode === "home" ? (
          ...
          <AttachmentManagerView
            ...
            onSendToOcr={async (attachment) => {
              ...
              setOcrCenterFocusKey(`${item.id}:${item.attachments?.[0]?.id || ''}`);
              setMainMode('ocr');
          ...
          <OcrCenterView
            ...
            attachmentUrl={(inboxId, attachmentId) => api?.inboxAttachmentFileUrl(inboxId, attachmentId) || "#"}
          />
        ) : mainMode === "inbox" ? (
          <InboxView
            items={inboxItems}
                ...
                <div className="affine-week-strip">
                  {journalWeekDays.map((day) => (
                    ...
                    >
                      <span>{journalWeekdayLabel(day)}</span>
                      <b>{journalDayNumber(day)}</b>
                      {journalDateSet.has(day) && <i className="journal-dot" />}
                    </button>
                  ))}
                </div>
              </div>
              <div className="affine-journal-layout">
                <div className="affine-journal-main">
                    ...
                    <div className="journal-meta-row affine-journal-meta-row">
                      ...
                      <label>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateJournalMetaDraft({ weather: e.target.value })
                          }
                          placeholder="晴れ / 雨"
                        />
                      </label>
                      <label>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateJournalMetaDraft({ tagsText: e.target.value })
                          }
                          placeholder="仕事, 家族"
                        />
                      </label>
                  ...
                  <BlockNotePageEditor
                    ...
                    aiClient={api}
                    attachmentApiBaseUrl={apiUrl}
                    aiPageTitle={formatJournalDisplayDate(currentJournal.date)}
                ...
                <aside className="affine-journal-side v89-journal-side">
                  <section className="v89-journal-panel">
                    ...
                    <div className="v89-journal-tabs">
                      <button
                        className={journalSideTab === "related" ? "active" : ""}
                      ...
                      <button
                        className={journalSideTab === "review" ? "active" : ""}
                      ...
                      <button
                        className={journalSideTab === "history" ? "active" : ""}
                      ...
                      <button
                        className={journalSideTab === "attachments" ? "active" : ""}
                  ...
                  {journalSideTab === "attachments" && (
                    <section className="v89-journal-panel journal-attachments-panel-v566">
                      <div className="affine-side-title">
                        <span title="添付ファイル" aria-label="添付ファイル">📎</span>
                      ...
                      {journalAttachments.length === 0 ? (
                        <div className="journal-attachments-empty-v566">
                          <b>添付はまだありません</b>
                          <span>PDF・画像・Office文書などを、この日の記録と一緒に保存できます。</span>
                          <button onClick={() => void addJournalAttachments()} disabled={journalAttachmentUploading}>ファイルを添付</button>
                        </div>
                      ) : (
                        <div className="journal-attachment-list-v566">
                          {journalAttachments.map((attachment) => {
                            const previewable = isJournalImageAttachment(attachment.fileName) || isJournalPdfAttachment(attachment.fileName);
                            const ocrItem = inboxItems.find((candidate) => candidate.ocrSource?.sourceType === "journal"
                              && candidate.ocrSource?.date === currentJournal.date
                              && candidate.ocrSource?.attachmentId === attachment.id);
                            const ocrAttachment = ocrItem?.attachments?.[0];
                            const ocrReady = ocrAttachment?.ocr?.status === "ready" || ocrAttachment?.pdfText?.status === "ready";
                            const ocrActive = ["queued", "running", "cancelling"].includes(String(ocrAttachment?.ocrQueue?.status || ""));
                            const ocrFailed = ["failed", "cancelled"].includes(String(ocrAttachment?.ocrQueue?.status || "")) || ocrAttachment?.ocr?.status === "failed" || ocrAttachment?.pdfText?.status === "failed";
                            return (
                              <article key={attachment.id} className="journal-attachment-card-v567">
                                <button
                                  type="button"
                                  className="journal-attachment-open-v567"
                                  onClick={() => previewable ? setJournalAttachmentPreview(attachment) : window.open(api?.journalAttachmentFileUrl(currentJournal.date, attachment.id) || "#", "_blank", "noopener,noreferrer")}
                                  title={previewable ? "プレビューを開く" : "ファイルを開く"}
                                >
                                  <span className="journal-attachment-icon-v567">{journalAttachmentIcon(attachment.fileName)}</span>
                                  <span className="journal-attachment-copy-v567">
                                    <b>{attachment.fileName}</b>
                                    <small>{formatJournalAttachmentSize(attachment.size)} ・ {new Date(attachment.createdAt).toLocaleDateString("ja-JP")}</small>
                                  </span>
                                  <span className="journal-attachment-kind-v567">{isJournalImageAttachment(attachment.fileName) ? "画像" : isJournalPdfAttachment(attachment.fileName) ? "PDF" : journalAttachmentExtension(attachment.fileName).toUpperCase() || "FILE"}</span>
                                </button>
                                <div className="journal-attachment-actions-v568">
                                  {(isJournalImageAttachment(attachment.fileName) || isJournalPdfAttachment(attachment.fileName)) ? (
                                    ocrReady ? <button type="button" className="journal-attachment-ocr-v568 is-ready" onClick={() => void sendJournalAttachmentToOcrCenter(attachment)} title="OCR結果を見る">結果</button>
                                    : ocrActive ? <button type="button" className="journal-attachment-ocr-v568 is-active" onClick={() => void sendJournalAttachmentToOcrCenter(attachment)} title="OCRセンターで状態を見る">処理中</button>
                                    : ocrFailed ? <button type="button" className="journal-attachment-ocr-v568 is-failed" onClick={() => void sendJournalAttachmentToOcrCenter(attachment)} title="OCRセンターで再実行">再試行</button>
                                    : <button type="button" className="journal-attachment-ocr-v568" onClick={() => void sendJournalAttachmentToOcrCenter(attachment)} title="OCRセンターへ送る">OCR</button>
                                  ) : null}
                                  <a
                                    className="journal-attachment-external-v567"
                                    href={api?.journalAttachmentFileUrl(currentJournal.date, attachment.id) || "#"}
                                    target="_blank"
                                    rel="noreferrer"
                                    title="別ウィンドウで開く"
          ...
          <>
            ...
            <DatabaseTable
              database={currentDb}
              editing={dbEditing}
              onChange={(updated) => autoSaveDatabase(updated, "データベース")}
          ...
          <>
            ...
            <div className="toolbar notion-toolbar">
              ...
              <span className="autosave-indicator">
                {editing ? (dirty ? "保存中…" : "保存済み") : "閲覧のみ"}
              </span>
              {workspaceActiveItem?.kind === "database" ? (
                <span className="workspace-toolbar-context-v523" title="データベース固有の共有設定・削除・編集操作は、下のデータベース見出しにあります">▦ データベースを編集中</span>
              ) : (
                <>
              <button
                className="secondary favorite-page-action icon-toolbar-button"
                onClick={() => toggleFavorite(current.meta.id)}
                title={current.meta.favorite ? "お気に入り解除" : "お気に入り"}
                aria-label={
                  current.meta.favorite ? "お気に入り解除" : "お気に入り"
                }
              >
              ...
              <button
                ...
                onClick={() => {
                  ...
                  void ensurePageInfoTabData(current.meta.id, "comments");
                }}
                title="コメント"
                aria-label="コメント"
              >
                ...
                {pageComments.filter((c) => !c.resolved).length ? (
                  <span>{pageComments.filter((c) => !c.resolved).length}</span>
                ) : null}
              </button>
              <button
                className="icon-toolbar-button"
              ...
              >
                <button
                  className={
                    pageScope(current.meta) === "private" ? "active" : ""
                  }
                ...
                <button
                  className={
                    pageScope(current.meta) === "shared" ? "active" : ""
                  }
              ...
              onSaveDatabase={(database) => void saveWorkspaceDatabase(database)}
              onChangeDatabaseScope={(databaseId, scope) => void changeWorkspaceDatabaseScope(databaseId, scope)}
              onDeleteDatabase={(databaseId) => void deleteDatabaseById(databaseId)}
              onActiveItemChange={(item) => {
                ...
                setWorkspaceActiveItem({ ...item, parentId: item.kind === "page" && current?.meta.id === item.id ? (current.meta.parentId ?? null) : null });
                if (item.kind !== "page" || !api || current?.meta.id === item.id) return;
                void api.getPage(item.id).then((page) => {
                  if (workspaceSelectionRequestRef.current !== requestId) return;
                  setWorkspaceActiveItem((previous) => previous?.kind === "page" && previous.id === item.id
                    ? { ...previous, parentId: page.meta.parentId ?? null }
                    : previous);
                }).catch((error) => console.warn("WORKSPACE_ACTIVE_PAGE_PARENT_RESOLVE_FAILED", item.id, error));
              }}
            />
            <Breadcrumbs
            ...
            <section className="notion-page-hero">
              ...
              <div className="notion-title-stack">
                ...
                />
                <span className={`page-scope-badge ${pageScope(current.meta)}`}>
            ...
            <section className="notion-collapsible-properties">
              ...
              {propertiesOpen && (
                <PageInfoPanel
                  ...
                  onOpenPage={openPage}
                  onOpenDatabase={(databaseId) => openDatabaseInWorkspace(databaseId)}
            ...
            {pageReadOnlyReason && !editing && (
              <div className="readonly-lock-note">
                ...
                <button className="secondary" onClick={() => void startEdit()}>
                  編集を再試行
                </button>
              </div>
            )}
            <div className="page-writing-layout-v49 page-writing-layout-v121">
              ...
              <div className="page-right-utility-v105">
                <WorkspaceRelatedPanel
                  api={api}
                  pageId={current.meta.id}
      ...
      <button
        type="button"
        className={`workspace-ai-fab notion-ai-orb-v325 notion-ai-glyph-v463${workspaceAiGeneration.busy ? " is-generating-v463" : ""}`}
        title={
          workspaceAiGeneration.busy
            ? "AIが回答を生成中。押すと会話へ戻ります"
            : "AIアシスタント"
        }
        aria-label={
          workspaceAiGeneration.busy
            ? "AIが回答を生成中。AIアシスタントを開く"
            : "AIアシスタントを開く"
        }
        aria-busy={workspaceAiGeneration.busy}
        onClick={() => {
          setWorkspaceAiDrawerMode("chat");
          setWorkspaceAiInitialQuery("");
          setWorkspaceAiSearchOpen(true);
      ...
      >
        ...
        <span
          className="notion-ai-orbit-v464 notion-ai-orbit-one-v464"
          aria-hidden="true"
        >
          <i />
        </span>
        <span
          className="notion-ai-orbit-v464 notion-ai-orbit-two-v464"
          aria-hidden="true"
        >
          <i />
        </span>
        <span
          className="notion-ai-orb-core notion-ai-core-v464"
          aria-hidden="true"
        >
          <svg viewBox="0 0 48 48" focusable="false">
            ...
            <path
              className="orb-main"
              d="M24 6.3c3.2 8.15 7.5 12.4 15.7 15.7-8.2 3.25-12.5 7.55-15.7 15.7-3.22-8.15-7.52-12.45-15.7-15.7C16.48 18.7 20.78 14.45 24 6.3Z"
            />
            <path
              className="orb-mini orb-mini-a"
              d="M37.7 8.1c1.25 3.18 2.9 4.8 6.08 6.08-3.18 1.25-4.83 2.9-6.08 6.08-1.28-3.18-2.93-4.83-6.1-6.08 3.17-1.28 4.82-2.9 6.1-6.08Z"
            />
            <path
              className="orb-mini orb-mini-b"
              d="M9.4 29.9c.96 2.48 2.24 3.75 4.72 4.72-2.48.96-3.76 2.25-4.72 4.73-.98-2.48-2.26-3.77-4.74-4.73 2.48-.97 3.76-2.24 4.74-4.72Z"
            />
          </svg>
        </span>
        <span
          className="notion-ai-orb-generation-v463 notion-ai-generation-v464"
          aria-hidden="true"
        />
      </button>
      <div
        className={`workspace-ai-drawer-backdrop ${workspaceAiSearchOpen ? "" : "is-hidden-v461"}`}
        onMouseDown={() => setWorkspaceAiSearchOpen(false)}
      >
        <aside
          className={`workspace-ai-drawer ${workspaceAiDrawerMode === "chat" ? "workspace-ai-drawer-chat-v353" : ""}`}
          onMouseDown={(event) => event.stopPropagation()}
        >
          {workspaceAiDrawerMode === "chat" ? (
            <WorkspaceAiChatPanel
              api={api}
              autoFocus={workspaceAiSearchOpen}
              ...
              onOpenPage={(id) => {
                setWorkspaceAiSearchOpen(false);
                openPage(id);
              }}
              onOpenDatabase={(id) => {
                setWorkspaceAiSearchOpen(false);
              ...
              onOpenDatabaseRow={(databaseId, rowId) => {
                setWorkspaceAiSearchOpen(false);
                openDatabaseRow(databaseId, rowId);
              }}
              onOpenJournal={(date) => {
                setWorkspaceAiSearchOpen(false);
                openJournal(date);
