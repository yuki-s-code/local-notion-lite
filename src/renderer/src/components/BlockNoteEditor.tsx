  ...
  },
  {
    render: (props) => {
      ...
      const open = (event: ReactMouseEvent) => {
        ...
        if (normalizedUrl)
          void window.localNotion.openExternalHttpUrl(normalizedUrl);
      };
      ...
      return (
        ...
        >
          <div className="external-link-card-mark" aria-hidden="true">
            ↗
          </div>
          <div className="external-link-card-main">
            {editable ? (
              <>
                <input
                  ...
                  onBlur={(event) => {
                    ...
                    const nextSiteName =
                      siteName || externalLinkCardLabel(nextUrl);
                    update({
                      url: nextUrl,
                      title: nextTitle,
                ...
                <input
                  ...
                  onKeyDown={stopEditorEvent}
                  onBlur={(event) =>
                    update({ title: event.currentTarget.value.trim() })
                  }
                />
                <textarea
                  ...
                  onKeyDown={stopEditorEvent}
                  onBlur={(event) =>
                    update({ description: event.currentTarget.value.trim() })
                  }
                />
              </>
            ) : (
              <button
                ...
                title={
                  normalizedUrl ||
                  "有効な http:// または https:// URL を設定してください"
                }
              >
                <strong>
                  {title || externalLinkCardLabel(url) || "外部リンク"}
                </strong>
            ...
            <div className="external-link-card-footer">
              <span>
                {siteName || externalLinkCardLabel(url) || "外部サイト"}
              </span>
...
function insertExternalLinkCardItem(editor: any) {
  return {
    title: "外部リンクカード",
    subtext:
      "http(s) URLを見やすいカードとして追加し、既定のブラウザで開きます",
    aliases: [
      "link card",
      "url card",
      "external",
...
function SelectionAiToolbarButton({
  captureSelection,
  onActivate,
}: {
  captureSelection: () => CapturedAiSelection | null;
  onActivate: (selection: CapturedAiSelection | null) => void;
}) {
  const Components = useComponentsContext()!;
  const capturedSelectionRef = useRef<CapturedAiSelection | null>(null);

  return (
    <span
      className="blocknote-formatting-ai-action"
      onMouseDownCapture={(event) => {
        // The native formatting toolbar normally moves focus before its click handler
        // runs. Capture the BlockNote selection during pointer-down so the selection
        // and its viewport position survive opening the local AI panel.
        capturedSelectionRef.current = captureSelection();
        event.preventDefault();
        event.stopPropagation();
      }}
      onClick={(event) => {
        event.preventDefault();
        event.stopPropagation();
        onActivate(capturedSelectionRef.current);
      }}
    >
      <Components.FormattingToolbar.Button mainTooltip="選択範囲をAI編集">
        ✦ AI編集
...
function CustomFormattingToolbar({
  ...
  onOpenEditorAi: (selection: CapturedAiSelection | null) => void;
  captureSelection: () => CapturedAiSelection | null;
}) {
  ...
  return (
    <FormattingToolbar>
      ...
      <UnnestBlockButton />
      <CreateLinkButton />
      <SelectionAiToolbarButton
        captureSelection={captureSelection}
        onActivate={onOpenEditorAi}
      />
    </FormattingToolbar>
  );
}
...
type AiOverlayPlacement = "above" | "below";
...
type CapturedAiSelection = {
  sourceText: string;
  tiptapRange: TiptapSelectionRange | null;
  anchor: AiSelectionAnchor;
};

type EditorAiSession =
  | {
      id: number;
      kind: "selection";
      sourceText: string;
...
function clampOverlayLeft(left: number, width: number) {
  return Math.max(12, Math.min(window.innerWidth - width - 12, left));
}
...
function getBlockNoteFloatingToolbarRect() {
...
function isSameAiPanelAnchor(
  current: AiPanelAnchor | null,
  next: AiPanelAnchor,
) {
  return Boolean(
    current &&
...
function placeSelectionAiPanel(anchor: AiSelectionAnchor): AiPanelAnchor {
  const width = Math.min(440, window.innerWidth - 24);
  const preferredHeight = Math.min(
    540,
    Math.max(320, window.innerHeight * 0.64),
  );
  const left = clampOverlayLeft(
    anchor.rect.left + anchor.rect.width / 2 - width / 2,
    width,
  );
  const toolbar = getBlockNoteFloatingToolbarRect();
  const candidates: Array<{ top: number; placement: AiOverlayPlacement }> = [
    { top: anchor.rect.bottom + 14, placement: "below" },
    { top: anchor.rect.top - preferredHeight - 14, placement: "above" },
  ];
  const candidate = candidates.find(({ top }) => {
    const box = {
      left,
      top,
      right: left + width,
    ...
    return (
      top >= 12 &&
      top + preferredHeight <= window.innerHeight - 12 &&
      (!toolbar || !intersectsRect(toolbar, box))
    );
  }) ??
    candidates.find(
      ({ top }) =>
        top >= 12 && top + preferredHeight <= window.innerHeight - 12,
    ) ?? {
      top: Math.max(
        12,
        Math.min(
          window.innerHeight - preferredHeight - 12,
          anchor.rect.bottom + 14,
        ),
      ),
      placement: "below" as const,
    };
  return { left, top: candidate.top, placement: candidate.placement };
}
...
function stripLocalResourceIconPrefix(text: string): string {
  return stripDatabaseRowIconPrefix(
    stripDatabaseIconPrefix(stripPageIconPrefix(text)),
  );
}
...
function syncInlineResourceTitles(
  ...
  for (const part of normalized) {
    if (part?.type === "link") {
      ...
      const dbrow = localDatabaseRowFromHref(href);
      if (dbrow) {
        const fallback = stripDatabaseRowIconPrefix(
          textFromInlineContent(part.content),
        );
      ...
      const databaseId = localDatabaseIdFromHref(href);
      if (databaseId) {
        const fallback = stripDatabaseIconPrefix(
          textFromInlineContent(part.content),
        );
        next = stripTrailingLocalResourceIcons(next);
        next.push(
...
function resolveStableAttachmentUrl(
  raw: string,
  attachmentApiBaseUrl: string,
): string {
  ...
  if (raw.startsWith(STABLE_ATTACHMENT_SCHEME)) {
    try {
      ...
      if (parts[0] === "dbrow" && parts.length >= 5) {
        ...
        return new URL(
          `/databases/${encodeURIComponent(databaseId)}/rows/${encodeURIComponent(rowId)}/attachments/${encodeURIComponent(attachmentId)}/name/${encodeURIComponent(fileName)}`,
          base,
        ).toString();
      }
      ...
      return new URL(
        `/pages/${encodeURIComponent(pageId)}/attachments/${encodeURIComponent(attachmentId)}/name/${encodeURIComponent(fileName)}`,
        base,
      ).toString();
    } catch {
      return raw;
  ...
  try {
    ...
    const isPageAttachment =
      /^\/pages\/[^/]+\/attachments\/[^/]+\/(?:file|download)(?:\/|$)/.test(
        parsed.pathname,
      ) || /^\/pages\/[^/]+\/attachments\/[^/]+\/name\//.test(parsed.pathname);
    const isRowAttachment =
      /^\/databases\/[^/]+\/rows\/[^/]+\/attachments\/[^/]+\/(?:file|download)(?:\/|$)/.test(
        parsed.pathname,
      ) ||
      /^\/databases\/[^/]+\/rows\/[^/]+\/attachments\/[^/]+\/name\//.test(
        parsed.pathname,
      );
...
function mapAttachmentUrls(value: any, mapper: (raw: string) => string): any {
  ...
  if (Array.isArray(value))
    return value.map((item) => mapAttachmentUrls(item, mapper));
  if (typeof value !== "object") return value;
  const next: Record<string, any> = {};
  for (const [key, item] of Object.entries(value))
    next[key] = mapAttachmentUrls(item, mapper);
  return next;
}

function rebaseStoredAttachmentUrls(
  value: any,
  attachmentApiBaseUrl: string,
): any {
  return mapAttachmentUrls(value, (raw) =>
    resolveStableAttachmentUrl(raw, attachmentApiBaseUrl),
  );
}
...
function normalizeLegacyLocalLinks(
  ...
  return safeBlocks.map((block: any) =>
    normalizeBlockForBlockNote(
      rebaseStoredAttachmentUrls(block, attachmentApiBaseUrl),
    ),
  );
}
...
function readableLinkedDatabaseViewEmbed(
...
export function BlockNotePageEditor(props: Props) {
  ...
  if (!isReady) {
    return (
      <div
        className="blocknote-shell blocknote-deferred-shell"
        aria-busy="true"
      >
...
function BlockNotePageEditorRuntime({
  ...
  const [aiStatus, setAiStatus] = useState<"idle" | "working" | "error">(
    "idle",
  );
  const [aiError, setAiError] = useState("");
  const [aiDraft, setAiDraft] = useState("");
  const [aiPanelAnchor, setAiPanelAnchor] = useState<AiPanelAnchor | null>(
    null,
  );
  // The session is the single source of truth for both selection and document AI.
  // Native browser selection is allowed to disappear after the toolbar/composer
  // receives focus; generation and replacement always use this frozen snapshot.
  const editorAiSessionRef = useRef<EditorAiSession | null>(null);
  const editorAiSessionSequenceRef = useRef(0);
  const aiDraftSessionIdRef = useRef<number | null>(null);
  const [pageQuery, setPageQuery] = useState("");
  const [dbQuery, setDbQuery] = useState("");
  // Update glossary hints only after the user pauses. This avoids a React render per keystroke.
  const [glossaryText, setGlossaryText] = useState(() =>
    docToPlainText(initialContent).slice(0, 12_000),
  );
  const glossaryTimerRef = useRef<number | null>(null);
  ...
  useEffect(
    () => () => {
      if (glossaryTimerRef.current !== null)
        window.clearTimeout(glossaryTimerRef.current);
    },
    [],
  );

  function scheduleGlossaryText(doc: BlockNoteDoc) {
    if (glossaryTimerRef.current !== null)
      window.clearTimeout(glossaryTimerRef.current);
    glossaryTimerRef.current = window.setTimeout(() => {
      glossaryTimerRef.current = null;
      setGlossaryText(docToPlainText(doc).slice(0, 12_000));
    }, 650);
  }
  ...
  function insertPageLink(page: PageWithLock) {
    ...
    setPageQuery("");
    queueMicrotask(() =>
      emitPersistedChange(anyEditor.document as BlockNoteDoc),
    );
  }
  ...
  function insertLinkedDatabaseView(
    database: WorkspaceDatabase,
    viewId: string,
  ) {
  ...
  const captureEditorAiSelection = (): CapturedAiSelection | null => {
    ...
    if (!sourceText || !range || !selectionBelongsToEditor) return null;

    const tiptap =
      (editor as any)._tiptapEditor || (editor as any).tiptapEditor;
    const selectionState = tiptap?.state?.selection;
    const tiptapRange =
      selectionState &&
      Number.isFinite(selectionState.from) &&
      Number.isFinite(selectionState.to)
        ? { from: selectionState.from, to: selectionState.to }
        : null;
  ...
  const createEditorAiSession = (
    selection: CapturedAiSelection | null,
  ): EditorAiSession => {
    const id = ++editorAiSessionSequenceRef.current;
    if (selection) {
      return {
        id,
        kind: "selection",
        sourceText: selection.sourceText,
        tiptapRange: selection.tiptapRange,
    ...
    return {
      id,
      kind: "document",
      sourceText: docToPlainText(
        (editor as any).document as BlockNoteDoc,
      ).trim(),
    };
  };

  const getAiSessionAnchor = (): AiSelectionAnchor | null => {
    const session = editorAiSessionRef.current;
    if (!session || session.kind !== "selection") return null;
    const tiptap =
      (editor as any)._tiptapEditor || (editor as any).tiptapEditor;
    const view = tiptap?.view;
    if (session.tiptapRange && view?.coordsAtPos) {
      try {
        const start = view.coordsAtPos(session.tiptapRange.from);
        const end = view.coordsAtPos(session.tiptapRange.to);
        ...
        // The original DOM rectangle remains a safe display fallback after a re-render.
      }
    }
  ...
  useEffect(() => {
    if (!editing || !aiClient || !aiPanelOpen || !aiPanelAnchor) return;
    let frame = 0;
    const reposition = () => {
      cancelAnimationFrame(frame);
      frame = requestAnimationFrame(() => {
        const anchor = getAiSessionAnchor();
        if (anchor) {
          const nextPanelAnchor = placeSelectionAiPanel(anchor);
          setAiPanelAnchor((current) =>
            isSameAiPanelAnchor(current, nextPanelAnchor)
              ? current
              : nextPanelAnchor,
          );
        }
  ...
  }, [editing, aiClient, aiPanelOpen, aiPanelAnchor, editor]);

  const clearEditorAiSession = () => {
    editorAiSessionRef.current = null;
  ...
  const closeEditorAi = () => {
    ...
    setAiError("");
    clearEditorAiSession();
  };

  const openEditorAi = (capturedSelection?: CapturedAiSelection | null) => {
    const selection = capturedSelection ?? captureEditorAiSelection();
    const session = createEditorAiSession(selection);
    editorAiSessionRef.current = session;
    aiDraftSessionIdRef.current = null;
    setAiError("");
    setAiDraft("");
    setAiStatus("idle");
    setAiPanelAnchor(
      session.kind === "selection"
        ? placeSelectionAiPanel(session.anchor)
        : null,
    );
    setAiPanelOpen(true);
  };

  const runEditorAi = async (
    preset: "summary" | "rewrite" | "bullets" | "todo" | "custom",
  ) => {
    if (!aiClient) {
      setAiError(
        "AI接続が準備できていません。アプリの接続状態を確認してください。",
      );
      setAiStatus("error");
      return;
    }
    ...
    if (!session) {
      setAiError(
        "AI編集セッションが見つかりません。もう一度ページまたは選択範囲を開いてください。",
      );
      setAiStatus("error");
      return;
    }
    const currentText = session.sourceText;
    if (!currentText) {
    ...
    const action =
      preset === "summary"
        ? "次の文章を、内容を変えずに日本語で簡潔に要約してください。"
        : preset === "rewrite"
          ? "次の文章を、意味を変えずに読みやすく自然な業務文へ書き換えてください。"
          : preset === "bullets"
            ? "次の文章を、内容を落とさずに見出し付きの箇条書きへ整理してください。"
            : preset === "todo"
              ? "次の文章から、実行すべきTODO・確認事項・期限を箇条書きで抽出してください。"
              : aiInstruction.trim();
    if (!action) {
    ...
    try {
      ...
      if (!result?.ok || !answer)
        throw new Error(
          String(result?.message || "AIの編集結果を取得できませんでした。"),
        );
      // Ignore a late response after this panel was closed or a new session began.
      if (editorAiSessionRef.current?.id !== session.id) return;
      aiDraftSessionIdRef.current = session.id;
      setAiDraft(answer);
      setAiStatus("idle");
    } catch (error: any) {
      if (editorAiSessionRef.current?.id !== session.id) return;
  ...
  const applyAiDraft = (mode: "replace" | "append") => {
    ...
    const session = editorAiSessionRef.current;
    if (!text || !session || aiDraftSessionIdRef.current !== session.id) return;
    const anyEditor = editor as any;
    try {
      if (mode === "replace" && session.kind === "selection") {
        ...
        if (
          ...
          throw new Error(
            "選択範囲の位置を保持できませんでした。もう一度範囲を選択してAI編集を実行してください。",
          );
        }
        // Do not fall back to the current browser selection: it may now point at the
        // AI composer or a different block. Only the frozen session range is valid.
        const currentSelectionText = String(
          tiptap.state?.doc?.textBetween?.(
            session.tiptapRange.from,
            session.tiptapRange.to,
            "\n",
            "\n",
          ) || "",
        ).trim();
        const normalizeSessionText = (value: string) =>
          value.replace(/\s+/g, " ").trim();
        if (
          normalizeSessionText(currentSelectionText) !==
          normalizeSessionText(session.sourceText)
        ) {
          throw new Error(
            "選択範囲の内容が変更されました。意図しない場所を置換しないため、もう一度範囲を選択してAI編集を実行してください。",
          );
        }
        tiptap.commands.setTextSelection(session.tiptapRange);
        tiptap.commands.insertContent(text);
      } else {
        const blocks = text
          .split(/\n{2,}/)
          .map((part: string) => blockWithText(part.trim()))
          .filter((block: any) => textFromInlineContent(block.content));
        const cursor = anyEditor.getTextCursorPosition?.();
        if (cursor?.block && typeof anyEditor.insertBlocks === "function")
          anyEditor.insertBlocks(blocks, cursor.block, "after");
        else if (typeof anyEditor.insertBlocks === "function")
          anyEditor.insertBlocks(blocks);
        else
          anyEditor.insertInlineContent?.([
            { type: "text", text: `\n${text}`, styles: {} },
          ]);
  ...
  const getSlashMenuItems = useMemo(() => {
    return async (query: string) => {
      try {
        ...
        const linkCardItems = [
          insertExternalLinkCardItem(editor as any),
        ] as any;
  ...
  return (
    ...
    >
      ...
      {editing && aiClient && (
        <section className="blocknote-ai-editor-tools" aria-label="AI編集">
          {!aiPanelOpen && (
            <button
              type="button"
              className="blocknote-ai-fab"
              onMouseDown={(event) => event.preventDefault()}
              onClick={() => openEditorAi(null)}
              aria-label="AI編集を開く"
            >
              <span>✦</span>
              <strong>AI編集</strong>
            ...
            <aside className="blocknote-ai-dock" aria-label="AI編集">
              <div className="blocknote-ai-dock-head">
                <div className="blocknote-ai-dock-title">
                  <span className="blocknote-ai-spark" aria-hidden="true">
                    ✦
                  </span>
                  <div>
                    <strong>AI編集</strong>
                    <small>現在のページを対象</small>
                  </div>
                </div>
                <button
                  type="button"
                  className="blocknote-ai-icon-button"
                  onClick={closeEditorAi}
                  aria-label="AI編集を閉じる"
                >
                  ×
                </button>
              ...
              <div className="blocknote-ai-quick-actions">
                <button
                  ...
                  onMouseDown={(event) => event.preventDefault()}
                  onClick={() => void runEditorAi("rewrite")}
                >
                  <span>✎</span>書き換え
                </button>
                <button
                  ...
                  disabled={aiStatus === "working"}
                  onMouseDown={(event) => event.preventDefault()}
                  onClick={() => void runEditorAi("summary")}
                >
                  <span>≡</span>要約
                </button>
                <button
                  ...
                  disabled={aiStatus === "working"}
                  onMouseDown={(event) => event.preventDefault()}
                  onClick={() => void runEditorAi("bullets")}
                >
                  <span>•</span>箇条書き
                </button>
                <button
              ...
              <label className="blocknote-ai-composer">
                ...
                <textarea
                  ...
                  placeholder="例：保護者向けに、やわらかく分かりやすい表現にする"
                  rows={3}
                  onKeyDown={(event) => {
                    if (
              ...
              <div className="blocknote-ai-dock-footer">
                ...
                <button
                  type="button"
                  className="blocknote-ai-generate"
                  disabled={aiStatus === "working"}
                  onClick={() => void runEditorAi("custom")}
                >
                  {aiStatus === "working" ? "生成中…" : "生成"}
                </button>
              </div>
              {aiError && (
                <div className="blocknote-ai-error" role="alert">
                  {aiError}
                </div>
              )}
              {aiDraft && (
                <div className="blocknote-ai-result">
                  <div className="blocknote-ai-result-head">
                    <strong>プレビュー</strong>
          ...
          {aiPanelOpen &&
            ...
            createPortal(
              <aside
                className={`blocknote-ai-selection-panel ${aiPanelAnchor.placement}`}
                style={{ left: aiPanelAnchor.left, top: aiPanelAnchor.top }}
                aria-label="選択範囲のAI編集"
              ...
              >
                <div className="blocknote-ai-dock-head">
                  <div className="blocknote-ai-dock-title">
                    ...
                    <div>
                      <strong>選択範囲をAI編集</strong>
                      <small>{`${Math.min(editorAiSessionRef.current?.kind === "selection" ? editorAiSessionRef.current.sourceText.length : 0, 8000)}文字`}</small>
                    </div>
                  </div>
                  <button
                    type="button"
      ...
      <BlockNoteView
        ...
        onChange={emitChange}
      >
        {/* BlockNote's selection toolbar is replaced through its official controller so
            the AI action is part of the same floating menu, not a competing overlay. */}
        {editing && editorReady && (
          <>
            <FormattingToolbarController
              formattingToolbar={() => (
                <CustomFormattingToolbar
                  captureSelection={captureEditorAiSelection}
                  onOpenEditorAi={(selectionAnchor) =>
                    openEditorAi(selectionAnchor)
                  }
                />
              )}
            />
