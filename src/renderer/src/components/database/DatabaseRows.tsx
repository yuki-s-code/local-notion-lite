...
const FastCell = React.memo(function FastCell({ database, allDatabases, api, pages, journals, row, rowId, prop, value, editing, onUpdate, onOpenRelationTarget, pinnedLeft }: FastCellProps) {
  ...
  const [relationAnchorRect, setRelationAnchorRect] = useState<DOMRect | null>(null);
  ...
  useEffect(() => {
    ...
    setRelationQuery('');
    setRelationAnchorRect(null);
  }, [rowId, prop.id]);

  const relationCandidates = useMemo(() => {
    if (prop.type !== 'relation' || !relationOpen) return [];
    return getRelationCandidates(prop, database, allDatabases, pages, journals, rowId, {
      limit: prop.isSubItemRelation ? 5000 : 1000,
    });
  }, [prop, database, allDatabases, pages, journals, rowId, relationOpen]);
  ...
  const flushPending = (immediate = true) => {
    ...
    onUpdate(pending.rowId, pending.propId, pending.value, immediate);
  };

  useEffect(() => () => {
    // Virtual scrolling can unmount a row while a debounced text edit is pending.
    // Flush the latest value instead of clearing the timer, otherwise typing then
    // scrolling can lose the edit.
    flushPending(true);
  }, []);

  useEffect(() => {
    if (!relationOpen) return;
    const updatePosition = () => setRelationAnchorRect(anchorButtonRef.current?.getBoundingClientRect() ?? null);
    updatePosition();
  ...
  function schedule(value: any, immediate = false) {
    ...
    pendingRef.current = { rowId, propId: prop.id, value };
    if (immediate) {
      flushPending(true);
      return;
    }
  ...
  function toggleRelationPopover(event: React.MouseEvent<HTMLButtonElement>) {
    event.stopPropagation();
    const nextOpen = !relationOpen;
    setRelationOpen(nextOpen);
    setRelationAnchorRect(nextOpen ? event.currentTarget.getBoundingClientRect() : null);
  }
  ...
  if (prop.type === 'relation') {
    ...
    const popover = relationOpen && relationAnchorRect ? createPortal(
      <div
        className="relation-popover-v96 relation-popover-v101 relation-popover-v124 relation-popover-portal-v728"
        style={{
          left: Math.min(relationAnchorRect.left, window.innerWidth - 480),
          top: Math.min(relationAnchorRect.bottom + 6, window.innerHeight - 440),
        }}
      ...
      >
        ...
        <input className="relation-search-v124" placeholder="Relation候補を検索" value={relationQuery} autoFocus onChange={e => setRelationQuery(e.target.value)} />
        ...
        <button type="button" className="relation-close-v717" onClick={(event) => { event.stopPropagation(); setRelationOpen(false); }}>閉じる</button>
      </div>,
      document.body,
    ) : null;
    return <td {...tdProps}><div className={`relation-cell-v96 relation-cell-v124 ${relationOpen ? 'is-open' : ''}`}>
      <button ref={anchorButtonRef} type="button" onClick={toggleRelationPopover}>↔ {selected.length ? `${selected.length}件` : (prop.isSubItemRelation ? '親アイテム' : 'Relation')}</button>
      {popover}
    </div></td>;
  }
  if (!editing) {
