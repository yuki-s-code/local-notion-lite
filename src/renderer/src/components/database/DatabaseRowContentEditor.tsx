...
export function DatabaseRowContentEditor({ api, database, rowId, title, editing, pages, allDatabases, onOpenDatabase, onOpenDatabaseRow, onOpenPage, onChildPageCreated }: Props) {
  ...
  const latestBlocksRef = useRef<BlockNoteDoc>(EMPTY_DOC);
  const postSaveTimerRef = useRef<number | null>(null);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (postSaveTimerRef.current) window.clearTimeout(postSaveTimerRef.current);
      pendingBlocksRef.current = null;
    };
  ...
  async function save(nextBlocks: BlockNoteDoc) {
    ...
    try {
      ...
      if (!mountedRef.current) return;
      // Do not feed the saved payload back into the live editor while the user is typing.
      // Updating `content` and immediately refreshing links used to cause a large parent/tree
      // refresh on every autosave, which made the BlockNote surface temporarily unresponsive.
      baseUpdatedAtRef.current = saved.updatedAt;
      lastSavedSignatureRef.current = signature;
      setStatus(pendingBlocksRef.current ? 'dirty' : 'saved');
      setMessage('');

      // Link/search/graph invalidation is coalesced and runs after the editor has been idle.
      // The database-row content itself is already durable at this point.
      if (postSaveTimerRef.current) window.clearTimeout(postSaveTimerRef.current);
      postSaveTimerRef.current = window.setTimeout(() => {
        ...
        try {
          ...
          workspaceMutationCoordinator.publish({
            kind: 'database-row-content-saved',
            databaseIds: [database.id],
            databaseRowIds: [`${database.id}:${rowId}`],
            cacheScopes: ['workspace', 'graph', 'search', 'tasks', 'attachments', 'notifications'],
          }, [workspaceMutationCoordinator.databaseRowTarget(database.id, rowId)]);
        } catch {}
      }, 1200);
    } catch (error) {
      if (!mountedRef.current) return;
  ...
  async function createChildPage(options: { title?: string; open?: boolean } = {}) {
    ...
    try {
      ...
      try {
        ...
        workspaceMutationCoordinator.publish({
          kind: 'database-row-child-page-created',
          pageIds: [created.meta.id],
          databaseIds: [database.id],
          databaseRowIds: [`${database.id}:${rowId}`],
          cacheScopes: ['workspace', 'graph', 'search', 'tasks', 'notifications'],
        }, [
          workspaceMutationCoordinator.pageTarget(created.meta.id),
          workspaceMutationCoordinator.databaseRowTarget(database.id, rowId),
        ]);
        // Creating a child changes both semantic sources: the new page and
        // the parent DB-row body that now contains its generated link.
      } catch {}
      onChildPageCreated?.();
  ...
  async function deleteChildPage(page: PageWithLock) {
    ...
    try {
      ...
      try {
        ...
        workspaceMutationCoordinator.publish({
          kind: 'database-row-child-page-removed',
          pageIds: [page.id],
          databaseIds: [database.id],
          databaseRowIds: [`${database.id}:${rowId}`],
          cacheScopes: ['workspace', 'graph', 'search', 'tasks', 'notifications'],
        }, [
          workspaceMutationCoordinator.pageTarget(page.id),
          workspaceMutationCoordinator.databaseRowTarget(database.id, rowId),
        ]);
        // The deleted source is replaced with no chunks; the parent row is
        // re-embedded without the removed generated child-page link.
  ...
  function handleChange(nextBlocks: BlockNoteDoc) {
    ...
    setStatus('dirty');
    if (saveTimerRef.current) window.clearTimeout(saveTimerRef.current);
    // Save only after a real typing pause. The request is still serialized by save().
    saveTimerRef.current = window.setTimeout(() => save(latestBlocksRef.current), 1800);
  }
      ...
      <div className="db-row-content-body-v255">
        <BlockNotePageEditor
          ...
          onUploadFile={async (file) => {
            ...
            void refreshAttachments();
            workspaceMutationCoordinator.publish({
              kind: 'database-row-attachment-added',
              databaseIds: [database.id],
              databaseRowIds: [`${database.id}:${rowId}`],
