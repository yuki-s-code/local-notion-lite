  ...
  return (
    ...
    >
      ...
      <span className="related-item-main-v285">
        ...
        <small>{clampText(chunk.text)}</small>
        <span className="related-meta-v285">
          <em>{scoreLabel(item.score)}信頼</em>
          {item.semanticScore ? <em>意味 {item.semanticScore}%</em> : null}
          {item.titleScore ? <em>タイトル {item.titleScore}%</em> : null}
          {item.metaScore ? <em>メタ {item.metaScore}%</em> : null}
          {item.relationBoost ? <em>補正 +{item.relationBoost}</em> : null}
          {formatDate(chunk.updatedAt) ? <em>{formatDate(chunk.updatedAt)}</em> : null}
        </span>
      </span>
    </button>
  );
  ...
  return (
    <aside className={`workspace-related-panel-v285${compact ? " workspace-related-panel-compact-v286" : ""}`} aria-label="関連情報">
      ...
      <div className="related-index-status-v285">
        <span>{related?.available ? "Index ready" : "Index未作成"}</span>
        <small>{related ? `${related.indexedCount} chunks / ${total} hits / 厳選表示${related.hiddenLowScoreCount ? ` / 低関連${related.hiddenLowScoreCount}件非表示` : ""}` : loading ? "読み込み中" : "未取得"}</small>
      </div>
