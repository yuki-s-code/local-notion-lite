...
/** Browser-only fallbacks are intentionally bounded; normal pipelines run in DuckDB. */
const MAX_CLIENT_TRANSFORM_ROWS = 5_000;
...
// Keep only the current viewport and nearby result pages in memory. Large 100k-row
// results otherwise accumulate every page touched during a long scroll.
const ANALYSIS_TABLE_CACHED_PAGES = 7;
const EMPTY_ANALYSIS_SNAPSHOTS: AnalysisCellSnapshot[] = [];
...
type ExecutableCell = SqlCell | DataFrameCell | FunctionCell | PivotCell | SummaryCell | QualityCell | PreprocessCell | ImportCell | ChartCell;
type UpstreamResultOption = { id: string; label: string; result?: AnalysisQueryResult };
type NotebookOutlineItem = { id: string; title: string; level: number; index: number; kind: 'section' | 'markdown' };
type CellImpact = { direct: Array<{ id: string; title: string }>; total: Array<{ id: string; title: string }> };

function markdownHeadings(content: string): Array<{ title: string; level: number }> {
  return String(content || '').split(/\r?\n/).flatMap((line) => {
    const match = /^(#{1,3})\s+(.+?)\s*$/.exec(line);
    return match ? [{ level: match[1].length, title: match[2].replace(/[*_`]/g, '').trim().slice(0, 160) }] : [];
  }).filter((item) => item.title);
}

function MarkdownPreview({ content }: { content: string }) {
  const lines = String(content || '').split(/\r?\n/);
  return <div className="analysis-markdown-rendered">{lines.map((line, index) => {
    const heading = /^(#{1,3})\s+(.+?)\s*$/.exec(line);
    if (heading) {
      const Tag = `h${heading[1].length}` as 'h1' | 'h2' | 'h3';
      return <Tag key={`${index}-${line}`}>{heading[2]}</Tag>;
    }
    if (/^[-*]\s+/.test(line)) return <li key={`${index}-${line}`}>{line.replace(/^[-*]\s+/, '')}</li>;
    if (!line.trim()) return <div className="analysis-markdown-spacer" key={`space-${index}`} />;
    return <p key={`${index}-${line}`}>{line}</p>;
  })}</div>;
}

function SnapshotComparison({ result, snapshots, history }: { result: AnalysisQueryResult; snapshots: AnalysisCellSnapshot[]; history: AnalysisCellExecution[] }) {
  const [snapshotId, setSnapshotId] = useState('');
  const selected = snapshots.find((item) => item.id === snapshotId) || snapshots[0];
  const diff = useMemo(() => {
    if (!selected) return null;
    const keyOf = (row: Record<string, unknown>, columns: string[]) => columns.map((column) => JSON.stringify(row[column] ?? null)).join('\u0001');
    const previousKeys = new Set(selected.rows.map((row) => keyOf(row, selected.columns)));
    const currentRows = result.rows.slice(0, 1000);
    const currentKeys = new Set(currentRows.map((row) => keyOf(row, result.columns)));
    let retained = 0;
    currentKeys.forEach((key) => { if (previousKeys.has(key)) retained += 1; });
    ...
    return {
      rowDelta: result.rowCount - selected.rowCount,
      addedColumns: result.columns.filter((column) => !selected.columns.includes(column)),
      removedColumns: selected.columns.filter((column) => !result.columns.includes(column)),
      retained,
      added,
      removed,
      currentSample: currentRows.length,
      previousSample: selected.rows.length,
    };
  }, [result, selected]);
  if (!selected || !diff) return null;
  const previousRun = history[1];
  return <details className="analysis-snapshot-diff"><summary>前回・保存結果と比較</summary><div className="analysis-snapshot-diff-controls"><label>比較対象<select value={selected.id} onChange={(event) => setSnapshotId(event.target.value)}>{snapshots.map((snapshot) => <option key={snapshot.id} value={snapshot.id}>{snapshot.label}（{new Date(snapshot.createdAt).toLocaleString('ja-JP')}）</option>)}</select></label></div><div className="analysis-snapshot-diff-grid"><span><b>{diff.rowDelta >= 0 ? '+' : ''}{diff.rowDelta.toLocaleString()}</b> 行数差</span><span><b>{diff.added}</b> サンプル追加</span><span><b>{diff.removed}</b> サンプル削除</span><span><b>{diff.retained}</b> サンプル共通</span></div>{(diff.addedColumns.length || diff.removedColumns.length) > 0 && <p>列の変更：{diff.addedColumns.length ? `追加 ${diff.addedColumns.join('、')}` : ''}{diff.addedColumns.length && diff.removedColumns.length ? ' / ' : ''}{diff.removedColumns.length ? `削除 ${diff.removedColumns.join('、')}` : ''}</p>}{previousRun && <p>直前の実行との差：{result.rowCount - previousRun.rowCount >= 0 ? '+' : ''}{(result.rowCount - previousRun.rowCount).toLocaleString()}行 / 実行時間 {previousRun.elapsedMs}ms → {result.elapsedMs}ms</p>}<small>比較は端末内に保存した最大1,000行のスナップショットと、現在読み込まれている最大1,000行の結果サンプルで行います。大規模結果をブラウザへ追加取得しません。</small></details>;
}

function AnalysisFlowPanel({ nodes, edges, onSelect }: { nodes: Array<{ id: string; title: string; state: 'fresh' | 'stale' | 'idle' | 'setting' }>; edges: Array<{ from: string; to: string }>; onSelect: (id: string) => void }) {
  if (!nodes.length) return null;
  const byId = new Map(nodes.map((node) => [node.id, node]));
  const inputs = new Map<string, string[]>();
  edges.forEach((edge) => { const list = inputs.get(edge.to) || []; list.push(edge.from); inputs.set(edge.to, list); });
  const visibleNodes = nodes.slice(0, 80);
  return <div className="analysis-flow-panel"><b>データフロー</b><small>入力 → 出力。選択するとセルへ移動します。</small><div className="analysis-flow-list">{visibleNodes.map((node) => <button type="button" key={node.id} className={`analysis-flow-node ${node.state}`} onClick={() => onSelect(node.id)}><span>{(inputs.get(node.id) || []).length ? (inputs.get(node.id) || []).map((id) => byId.get(id)?.title || '入力').join(' + ') : '入力なし'}</span><strong>{node.title}</strong><em>{node.state === 'fresh' ? '最新' : node.state === 'stale' ? '再実行' : node.state === 'idle' ? '未実行' : '設定'}</em></button>)}</div>{nodes.length > visibleNodes.length && <small>表示は先頭 {visibleNodes.length} セルまでです。ノートをセクションで分けると追いやすくなります。</small>}</div>;
}

function isExecutableCell(cell: AnalysisCell): cell is ExecutableCell {
  return ['sql', 'dataframe', 'function', 'pivot', 'summary', 'quality', 'preprocess', 'import', 'chart'].includes(cell.type);
}
...
// SQL builders for the common notebook pipeline operations. They deliberately
// operate on one named temporary result on the server, so a 100k-row upstream
// result does not have to be fetched back into the renderer just to filter,
// select, sort, aggregate, or limit it.
const PIPELINE_SOURCE_NAME = '__pipeline_source';
const PIPELINE_SOURCE_TABLE = `result_${PIPELINE_SOURCE_NAME}`;
const PIPELINE_RIGHT_SOURCE_NAME = '__pipeline_right';
const PIPELINE_RIGHT_SOURCE_TABLE = `result_${PIPELINE_RIGHT_SOURCE_NAME}`;

function quoteAnalysisLiteral(value: unknown): string {
...
function namedPipelineSource(result: AnalysisQueryResult, name = PIPELINE_SOURCE_NAME): AnalysisNamedResult {
  return {
    name,
...
function dataframePipelineSql(source: AnalysisQueryResult, transform: AnalysisDataFrameTransform): string | null {
  const column = transform.column || source.columns[0] || '';
  const table = quoteAnalysisIdentifier(PIPELINE_SOURCE_TABLE);
  const sourceColumn = column && source.columns.includes(column) ? quoteAnalysisIdentifier(column) : '';
  if (transform.operation === 'select') {
    const selected = (transform.columns || []).filter((item) => source.columns.includes(item));
    if (!selected.length) return null;
    return `SELECT ${selected.map(quoteAnalysisIdentifier).join(', ')} FROM ${table}`;
  }
  if (transform.operation === 'sort' && sourceColumn) {
    return `SELECT * FROM ${table} ORDER BY CAST(${sourceColumn} AS VARCHAR) ${transform.direction === 'desc' ? 'DESC' : 'ASC'}`;
  }
  if (transform.operation === 'limit') {
    return `SELECT * FROM ${table} LIMIT ${Math.max(1, Math.min(MAX_ANALYSIS_ROWS, Number(transform.limit || 100)))}`;
  }
  if (transform.operation === 'filter' && sourceColumn) {
    const value = quoteAnalysisLiteral(transform.value ?? '');
    const where = transform.operator === 'notEmpty'
      ? `NULLIF(TRIM(CAST(${sourceColumn} AS VARCHAR)), '') IS NOT NULL`
      : transform.operator === 'greaterThan'
        ? `TRY_CAST(${sourceColumn} AS DOUBLE) > TRY_CAST(${value} AS DOUBLE)`
        : transform.operator === 'lessThan'
          ? `TRY_CAST(${sourceColumn} AS DOUBLE) < TRY_CAST(${value} AS DOUBLE)`
          : transform.operator === 'equals'
            ? `CAST(${sourceColumn} AS VARCHAR) = CAST(${value} AS VARCHAR)`
            : `LOWER(CAST(${sourceColumn} AS VARCHAR)) LIKE '%' || LOWER(CAST(${value} AS VARCHAR)) || '%'`;
    return `SELECT * FROM ${table} WHERE ${where}`;
  }
  return null;
}

function simplePivotPipelineSql(source: AnalysisQueryResult, pivot: AnalysisPivotTransform): string | null {
  // Dynamic cross-tab columns require a schema discovery pass. Keep that rare
  // mode on the legacy path; the non-cross-tab aggregate covers the common case
  // and can remain entirely in DuckDB.
  if (pivot.columnColumn || !pivot.rowColumn || !source.columns.includes(pivot.rowColumn)) return null;
  const rowColumn = quoteAnalysisIdentifier(pivot.rowColumn);
  const valueColumn = pivot.valueColumn && source.columns.includes(pivot.valueColumn) ? quoteAnalysisIdentifier(pivot.valueColumn) : '';
  const metric = pivot.aggregation === 'count'
    ? 'COUNT(*) AS "count"'
    : pivot.aggregation === 'sum'
      ? `SUM(COALESCE(TRY_CAST(${valueColumn} AS DOUBLE), 0)) AS ${quoteAnalysisIdentifier(`sum_${pivot.valueColumn}`)}`
      : `AVG(TRY_CAST(${valueColumn} AS DOUBLE)) AS ${quoteAnalysisIdentifier(`avg_${pivot.valueColumn}`)}`;
  if ((pivot.aggregation === 'sum' || pivot.aggregation === 'average') && !valueColumn) return null;
  return `SELECT ${rowColumn}, ${metric} FROM ${quoteAnalysisIdentifier(PIPELINE_SOURCE_TABLE)} GROUP BY ${rowColumn} ORDER BY ${rowColumn}`;
}

function summaryPipelineSql(source: AnalysisQueryResult, summary: AnalysisSummaryTransform): string | null {
  // Preserve the existing automatic numeric-column discovery for legacy cells;
  // when a numeric column is explicitly chosen, DuckDB can compute statistics
  // without transferring every source row to the renderer.
  if (!summary.numericColumn || !source.columns.includes(summary.numericColumn)) return null;
  const numeric = quoteAnalysisIdentifier(summary.numericColumn);
  const group = summary.groupColumn && source.columns.includes(summary.groupColumn) ? quoteAnalysisIdentifier(summary.groupColumn) : '';
  const missing = source.columns.map((column) => {
    const escaped = quoteAnalysisIdentifier(column);
    return `CASE WHEN ${escaped} IS NULL OR TRIM(CAST(${escaped} AS VARCHAR)) = '' THEN 1 ELSE 0 END`;
  }).join(' + ') || '0';
  const value = `TRY_CAST(${numeric} AS DOUBLE)`;
  const selectGroup = group ? `COALESCE(CAST(${group} AS VARCHAR), '(空欄)') AS "group"` : `'全体' AS "group"`;
  const groupBy = group ? ` GROUP BY ${group}` : '';
  const orderBy = group ? ' ORDER BY "group"' : '';
  return `SELECT ${selectGroup}, COUNT(*) AS "row_count", SUM(${missing}) AS "missing_cells", ${quoteAnalysisLiteral(summary.numericColumn)} AS "numeric_column", COUNT(${value}) AS "numeric_count", AVG(${value}) AS "mean", MEDIAN(${value}) AS "median", MIN(${value}) AS "min", MAX(${value}) AS "max", STDDEV_SAMP(${value}) AS "stddev", COUNT(DISTINCT CAST(${numeric} AS VARCHAR)) AS "unique_values" FROM ${quoteAnalysisIdentifier(PIPELINE_SOURCE_TABLE)}${groupBy}${orderBy}`;
}


function analysisBlankSql(column: string): string {
  const ref = quoteAnalysisIdentifier(column);
  return `${ref} IS NULL OR TRIM(CAST(${ref} AS VARCHAR)) = ''`;
}

function analysisSelectWithReplacements(columns: string[], replacements: Record<string, string>): string {
  return columns.map((column) => `${replacements[column] || quoteAnalysisIdentifier(column)} AS ${quoteAnalysisIdentifier(column)}`).join(', ');
}

function preprocessPipelineSql(source: AnalysisQueryResult, transform: AnalysisPreprocessTransform): string | null {
  if (!transform.sourceCellId) return null;
  const table = quoteAnalysisIdentifier(PIPELINE_SOURCE_TABLE);
  const columns = source.columns;
  const chosen = (transform.columns || []).filter((column) => columns.includes(column));
  const selectedColumns = analysisSelectWithReplacements(columns, {});
  if (transform.operation === 'removeDuplicates') {
    if (!chosen.length) return null;
    const partition = chosen.map(quoteAnalysisIdentifier).join(', ');
    return `SELECT * EXCLUDE (__analysis_row_number) FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY ${partition}) AS __analysis_row_number FROM ${table}) AS deduplicated WHERE __analysis_row_number = 1`;
  }
  if (transform.operation === 'handleMissing') {
    if (!chosen.length) return null;
    const strategy = transform.missingStrategy || 'custom';
    if (strategy === 'forwardFill') return null;
    if (strategy === 'dropRows') {
      return `SELECT ${selectedColumns} FROM ${table} WHERE ${chosen.map((column) => `NOT (${analysisBlankSql(column)})`).join(' AND ')}`;
    }
    const replacements: Record<string, string> = {};
    if (strategy === 'mean' || strategy === 'median') {
      const statExpressions = chosen.map((column) => {
        const ref = quoteAnalysisIdentifier(column);
        const value = `TRY_CAST(REPLACE(REPLACE(REPLACE(CAST(${ref} AS VARCHAR), ',', ''), '，', ''), '％', '') AS DOUBLE)`;
        const aggregate = strategy === 'mean' ? `AVG(${value})` : `MEDIAN(${value})`;
        return `${aggregate} AS ${quoteAnalysisIdentifier(`__fill_${column}`)}`;
      }).join(', ');
      for (const column of chosen) {
        const ref = quoteAnalysisIdentifier(column);
        replacements[column] = `CASE WHEN ${analysisBlankSql(column)} THEN stats.${quoteAnalysisIdentifier(`__fill_${column}`)} ELSE ${ref} END`;
      }
      return `WITH stats AS (SELECT ${statExpressions} FROM ${table}) SELECT ${analysisSelectWithReplacements(columns, replacements)} FROM ${table} CROSS JOIN stats`;
    }
    for (const column of chosen) {
      const ref = quoteAnalysisIdentifier(column);
      const fill = strategy === 'zero' ? '0' : quoteAnalysisLiteral(transform.fillValue ?? '');
      replacements[column] = `CASE WHEN ${analysisBlankSql(column)} THEN ${fill} ELSE ${ref} END`;
    }
    return `SELECT ${analysisSelectWithReplacements(columns, replacements)} FROM ${table}`;
  }
  if (transform.operation === 'trimText') {
    if (!chosen.length) return null;
    const replacements = Object.fromEntries(chosen.map((column) => [column, `CASE WHEN ${analysisBlankSql(column)} THEN ${quoteAnalysisIdentifier(column)} ELSE TRIM(CAST(${quoteAnalysisIdentifier(column)} AS VARCHAR)) END`]));
    return `SELECT ${analysisSelectWithReplacements(columns, replacements)} FROM ${table}`;
  }
  if (transform.operation === 'coerceNumber' || transform.operation === 'coerceDate') {
    if (!chosen.length) return null;
    const replacements: Record<string, string> = {};
    const invalidPredicates: string[] = [];
    for (const column of chosen) {
      const ref = quoteAnalysisIdentifier(column);
      const cast = transform.operation === 'coerceNumber'
        ? `TRY_CAST(REPLACE(REPLACE(REPLACE(REPLACE(CAST(${ref} AS VARCHAR), ',', ''), '，', ''), '％', ''), '−', '-') AS DOUBLE)`
        : `TRY_CAST(CAST(${ref} AS VARCHAR) AS DATE)`;
      replacements[column] = `CASE WHEN ${analysisBlankSql(column)} THEN NULL ELSE ${cast} END`;
      invalidPredicates.push(`(${analysisBlankSql(column)} OR ${cast} IS NOT NULL)`);
    }
    const where = transform.invalidAction === 'dropRows' ? ` WHERE ${invalidPredicates.join(' AND ')}` : '';
    return `SELECT ${analysisSelectWithReplacements(columns, replacements)} FROM ${table}${where}`;
  }
  if (transform.operation === 'replaceValues') {
    const column = transform.column || '';
    if (!columns.includes(column)) return null;
    const ref = quoteAnalysisIdentifier(column);
    const replacements = { [column]: `CASE WHEN CAST(${ref} AS VARCHAR) = CAST(${quoteAnalysisLiteral(transform.findValue ?? '')} AS VARCHAR) THEN ${quoteAnalysisLiteral(transform.replaceValue ?? '')} ELSE ${ref} END` };
    return `SELECT ${analysisSelectWithReplacements(columns, replacements)} FROM ${table}`;
  }
  if (transform.operation === 'excludeOutliers') {
    ...
    if (!columns.includes(column)) return null;
    const ref = quoteAnalysisIdentifier(column);
    const value = `TRY_CAST(REPLACE(REPLACE(REPLACE(REPLACE(CAST(${ref} AS VARCHAR), ',', ''), '，', ''), '％', ''), '−', '-') AS DOUBLE)`;
    const bounds = transform.outlierMethod === 'threeSigma'
      ? `AVG(__analysis_value) - STDDEV_POP(__analysis_value) * 3 AS __lower, AVG(__analysis_value) + STDDEV_POP(__analysis_value) * 3 AS __upper`
      : `QUANTILE_CONT(__analysis_value, 0.25) - (QUANTILE_CONT(__analysis_value, 0.75) - QUANTILE_CONT(__analysis_value, 0.25)) * 1.5 AS __lower, QUANTILE_CONT(__analysis_value, 0.75) + (QUANTILE_CONT(__analysis_value, 0.75) - QUANTILE_CONT(__analysis_value, 0.25)) * 1.5 AS __upper`;
    return `WITH source_with_value AS (SELECT *, ${value} AS __analysis_value FROM ${table}), bounds AS (SELECT ${bounds} FROM source_with_value WHERE __analysis_value IS NOT NULL) SELECT ${columns.map((columnName) => `source_with_value.${quoteAnalysisIdentifier(columnName)} AS ${quoteAnalysisIdentifier(columnName)}`).join(', ')} FROM source_with_value CROSS JOIN bounds WHERE __analysis_value IS NULL OR (__analysis_value >= __lower AND __analysis_value <= __upper)`;
  }
  return null;
}

function qualityPipelineSql(source: AnalysisQueryResult, quality: AnalysisQualityTransform): string | null {
  const table = quoteAnalysisIdentifier(PIPELINE_SOURCE_TABLE);
  const columns = (quality.columns || []).filter((column) => source.columns.includes(column));
  const checked = columns.length ? columns : source.columns;
  if (!checked.length) return null;
  const statements: string[] = [];
  if (quality.checkMissing !== false) {
    for (const column of checked) {
      const ref = quoteAnalysisIdentifier(column);
      statements.push(`SELECT '欠損値' AS "check", ${quoteAnalysisLiteral(column)} AS "column", COUNT(*) FILTER (WHERE ${analysisBlankSql(column)}) AS "issue_count", CASE WHEN COUNT(*) = 0 THEN 0 ELSE CAST(COUNT(*) FILTER (WHERE ${analysisBlankSql(column)}) AS DOUBLE) / COUNT(*) END AS "rate", CASE WHEN COUNT(*) FILTER (WHERE ${analysisBlankSql(column)}) > 0 THEN '要確認' ELSE '問題なし' END AS "status" FROM ${table}`);
    }
  }
  if (quality.checkDuplicates !== false) {
    const groupBy = checked.map(quoteAnalysisIdentifier).join(', ');
    const label = checked.join(', ');
    statements.push(`WITH duplicate_groups AS (SELECT COUNT(*) AS __count FROM ${table} GROUP BY ${groupBy}), totals AS (SELECT COUNT(*) AS __total FROM ${table}) SELECT '重複行' AS "check", ${quoteAnalysisLiteral(label)} AS "column", COALESCE(SUM(CASE WHEN __count > 1 THEN __count - 1 ELSE 0 END), 0) AS "issue_count", CASE WHEN totals.__total = 0 THEN 0 ELSE CAST(COALESCE(SUM(CASE WHEN __count > 1 THEN __count - 1 ELSE 0 END), 0) AS DOUBLE) / totals.__total END AS "rate", CASE WHEN COALESCE(SUM(CASE WHEN __count > 1 THEN __count - 1 ELSE 0 END), 0) > 0 THEN '要確認' ELSE '問題なし' END AS "status" FROM duplicate_groups CROSS JOIN totals GROUP BY totals.__total`);
  }
  if (quality.checkNonNumeric) {
    for (const column of checked) {
      const ref = quoteAnalysisIdentifier(column);
      const cast = `TRY_CAST(REPLACE(REPLACE(REPLACE(REPLACE(CAST(${ref} AS VARCHAR), ',', ''), '，', ''), '％', ''), '−', '-') AS DOUBLE)`;
      const populated = `NOT (${analysisBlankSql(column)})`;
      statements.push(`SELECT '数値形式外' AS "check", ${quoteAnalysisLiteral(column)} AS "column", COUNT(*) FILTER (WHERE ${populated} AND ${cast} IS NULL) AS "issue_count", CASE WHEN COUNT(*) FILTER (WHERE ${populated}) = 0 THEN 0 ELSE CAST(COUNT(*) FILTER (WHERE ${populated} AND ${cast} IS NULL) AS DOUBLE) / COUNT(*) FILTER (WHERE ${populated}) END AS "rate", CASE WHEN COUNT(*) FILTER (WHERE ${populated} AND ${cast} IS NULL) > 0 THEN '要確認' ELSE '問題なし' END AS "status" FROM ${table}`);
    }
  }
  return statements.length ? statements.join(' UNION ALL ') : null;
}

function functionPipelineSql(source: AnalysisQueryResult, transform: AnalysisFunctionTransform, joinedSource?: AnalysisQueryResult): string | null {
  const table = quoteAnalysisIdentifier(PIPELINE_SOURCE_TABLE);
  const outputColumn = transform.outputColumn || ({
    join: '結合結果', unpivot: '値', splitText: '分割値', dateDiff: '日数差', conditionalColumn: '条件列', formula: '計算値', dropDuplicates: '重複除去', renameColumn: '列名変更',
  } as const)[transform.operation as 'join' | 'unpivot' | 'splitText' | 'dateDiff' | 'conditionalColumn' | 'formula' | 'dropDuplicates' | 'renameColumn'];
  const sourceColumn = transform.valueColumn && source.columns.includes(transform.valueColumn) ? quoteAnalysisIdentifier(transform.valueColumn) : '';

  if (transform.operation === 'join') {
    if (!joinedSource || !transform.joinLeftColumn || !transform.joinRightColumn || !source.columns.includes(transform.joinLeftColumn) || !joinedSource.columns.includes(transform.joinRightColumn)) return null;
    const rightTable = quoteAnalysisIdentifier(PIPELINE_RIGHT_SOURCE_TABLE);
    const leftKey = quoteAnalysisIdentifier(transform.joinLeftColumn);
    const rightKey = quoteAnalysisIdentifier(transform.joinRightColumn);
    const projectedRight = joinedSource.columns.map((column) => {
      const output = source.columns.includes(column) ? `right_${column}` : column;
      return `right.${quoteAnalysisIdentifier(column)} AS ${quoteAnalysisIdentifier(output)}`;
    });
    const joinType = transform.joinType === 'inner' ? 'INNER JOIN' : 'LEFT JOIN';
    return `SELECT left_source.*, ${projectedRight.join(', ')} FROM ${table} AS left_source ${joinType} ${rightTable} AS right ON CAST(left_source.${leftKey} AS VARCHAR) = CAST(right.${rightKey} AS VARCHAR)`;
  }

  if (transform.operation === 'unpivot') {
    const values = (transform.valueColumns || []).filter((column) => source.columns.includes(column));
    if (!values.length) return null;
    const identifiers = source.columns.filter((column) => !values.includes(column));
    const selectIdentifiers = identifiers.map((column) => quoteAnalysisIdentifier(column)).join(', ');
    const statements = values.map((column) => `SELECT ${selectIdentifiers ? `${selectIdentifiers}, ` : ''}${quoteAnalysisLiteral(column)} AS "variable", ${quoteAnalysisIdentifier(column)} AS ${quoteAnalysisIdentifier(outputColumn || '値')} FROM ${table}`);
    return statements.join(' UNION ALL ');
  }

  if (transform.operation === 'splitText' && sourceColumn) {
    const delimiter = quoteAnalysisLiteral(transform.delimiter ?? ',');
    const index = Math.max(1, Math.min(101, Number(transform.splitIndex || 0) + 1));
    return `SELECT *, NULLIF(TRIM(list_extract(string_split(CAST(${sourceColumn} AS VARCHAR), ${delimiter}), ${index})), '') AS ${quoteAnalysisIdentifier(outputColumn || '分割値')} FROM ${table}`;
  }

  if (transform.operation === 'dateDiff' && sourceColumn && transform.secondColumn && source.columns.includes(transform.secondColumn)) {
    return `SELECT *, CASE WHEN TRY_CAST(${sourceColumn} AS DATE) IS NULL OR TRY_CAST(${quoteAnalysisIdentifier(transform.secondColumn)} AS DATE) IS NULL THEN NULL ELSE date_diff('day', TRY_CAST(${sourceColumn} AS DATE), TRY_CAST(${quoteAnalysisIdentifier(transform.secondColumn)} AS DATE)) END AS ${quoteAnalysisIdentifier(outputColumn || '日数差')} FROM ${table}`;
  }

  if (transform.operation === 'dropDuplicates' && sourceColumn) {
    return `SELECT * EXCLUDE (__analysis_row_number) FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY CAST(${sourceColumn} AS VARCHAR)) AS __analysis_row_number FROM ${table}) AS deduplicated WHERE __analysis_row_number = 1`;
  }

  if (transform.operation === 'renameColumn' && sourceColumn) {
    const renameTo = String(transform.renameTo || '').trim();
    if (!renameTo || (source.columns.includes(renameTo) && renameTo !== transform.valueColumn)) return null;
    return `SELECT ${source.columns.map((column) => column === transform.valueColumn ? `${quoteAnalysisIdentifier(column)} AS ${quoteAnalysisIdentifier(renameTo)}` : quoteAnalysisIdentifier(column)).join(', ')} FROM ${table}`;
  }

  if (transform.operation === 'conditionalColumn' && sourceColumn) {
    const expected = quoteAnalysisLiteral(transform.conditionValue ?? '');
    const condition = transform.conditionOperator === 'notEmpty'
      ? `NULLIF(TRIM(CAST(${sourceColumn} AS VARCHAR)), '') IS NOT NULL`
      : transform.conditionOperator === 'contains'
        ? `LOWER(CAST(${sourceColumn} AS VARCHAR)) LIKE '%' || LOWER(CAST(${expected} AS VARCHAR)) || '%'`
        : transform.conditionOperator === 'greaterThan'
          ? `TRY_CAST(${sourceColumn} AS DOUBLE) > TRY_CAST(${expected} AS DOUBLE)`
          : transform.conditionOperator === 'lessThan'
            ? `TRY_CAST(${sourceColumn} AS DOUBLE) < TRY_CAST(${expected} AS DOUBLE)`
            : `CAST(${sourceColumn} AS VARCHAR) = CAST(${expected} AS VARCHAR)`;
    return `SELECT *, CASE WHEN ${condition} THEN ${quoteAnalysisLiteral(transform.trueValue || '該当')} ELSE ${quoteAnalysisLiteral(transform.falseValue || '非該当')} END AS ${quoteAnalysisIdentifier(outputColumn || '条件列')} FROM ${table}`;
  }

  if (transform.operation === 'formula' && sourceColumn) {
    const kind = transform.formulaKind || 'arithmetic';
    const raw = String(transform.formulaValue ?? '');
    const numericValue = Number(raw);
    if (['arithmetic', 'round', 'ifGreater'].includes(kind) && raw.trim() && !Number.isFinite(numericValue)) return null;
    const number = `TRY_CAST(${sourceColumn} AS DOUBLE)`;
    const output = quoteAnalysisIdentifier(outputColumn || '計算値');
    if (kind === 'arithmetic') {
      const literal = quoteAnalysisLiteral(Number.isFinite(numericValue) ? numericValue : 0);
      const operator = transform.formulaOperator === 'subtract' ? '-' : transform.formulaOperator === 'multiply' ? '*' : transform.formulaOperator === 'divide' ? '/' : '+';
      const expression = operator === '/' ? `CASE WHEN TRY_CAST(${literal} AS DOUBLE) = 0 THEN NULL ELSE ${number} / TRY_CAST(${literal} AS DOUBLE) END` : `${number} ${operator} TRY_CAST(${literal} AS DOUBLE)`;
      return `SELECT *, ${expression} AS ${output} FROM ${table}`;
    }
    if (kind === 'round') return `SELECT *, ROUND(${number}, ${Math.max(0, Math.min(10, Math.floor(Number.isFinite(numericValue) ? numericValue : 0)))}) AS ${output} FROM ${table}`;
    if (kind === 'absolute') return `SELECT *, ABS(${number}) AS ${output} FROM ${table}`;
    if (kind === 'year') return `SELECT *, EXTRACT(YEAR FROM TRY_CAST(${sourceColumn} AS DATE)) AS ${output} FROM ${table}`;
    if (kind === 'month') return `SELECT *, EXTRACT(MONTH FROM TRY_CAST(${sourceColumn} AS DATE)) AS ${output} FROM ${table}`;
    if (kind === 'coalesce') return `SELECT *, CASE WHEN ${sourceColumn} IS NULL OR TRIM(CAST(${sourceColumn} AS VARCHAR)) = '' THEN ${quoteAnalysisLiteral(raw)} ELSE ${sourceColumn} END AS ${output} FROM ${table}`;
    if (kind === 'dateDiff' && transform.secondColumn && source.columns.includes(transform.secondColumn)) return `SELECT *, CASE WHEN TRY_CAST(${sourceColumn} AS DATE) IS NULL OR TRY_CAST(${quoteAnalysisIdentifier(transform.secondColumn)} AS DATE) IS NULL THEN NULL ELSE date_diff('day', TRY_CAST(${sourceColumn} AS DATE), TRY_CAST(${quoteAnalysisIdentifier(transform.secondColumn)} AS DATE)) END AS ${output} FROM ${table}`;
    if (kind === 'ifGreater') return `SELECT *, CASE WHEN ${number} > TRY_CAST(${quoteAnalysisLiteral(Number.isFinite(numericValue) ? numericValue : 0)} AS DOUBLE) THEN ${quoteAnalysisLiteral(transform.trueValue || '該当')} ELSE ${quoteAnalysisLiteral(transform.falseValue || '非該当')} END AS ${output} FROM ${table}`;
  }
  return null;
}
...
function downloadCsvBlob(blob: Blob, title: string): void {
...
function downloadCsv(result: AnalysisQueryResult, title: string): void {
  ...
  downloadCsvBlob(new Blob([`\ufeff${csv}`], { type: 'text/csv;charset=utf-8' }), title);
}
...
const ResultTable = memo(function ResultTable({ result, api, onOpenOrigin }: { result: AnalysisQueryResult; api: ApiClient | null; onOpenOrigin?: (target: OriginTarget) => void }) {
  ...
  const pagesRef = useRef<Record<number, Array<Record<string, unknown>>>>({ 0: result.rows });
  ...
  const trimCachedPages = useCallback((next: Record<number, Array<Record<string, unknown>>>, centerPage: number) => {
    const candidates = Object.keys(next).map(Number).filter((page) => page !== 0);
    if (candidates.length <= ANALYSIS_TABLE_CACHED_PAGES - 1) return next;
    const keep = new Set([0, ...candidates.sort((a, b) => Math.abs(a - centerPage) - Math.abs(b - centerPage)).slice(0, ANALYSIS_TABLE_CACHED_PAGES - 1)]);
    return Object.fromEntries(Object.entries(next).filter(([page]) => keep.has(Number(page))));
  }, []);
  const loadPage = useCallback(async (page: number, centerPage: number) => {
    if (!result.resultId || !api || pagesRef.current[page] || loadingPagesRef.current.has(page)) return;
    ...
    try {
      ...
      setPages((current) => {
        const next = trimCachedPages({ ...current, [page]: response.rows }, centerPage);
        pagesRef.current = next;
        return next;
      });
    } catch {
      // Cached server results can expire. The cell execution error remains the primary error surface.
    } finally {
      loadingPagesRef.current.delete(page);
    }
  }, [api, pageSize, result.resultId, trimCachedPages]);
  useEffect(() => {
    ...
    const centerPage = Math.floor((fromPage + toPage) / 2);
    // Fetch the visible range plus one adjacent page. This avoids a visible loading gap at page boundaries.
    for (let page = Math.max(0, fromPage - 1); page <= toPage + 1; page += 1) void loadPage(page, centerPage);
  }, [api, firstRow, lastRow, loadPage, pageSize, result.resultId]);
  const getRow = (index: number): Record<string, unknown> | undefined => {
    const page = Math.floor(index / pageSize);
    if (page === 0) return result.rows[index] || pages[0]?.[index];
  ...
  return <>
    {result.truncated && <div className="analysis-preview-limit-note">分析結果は最大100,000行まで保持します。表は必要なページだけ取得・描画し、スクロール済みの遠いページは自動解放します。</div>}
...
const AnalysisNotebookCellCard = memo(function AnalysisNotebookCellCard({
  cell, index, totalCells, hidden, selected, expanded, presentationMode, dictionary, api, result, sourceResult, upstream,
  record, history, freshnessState, freshnessLabel, snapshots, isRunning, cancelRequested, canRun, notebookTitle, onSelect, onShowResult,
  onUpdateCell, onMove, onRemove, onRun, onCancel, onCapture, onPin, onChartChange, onOpenOrigin, impact,
}: {
  cell: AnalysisCell; index: number; totalCells: number; hidden: boolean; selected: boolean; expanded: boolean; presentationMode: boolean;
  dictionary: AnalysisDataDictionary | null; api: ApiClient | null; result?: AnalysisQueryResult; sourceResult?: AnalysisQueryResult;
  upstream: UpstreamResultOption[]; record?: AnalysisCellExecution; history?: AnalysisCellExecution[];
  freshnessState: 'fresh' | 'stale' | 'idle'; freshnessLabel: string; snapshots: AnalysisCellSnapshot[];
  isRunning: boolean; cancelRequested: boolean; canRun: boolean; notebookTitle: string;
  onSelect: (id: string, executable: boolean) => void; onShowResult: (id: string) => void;
  onUpdateCell: (id: string, updater: (cell: AnalysisCell) => AnalysisCell) => void;
  onMove: (index: number, direction: -1 | 1) => void; onRemove: (id: string) => void;
  onRun: (cell: ExecutableCell) => void; onCancel: () => void; onCapture: (id: string) => void; onPin: (cell: ExecutableCell) => void;
  onChartChange: (id: string, chart: AnalysisChart) => void; onOpenOrigin: (target: OriginTarget) => void; impact?: CellImpact;
}) {
  const executable = isExecutableCell(cell);
  const showResult = expanded || selected;
  const resultPanel = executable && cell.type !== 'chart' ? (() => {
    if (!result) return <div className="analysis-empty-result">このセルを実行すると、結果をここに表示します。</div>;
    if (!showResult) return <div className="analysis-result-collapsed"><span>結果 {result.rowCount.toLocaleString()}行 ・ {result.elapsedMs.toLocaleString()}ms</span><button className="secondary" type="button" onClick={() => onShowResult(cell.id)}>結果を表示</button></div>;
    return <ResultPanel title={notebookTitle} result={result} cell={{ id: cell.id, type: 'sql', sql: '', chart: cell.chart }} api={api} onChartChange={(chart) => onChartChange(cell.id, chart)} onOpenOrigin={onOpenOrigin} />;
  })() : null;
  const subtitle = cell.type === 'section' ? (cell.description || '分析のまとまりを作成します。') : cell.type === 'function' ? 'pandas風の定型処理を安全に実行' : cell.type === 'preprocess' ? '元データを変更せず、分析用の表だけ整えます' : cell.type === 'import' ? '端末ローカルにだけ取り込む一時DataFrame' : cell.type === 'variable' ? `SQLでは {{${cell.variable.name}}}` : '分析セル';
  return <article id={`analysis-cell-${cell.id}`} hidden={hidden} className={`analysis-cell analysis-cell-${cell.type} ${selected ? 'selected' : ''}`} onClick={() => onSelect(cell.id, executable)}>
    <div className="analysis-cell-head"><div><b>{cellTitle(cell)}</b><span>{subtitle}</span></div>{!presentationMode && <div className="analysis-cell-actions"><button className="secondary" disabled={index === 0} onClick={() => onMove(index, -1)}>↑</button><button className="secondary" disabled={index === totalCells - 1} onClick={() => onMove(index, 1)}>↓</button><button className="secondary danger" onClick={() => onRemove(cell.id)}>削除</button>{executable && <><button className="secondary" disabled={!result} onClick={() => onCapture(cell.id)}>スナップショット</button><button className="secondary" disabled={!result || !api} onClick={() => onPin(cell)}>ダッシュボードに固定</button>{isRunning ? <button className="secondary danger" type="button" onClick={onCancel}>{cancelRequested ? '中止しています…' : '■ 中止'}</button> : <button className="primary" disabled={!canRun || !api} onClick={() => onRun(cell)}>▶ 実行</button>}</>}</div>}</div>
    {executable && <ExecutionBadge record={record} history={history} freshness={{ state: freshnessState, label: freshnessLabel }} />}
    {executable && impact && impact.total.length > 0 && <details className="analysis-impact"><summary>実行前の影響範囲：下流 {impact.total.length} セル</summary><p>このセルを実行すると、下流セルは再実行が必要になります。</p><div>{impact.direct.map((item) => <span key={item.id}>{item.title}</span>)}{impact.total.length > impact.direct.length && <span>ほか {impact.total.length - impact.direct.length} セル</span>}</div></details>}
    {executable && snapshots.length > 0 && <><div className="analysis-parameter-help">スナップショット {snapshots.length}件：{snapshots[0].rowCount.toLocaleString()}行（{new Date(snapshots[0].createdAt).toLocaleString('ja-JP')}）</div>{result && <SnapshotComparison result={result} snapshots={snapshots} history={history || []} />}</>}
    {cell.type === 'section' ? <div className="analysis-parameter-grid"><label>見出し<input value={cell.title} onChange={(e) => onUpdateCell(cell.id, (current) => ({ ...(current as SectionCell), title: e.target.value.slice(0, 160) }))} /></label><label>説明<input value={cell.description || ''} onChange={(e) => onUpdateCell(cell.id, (current) => ({ ...(current as SectionCell), description: e.target.value.slice(0, 500) }))} /></label><label><input type="checkbox" checked={cell.collapsed === true} onChange={(e) => onUpdateCell(cell.id, (current) => ({ ...(current as SectionCell), collapsed: e.target.checked }))} />このセクションを折りたたむ</label></div>
      : cell.type === 'sql' ? <><div className="analysis-sql-output"><label>出力名（任意）<input value={cell.outputName || ''} onChange={(e) => onUpdateCell(cell.id, (current) => ({ ...(current as SqlCell), outputName: resultName(e.target.value, '') }))} /><small>次のSQLでは <code>{cell.outputName ? `result_${cell.outputName}` : '出力名を設定すると参照できます'}</code></small></label></div><SqlEditor value={cell.sql} dictionary={dictionary} onChange={(sql) => onUpdateCell(cell.id, (current) => ({ ...(current as SqlCell), sql }))} />{resultPanel}</>
      : cell.type === 'dataframe' ? <><DataFrameEditor cell={cell} upstream={upstream} onChange={(transform) => onUpdateCell(cell.id, (current) => ({ ...(current as DataFrameCell), transform }))} />{resultPanel}</>
      : cell.type === 'function' ? <><FunctionEditor cell={cell} upstream={upstream} onChange={(transform) => onUpdateCell(cell.id, (current) => ({ ...(current as FunctionCell), transform }))} />{resultPanel}</>
      : cell.type === 'import' ? <><ImportEditor cell={cell} onChange={(imported) => onUpdateCell(cell.id, (current) => ({ ...(current as ImportCell), imported }))} />{resultPanel}</>
      : cell.type === 'pivot' ? <><PivotEditor cell={cell} upstream={upstream} onChange={(pivot) => onUpdateCell(cell.id, (current) => ({ ...(current as PivotCell), pivot }))} />{resultPanel}</>
      : cell.type === 'summary' ? <><SummaryEditor cell={cell} upstream={upstream} onChange={(summary) => onUpdateCell(cell.id, (current) => ({ ...(current as SummaryCell), summary }))} />{resultPanel}</>
      : cell.type === 'quality' ? <><QualityEditor cell={cell} upstream={upstream} onChange={(quality) => onUpdateCell(cell.id, (current) => ({ ...(current as QualityCell), quality }))} />{resultPanel}</>
      : cell.type === 'preprocess' ? <><PreprocessEditor cell={cell} upstream={upstream} onChange={(preprocess) => onUpdateCell(cell.id, (current) => ({ ...(current as PreprocessCell), preprocess }))} />{resultPanel}</>
      : cell.type === 'chart' ? <><ChartEditor cell={cell} upstream={upstream} result={sourceResult} onChange={(patch) => onUpdateCell(cell.id, (current) => ({ ...(current as ChartCell), ...patch }))} />{result ? (showResult ? <><PlotView result={result} chart={cell.chart} /><ResultTable result={result} api={api} onOpenOrigin={onOpenOrigin} /></> : <div className="analysis-result-collapsed"><span>グラフ結果 {result.rowCount.toLocaleString()}行 ・ {result.elapsedMs.toLocaleString()}ms</span><button className="secondary" type="button" onClick={() => onShowResult(cell.id)}>結果を表示</button></div>) : <div className="analysis-empty-result">入力結果を選択して実行すると、独立したグラフを表示します。</div>}</>
      : cell.type === 'parameter' ? <ParameterEditor cell={cell} onChange={(parameter) => onUpdateCell(cell.id, (current) => ({ ...(current as ParameterCell), parameter }))} />
      : cell.type === 'variable' ? <ParameterEditor cell={{ id: cell.id, type: 'parameter', parameter: cell.variable }} onChange={(variable) => onUpdateCell(cell.id, (current) => ({ ...(current as VariableCell), variable }))} />
      : <div className="analysis-markdown-cell"><textarea value={cell.content} onChange={(e) => onUpdateCell(cell.id, (current) => ({ ...(current as Extract<AnalysisCell, { type: 'markdown' }>), content: e.target.value.slice(0, 12000) }))} placeholder="# 見出し\n\n分析メモ" /><div className="analysis-markdown-preview"><span>プレビュー・目次へ反映</span><MarkdownPreview content={cell.content || 'メモを入力するとここに表示されます。'} /></div></div>}
  </article>;
});
...
function ResultPanel({ result, cell, onChartChange, title, api, onOpenOrigin }: { result: AnalysisQueryResult; cell: SqlCell; title: string; api: ApiClient | null; onChartChange: (chart: SqlCell['chart']) => void; onOpenOrigin?: (target: OriginTarget) => void }) {
  ...
  return <div className="analysis-result-card">
    <div className="analysis-result-head"><div><b>結果</b><span>{summary} ・ {result.elapsedMs}ms</span></div><button className="secondary" onClick={() => { if (result.resultId && result.hasMore && api) { void api.downloadAnalysisResultCsv(result.resultId).then((blob) => downloadCsvBlob(blob, title)).catch(() => undefined); } else downloadCsv(result, title); }}>CSV出力</button></div>
...
export function AnalysisNotebookScreen({ api, onBack, onStatus, onOpenPage, onOpenDatabase, onOpenDatabaseRow, onOpenJournal }: Props) {
  ...
  const [notebooks, setNotebooks] = useState<AnalysisNotebookSummary[]>([]);
  ...
  const cellsRef = useRef<AnalysisCell[]>(cells);
  const resultsRef = useRef<Record<string, AnalysisQueryResult>>(results);
  ...
  const openOrigin = useCallback((target: OriginTarget) => {
  ...
  }, [onOpenDatabase, onOpenDatabaseRow, onOpenJournal, onOpenPage, onStatus]);
  const refresh = async () => {
    if (!api) return;
  ...
  const openNotebook = async (id: string, selectedCell = '') => {
    if (!api || !id || id === notebook.id) return;
    setBusy(true);
    setError('');
    try {
      const opened = await api.getAnalysisNotebook(id);
      setNotebook(opened);
      setResults({});
      setSelectedCellId(selectedCell);
      setSidebarTab('notebooks');
      if (selectedCell) window.setTimeout(() => document.getElementById(`analysis-cell-${selectedCell}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 0);
    } catch (e: any) {
      setError(e?.message || '分析ノートを開けませんでした。');
    } finally {
      setBusy(false);
    }
  };
  ...
  const setCells = useCallback((next: AnalysisCell[]) => setNotebook((current) => ({ ...current, cells: next })), []);
  const updateCell = useCallback((id: string, updater: (cell: AnalysisCell) => AnalysisCell) => setNotebook((current) => ({ ...current, cells: notebookCells(current).map((cell) => cell.id === id ? updater(cell) : cell) })), []);
  const moveCell = useCallback((index: number, direction: -1 | 1) => setNotebook((current) => { const currentCells = notebookCells(current); const target = index + direction; if (target < 0 || target >= currentCells.length) return current; const next = [...currentCells]; [next[index], next[target]] = [next[target], next[index]]; return { ...current, cells: next }; }), []);
  const removeCell = useCallback((id: string) => { if (cellsRef.current.length <= 1) { setError('分析ノートには少なくとも1つのセルが必要です。'); return; } setNotebook((current) => ({ ...current, cells: notebookCells(current).filter((cell) => cell.id !== id) })); setResults((current) => { const next = { ...current }; delete next[id]; return next; }); }, []);
  const sync = async () => {
    if (!api) return;
    setBusy(true);
    setError('');
    const timer = window.setInterval(() => {
      void api.getAnalysisStatus().then(setStatus).catch(() => undefined);
    }, 450);
    try {
      const next = await api.syncAnalysisData();
      setStatus(next);
      onStatus('分析用DuckDBキャッシュを同期しました');
    } catch (e: any) {
  ...
  const hydrateResult = async (result: AnalysisQueryResult, signal?: AbortSignal): Promise<AnalysisQueryResult> => {
    if (!result.resultId || !result.hasMore || !api) return result;
    if (result.rowCount > MAX_CLIENT_TRANSFORM_ROWS) {
      throw new Error(`この処理はブラウザ側で全行を扱うため、${MAX_CLIENT_TRANSFORM_ROWS.toLocaleString()}行までに絞る必要があります。SQL・フィルター・前処理を使って入力結果を小さくしてください。`);
    }
    const pageSize = 500;
    const rows: Array<Record<string, unknown>> = [];
    for (let page = 0; rows.length < result.rowCount; page += 1) {
      const next = await api.getAnalysisResultPage(result.resultId, page, pageSize, signal);
      rows.push(...next.rows);
      if (!next.hasMore) return { ...next, rows, rowCount: next.rowCount, pageSize: rows.length, hasMore: false };
    }
    return { ...result, rows, pageSize: rows.length, hasMore: false };
  };
  ...
  const cancelAnalysisRun = useCallback(() => {
    ...
    setAnalysisCancelRequested(true);
  ...
  const runCell = async (cell: SqlCell | DataFrameCell | FunctionCell | PivotCell | SummaryCell | QualityCell | PreprocessCell | ImportCell | ChartCell, initialResults = results, refreshDependencies = true, commitRender = true, persistNotebook = true): Promise<AnalysisQueryResult | null> => {
    if (!api) return null;
    // A new top-level run owns one AbortController. Cancelling stops pending HTTP
    // work and prevents late responses from replacing newer cell results.
    analysisAbortRef.current?.abort();
    ...
    const throwIfCancelled = () => {
      if (abortController.signal.aborted) {
        const error = new Error('分析セルの実行を中止しました。');
        error.name = 'AbortError';
        throw error;
      }
    };
    const runSql = (sql: string, nextParameters: AnalysisParameter[] = [], namedResults: AnalysisNamedResult[] = []) =>
      api.runAnalysisSql(sql, nextParameters, namedResults, abortController.signal);
    const runPivot = (pivot: AnalysisPivotTransform, namedSource: AnalysisNamedResult) =>
      api.runAnalysisPivot(pivot, namedSource, abortController.signal);
    ...
    const execute = async (targetId: string): Promise<AnalysisQueryResult> => {
      ...
      try {
        ...
        const next = target.type === 'sql'
          ? await runSql(target.sql, parameters, namedResultsBefore(target.id, currentResults))
          : target.type === 'dataframe' ? await (async () => {
              const source = currentResults[target.transform.sourceCellId];
              if (!source) throw new Error('DataFrameセルの入力結果が見つかりません。');
              const sql = dataframePipelineSql(source, target.transform);
              return sql ? runSql(sql, [], [namedPipelineSource(source)]) : transformResult(await hydrateResult(source, abortController.signal), target.transform);
            })()
          : target.type === 'function' ? await (async () => {
              const source = currentResults[target.transform.sourceCellId];
              if (!source) throw new Error('分析関数セルの入力結果が見つかりません。');
              const joinedSource = target.transform.joinSourceCellId ? currentResults[target.transform.joinSourceCellId] : undefined;
              const sql = functionPipelineSql(source, target.transform, joinedSource);
              const namedResults = [namedPipelineSource(source), ...(joinedSource ? [namedPipelineSource(joinedSource, PIPELINE_RIGHT_SOURCE_NAME)] : [])];
              return sql ? runSql(sql, [], namedResults) : functionResult(await hydrateResult(source, abortController.signal), target.transform, joinedSource ? await hydrateResult(joinedSource, abortController.signal) : undefined);
            })()
          : target.type === 'import' ? importResult(target.imported)
          : target.type === 'pivot' ? await (async () => {
              const source = currentResults[target.pivot.sourceCellId];
              if (!source) throw new Error('ピボットセルの入力結果が見つかりません。');
              const sql = simplePivotPipelineSql(source, target.pivot);
              if (sql) return runSql(sql, [], [namedPipelineSource(source)]);
              if (target.pivot.columnColumn) return runPivot(target.pivot, namedPipelineSource(source));
              return pivotResult(await hydrateResult(source, abortController.signal), target.pivot);
            })()
          : target.type === 'summary' ? await (async () => {
              const source = currentResults[target.summary.sourceCellId];
              if (!source) throw new Error('統計・要約セルの入力結果が見つかりません。');
              const sql = summaryPipelineSql(source, target.summary);
              return sql ? runSql(sql, [], [namedPipelineSource(source)]) : summaryResult(await hydrateResult(source, abortController.signal), target.summary);
            })()
          : target.type === 'quality' ? await (async () => {
              const source = currentResults[target.quality.sourceCellId];
              if (!source) throw new Error('品質チェックする入力結果が見つかりません。');
              const sql = qualityPipelineSql(source, target.quality);
              return sql ? runSql(sql, [], [namedPipelineSource(source)]) : qualityResult(await hydrateResult(source, abortController.signal), target.quality);
            })()
          : target.type === 'preprocess' ? await (async () => {
              const source = currentResults[target.preprocess.sourceCellId];
              if (!source) throw new Error('前処理する入力結果が見つかりません。');
              const sql = preprocessPipelineSql(source, target.preprocess);
              return sql ? runSql(sql, [], [namedPipelineSource(source)]) : preprocessResult(await hydrateResult(source, abortController.signal), target.preprocess);
            })()
  ...
  const isExecutable = isExecutableCell;
  ...
  const openDashboardPin = (pin: AnalysisDashboardPin) => {
    ...
    if (target) void openNotebook(target.id, pin.cellId);
  ...
  const deleteNotebook = async (target: Pick<AnalysisNotebook, 'id' | 'title'>) => {
    if (!api) return;
    ...
    try {
      ...
      if (notebook.id === target.id) {
        ...
        if (replacement) await openNotebook(replacement.id);
        else create();
        setResults({});
      }
  ...
  const pinCellToDashboardRef = useRef(pinCellToDashboard);
  runCellRef.current = runCell;
  captureSnapshotRef.current = captureSnapshot;
  pinCellToDashboardRef.current = pinCellToDashboard;
  const selectCell = useCallback((id: string, executable: boolean) => { setSelectedCellId(id); if (executable) setExpandedResultCellId(id); }, []);
  const showResult = useCallback((id: string) => { setSelectedCellId(id); setExpandedResultCellId(id); }, []);
  const runCellFromCard = useCallback((target: ExecutableCell) => { void runCellRef.current(target); }, []);
  const captureSnapshotFromCard = useCallback((id: string) => { captureSnapshotRef.current(id); }, []);
  const pinCellFromCard = useCallback((target: ExecutableCell) => { void pinCellToDashboardRef.current(target); }, []);
  const chartChangeFromCard = useCallback((id: string, chart: AnalysisChart) => updateCell(id, (current) => ({ ...(current as ExecutableCell), chart } as AnalysisCell)), [updateCell]);
  const upstreamCacheRef = useRef<Record<string, { signature: string; value: UpstreamResultOption[] }>>({});
  const upstreamByCellId = useMemo(() => {
    const output: Record<string, UpstreamResultOption[]> = {};
    for (let index = 0; index < cells.length; index += 1) {
      const signature = cells.slice(0, index).map((item) => {
        const outputName = item.type === 'sql' ? item.outputName : item.type === 'dataframe' || item.type === 'function' ? item.transform.outputName : item.type === 'pivot' ? item.pivot.outputName : item.type === 'summary' ? item.summary.outputName : item.type === 'quality' ? item.quality.outputName : item.type === 'preprocess' ? item.preprocess.outputName : item.type === 'import' ? item.imported.outputName : '';
        return `${item.id}:${outputName}:${results[item.id]?.resultId || ''}:${results[item.id]?.executedAt || ''}`;
      }).join('|');
      const cached = upstreamCacheRef.current[cells[index].id];
      if (cached?.signature === signature) output[cells[index].id] = cached.value;
      else {
        const value: UpstreamResultOption[] = cells.slice(0, index).flatMap((item) => {
          if (!isExecutableCell(item) || item.type === 'chart') return [];
          const label = item.type === 'sql' ? `SQL: ${item.outputName || item.id}` : item.type === 'dataframe' ? `DataFrame: ${item.transform.outputName || item.id}` : item.type === 'function' ? `関数: ${item.transform.outputName || item.id}` : item.type === 'pivot' ? `Pivot: ${item.pivot.outputName || item.id}` : item.type === 'summary' ? `統計: ${item.summary.outputName || item.id}` : item.type === 'quality' ? `品質: ${item.quality.outputName || item.id}` : item.type === 'preprocess' ? `前処理: ${item.preprocess.outputName || item.id}` : `取込: ${item.imported.outputName || item.id}`;
          return [{ id: item.id, label, result: results[item.id] }];
        });
        upstreamCacheRef.current[cells[index].id] = { signature, value };
        output[cells[index].id] = value;
      }
    }
    return output;
  }, [cells, results]);
  const safeDependencies = useCallback((cell: AnalysisCell): string[] => {
    if (!isExecutableCell(cell)) return [];
    try { return resultDependencies(cell); } catch { return []; }
  }, [cells]);
  const dataFlow = useMemo(() => {
    const executableCells = cells.filter(isExecutableCell);
    const parameterCells = cells.filter((cell): cell is ParameterCell | VariableCell => cell.type === 'parameter' || cell.type === 'variable');
    const resultEdges = executableCells.flatMap((cell) => safeDependencies(cell).map((from) => ({ from, to: cell.id })));
    const parameterEdges = executableCells.flatMap((cell) => cell.type === 'sql'
      ? parameterCells.filter((parameterCell) => cell.sql.includes(`{{${parameterCell.type === 'parameter' ? parameterCell.parameter.name : parameterCell.variable.name}}}`)).map((parameterCell) => ({ from: parameterCell.id, to: cell.id }))
      : []);
    const edges = [...resultEdges, ...parameterEdges];
    const outgoing = new Map<string, string[]>();
    for (const edge of edges) { const list = outgoing.get(edge.from) || []; list.push(edge.to); outgoing.set(edge.from, list); }
    const flowNodes = cells.filter((cell) => isExecutableCell(cell) || cell.type === 'parameter' || cell.type === 'variable').map((cell) => ({
      id: cell.id, title: cellTitle(cell), state: cell.type === 'parameter' || cell.type === 'variable' ? 'setting' as const : freshnessFor(cell).state,
    }));
    const downstream = new Map<string, CellImpact>();
    for (const cell of executableCells) {
      const directIds = outgoing.get(cell.id) || [];
      const seen = new Set<string>(); const queue = [...directIds];
      while (queue.length) { const id = queue.shift()!; if (seen.has(id)) continue; seen.add(id); (outgoing.get(id) || []).forEach((nextId) => queue.push(nextId)); }
      const titleFor = (id: string): string => {
        const target = cells.find((item) => item.id === id);
        return target ? cellTitle(target) : 'セル';
      };
      downstream.set(cell.id, { direct: directIds.map((id) => ({ id, title: titleFor(id) })), total: [...seen].map((id) => ({ id, title: titleFor(id) })) });
    }
    return { nodes: flowNodes, edges, downstream };
  }, [cells, safeDependencies, results, executionHistory, status, parameters]);
  const selectedIndex = Math.max(0, cells.findIndex((cell) => cell.id === selectedCellId));
  const outlineItems = cells.flatMap((cell, index): NotebookOutlineItem[] => {
    if (cell.type === 'section') return [{ id: cell.id, title: cell.title || 'セクション', level: 1, index, kind: 'section' }];
    if (cell.type === 'markdown') return markdownHeadings(cell.content).map((heading, headingIndex) => ({ id: cell.id, title: heading.title, level: Math.min(4, heading.level + 1), index: index + headingIndex / 1000, kind: 'markdown' }));
    return [];
  });
  const parameterUsage = useMemo(() => parameters.map((parameter) => ({ parameter, uses: cells.filter((cell) => cell.type === 'sql' && cell.sql.includes(`{{${parameter.name}}}`)).length })), [parameters, cells]);
...
${draft.warnings.map((warning) => `- ${warning}`).join('\n')}` : ''}`);
  ...
  return <section className={`analysis-notebook-screen ${presentationMode ? 'analysis-presentation-mode' : ''}`}>
    ...
    <div className="analysis-layout"><aside className="analysis-sidebar"><div className="analysis-sidebar-tabs"><button className={sidebarTab === 'notebooks' ? 'active' : ''} onClick={() => setSidebarTab('notebooks')}>ノート</button><button className={sidebarTab === 'dashboard' ? 'active' : ''} onClick={() => setSidebarTab('dashboard')}>KPI</button><button className={sidebarTab === 'dictionary' ? 'active' : ''} onClick={() => setSidebarTab('dictionary')}>データ辞書</button><button className={sidebarTab === 'templates' ? 'active' : ''} onClick={() => setSidebarTab('templates')}>テンプレート</button><button className={sidebarTab === 'operations' ? 'active' : ''} onClick={() => setSidebarTab('operations')}>運用</button></div>
      {sidebarTab === 'notebooks' && <><div className="analysis-sidebar-head"><b>保存済み分析</b><button onClick={create} title="新規ノート">＋</button></div>{notebooks.length ? notebooks.map((item) => <div key={item.id} className={`analysis-notebook-item-wrap ${item.id === notebook.id ? 'active' : ''}`}><button className="analysis-notebook-item" onClick={() => { void openNotebook(item.id); }} disabled={busy || !!runningCellId}><strong>{item.title}</strong><small>{item.updatedAt ? new Date(item.updatedAt).toLocaleDateString('ja-JP') : '未保存'}</small></button><button type="button" className="analysis-notebook-delete" title={`「${item.title || '無題の分析'}」を削除`} aria-label={`「${item.title || '無題の分析'}」を削除`} disabled={busy || !!runningCellId} onClick={() => void deleteNotebook(item)}>×</button></div>) : <p className="analysis-empty-list">保存済みの分析はまだありません。</p>}{outlineItems.length > 0 && <div className="analysis-outline"><b>目次</b>{outlineItems.map((item, outlineIndex) => <button type="button" key={`${item.id}-${item.index}-${outlineIndex}`} className={`level-${item.level} ${item.kind}`} onClick={() => document.getElementById(`analysis-cell-${item.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' })}>{item.kind === 'markdown' ? '⌁ ' : ''}{item.title}</button>)}</div>}{parameterUsage.length > 0 && <div className="analysis-parameter-summary"><b>分析条件</b>{parameterUsage.map(({ parameter, uses }) => <button type="button" key={parameter.name} onClick={() => document.getElementById(`analysis-cell-${cells.find((cell) => (cell.type === 'parameter' ? cell.parameter.name : cell.type === 'variable' ? cell.variable.name : '') === parameter.name)?.id || ''}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })}><strong>{parameter.label || parameter.name}</strong><span>{parameter.value || '未設定'} ・ SQL {uses}件</span></button>)}</div>}<AnalysisFlowPanel nodes={dataFlow.nodes} edges={dataFlow.edges} onSelect={(id) => { setSelectedCellId(id); document.getElementById(`analysis-cell-${id}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' }); }} /></>}
      ...
      <div className="analysis-cells">{renderedCells.map(({ cell, index, hidden }) => <AnalysisNotebookCellCard
        key={cell.id} cell={cell} index={index} totalCells={cells.length} hidden={hidden}
        selected={selectedCellId === cell.id} expanded={expandedResultCellId === cell.id} presentationMode={presentationMode}
        dictionary={dictionary} api={api} result={results[cell.id]} sourceResult={cell.type === 'chart' ? results[cell.sourceCellId] : undefined}
        upstream={upstreamByCellId[cell.id] || []} record={latestExecution(cell.id)} history={executionHistory[cell.id]} freshnessState={freshnessFor(cell).state} freshnessLabel={freshnessFor(cell).label} snapshots={notebook.snapshots?.[cell.id] || EMPTY_ANALYSIS_SNAPSHOTS}
        isRunning={runningCellId === cell.id} cancelRequested={analysisCancelRequested} canRun={!runningCellId} notebookTitle={notebook.title}
        onSelect={selectCell} onShowResult={showResult}
        onUpdateCell={updateCell} onMove={moveCell} onRemove={removeCell} onRun={runCellFromCard} onCancel={cancelAnalysisRun}
        onCapture={captureSnapshotFromCard} onPin={pinCellFromCard} onChartChange={chartChangeFromCard} onOpenOrigin={openOrigin} impact={dataFlow.downstream.get(cell.id)}
      />)}</div></div></div>
