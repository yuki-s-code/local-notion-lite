...
export function LocalSmartAssistView({ api, pages, databases, journals, inboxItems, tasks, currentPage, currentDb, tagAliases = {}, tagPresentation = {}, onOpenPage, onOpenDatabase, onOpenDatabaseRow, onOpenJournal, onOpenInbox, onOpenTasks }: {
  ...
  const [attachmentIndexRebuild, setAttachmentIndexRebuild] = useState<any>(null);
  ...
  async function startAttachmentIndexRebuildFromAdmin() {
    if (!api) return;
    try {
      const status = await api.startAttachmentIndexRebuild();
      setAttachmentIndexRebuild(status);
      setTransformerModelMessage('添付Indexの再構築を開始しました。進捗はこの画面で確認できます。');
    } catch (err: any) {
      setTransformerModelMessage(`添付Index再構築を開始できませんでした: ${err?.message || err}`);
    }
  }

  async function cancelAttachmentIndexRebuildFromAdmin() {
    if (!api) return;
    try {
      const status = await api.cancelAttachmentIndexRebuild();
      setAttachmentIndexRebuild(status);
      setTransformerModelMessage('添付Index再構築の中止を要求しました。既存Indexは保持されます。');
    } catch (err: any) {
      setTransformerModelMessage(`添付Index再構築の中止に失敗しました: ${err?.message || err}`);
    }
  }

  useEffect(() => {
    if (!api || attachmentIndexRebuild?.state !== 'running') return;
    const timer = window.setInterval(() => {
      void api.getAttachmentIndexRebuildStatus().then((status) => {
        setAttachmentIndexRebuild(status);
        if (status?.state !== 'running') void refreshWorkspaceDerivedIndexInfo();
      }).catch(() => undefined);
    }, 700);
    return () => window.clearInterval(timer);
  }, [api, attachmentIndexRebuild?.state]);
  ...
  const reindexWorkspaceSemanticFailureFromAdmin = async (failure: any) => {
    ...
    try {
      const result = await api.reindexWorkspaceSemanticSource(
        sourceId,
        failure?.type ? String(failure.type) : undefined,
        failure?.databaseId ? String(failure.databaseId) : undefined,
      );
      const info = await api.getWorkspaceSemanticIndexInfo();
      setWorkspaceSemanticInfo(info);
          ...
          {smartAdminTab === 'model' ? <div className="smart-admin-model-panel-v236">
            ...
            <section className="smart-model-settings-card-v236">
              ...
              <div className="smart-model-actions-v236">
                ...
                <button type="button" onClick={startAttachmentIndexRebuildFromAdmin} disabled={attachmentIndexRebuild?.state === 'running'}>添付Indexのみ再構築</button>
                {attachmentIndexRebuild?.state === 'running' ? <button type="button" className="danger" onClick={cancelAttachmentIndexRebuildFromAdmin}>添付Index再構築を中止</button> : null}
            ...
            {workspaceDerivedIndexInfo ? <section className="smart-cache-topology-v320">
              ...
              {attachmentIndexRebuild ? <p className="muted-small">添付Index再構築: {attachmentIndexRebuild.state === 'running' ? `${attachmentIndexRebuild.processed ?? 0} / ${attachmentIndexRebuild.total ?? 0} の添付元を確認中（${attachmentIndexRebuild.indexed ?? 0}件）` : attachmentIndexRebuild.message || attachmentIndexRebuild.state}</p> : null}
