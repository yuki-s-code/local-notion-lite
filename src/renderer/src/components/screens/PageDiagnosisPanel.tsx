...
function lightDiagnosis(title: string, p: PageProperties): Issue[] {
  const issues: Issue[] = [];
  ...
  if (!issues.length) issues.push({ kind: 'good', title: '大きな確認事項は見つかりませんでした', detail: 'プロパティ上の確認事項は現在ありません。本文構造はパネルを開いた時に確認します。' });
  return issues;
}
...
export function PageDiagnosisPanel({ title, markdown, properties, onAskAi }: Props) {
  ...
  const issues = useMemo(
    () => open ? diagnosis(title, markdown, properties) : lightDiagnosis(title, properties),
    [open, title, markdown, properties],
  );
