...
import { createVersionedJsonStore } from "../../workspace/storage";
...
const STORAGE_KEY = "local-notion:workspace-document-tabs-v774";
const LEGACY_STORAGE_KEYS = [
  "local-notion:workspace-workbench-v518",
  "local-notion:workspace-workbench-v476",
];
...
const workbenchStore = createVersionedJsonStore<StoredWorkbench>({
  key: STORAGE_KEY,
  legacyKeys: LEGACY_STORAGE_KEYS,
  fallback: () => ({ tabs: [], pinned: [], closedTabs: [] }),
  sanitize: sanitizeStored,
});

function readStored(): StoredWorkbench {
  return workbenchStore.read();
}

function writeStored(value: StoredWorkbench): void {
  workbenchStore.write(value);
}
